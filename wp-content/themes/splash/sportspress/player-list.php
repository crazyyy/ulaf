...
$defaults = array(
	...
	'show_title'            => get_option( 'sportspress_list_show_title', 'yes' ) === 'yes',
	'show_player_photo'     => get_option( 'sportspress_list_show_photos', 'no' ) === 'yes',
	'show_player_flag'      => get_option( 'sportspress_list_show_flags', 'no' ) === 'yes',
	'link_posts'            => get_option( 'sportspress_link_players', 'yes' ) === 'yes',
	...
	'paginated'             => get_option( 'sportspress_list_paginated', 'yes' ) === 'yes',
...
if ( null === $grouping || 'default' === $grouping ) :
...
foreach ( $groups as $group ) :
	...
	foreach ( $data as $player_id => $row ) :
		if ( empty( $group->term_id ) || has_term( $group->term_id, 'sp_position', $player_id ) ) :
			...
			$tbody .= '<tr class="' . ( 0 === $i % 2 ? 'odd' : 'even' ) . '">';

			// Rank or number
			if ( ! is_array( $labels ) || array_key_exists( 'number', $labels ) ) :
				if ( isset( $orderby ) && 'number' !== $orderby ) :
			...
			foreach ( $labels as $key => $value ) :
				if ( in_array( $key, array( 'number', 'name', 'team', 'position' ), true ) ) {
					continue;
				}
...
endforeach;
	...
	$output .= '<div class="sp-table-wrapper"><table class="sp-player-list sp-data-table' . ( $sortable ? ' sp-sortable-table' : '' ) . ( $responsive ? ' sp-responsive-table ' . $identifier : '' ) . ( $scrollable ? ' sp-scrollable-table' : '' ) . ( $paginated ? ' sp-paginated-table' : '' ) . '" data-sp-rows="' . $rows . '"><thead><tr>';

	if ( ! is_array( $labels ) || array_key_exists( 'number', $labels ) ) :
		if ( in_array( $orderby, array( 'number', 'name' ), true ) ) :
			$output .= '<th class="data-number">#</th>';
		else :
			$output .= '<th class="data-rank">' . esc_html__( 'Rank', 'splash' ) . '</th>';
		endif;
	endif;

	foreach ( $labels as $key => $label ) :
		if ( 'number' !== $key && ( ! is_array( $columns ) || 'name' === $key || in_array( $key, $columns, true ) ) ) {
	...
	if ( $show_all_players_link ) :
		$output .= '<div class="sp-player-list-link sp-view-all-link"><a href="' . get_permalink( $id ) . '">' . esc_html__( 'View all players', 'splash' ) . '</a></div>';
	endif;
...
?>
<?php
echo wp_kses_post( splash_sanitize_text_field( $output ) );

...
function envato_market_themes_column( $group = 'install' ) {
	...
	foreach ( $premium as $slug => $theme ) :
		...
		if ( true === $theme['hasUpdate'] ) {
			...
			if ( current_user_can( 'update_themes' ) ) {
				...
				$updateActions['update'] = sprintf(
					...
					esc_attr__( 'Update %s now', 'splash' ),
					esc_attr( $name ),
					esc_attr( $slug ),
					esc_attr( $theme['version'] ),
					esc_html__( 'Update Available', 'splash' )
				);
				...
				$updateActions['details'] = sprintf(
					...
					esc_attr( $name ),
					sprintf(
						__( 'View version %1$s details.', 'splash' ),
						$theme['version']
					)
				);
			}
		...
		if ( 'active' === $group && current_user_can( 'edit_theme_options' ) && current_user_can( 'customize' ) ) {
			...
			$actions['customize'] = '<a href="' . esc_url( $customize_url ) . '" class="button button-primary load-customize hide-if-no-customize"><span aria-hidden="true">' . esc_html__( 'Customize', 'splash' ) . '</span><span class="screen-reader-text">' . sprintf( esc_html__( 'Customize &#8220;%s&#8221;', 'splash' ), $name ) . '</span></a>';
			...
			if ( $can_activate && current_user_can( 'switch_themes' ) ) {
				...
				// Activate link.
				$actions['activate'] = '<a href="' . esc_url( $activate_link ) . '" class="button"><span aria-hidden="true">' . esc_html__( 'Activate', 'splash' ) . '</span><span class="screen-reader-text">' . sprintf( esc_html__( 'Activate &#8220;%s&#8221;', 'splash' ), $name ) . '</span></a>';
				...
				if ( current_user_can( 'edit_theme_options' ) && current_user_can( 'customize' ) ) {
					...
					$actions['customize_preview'] = '<a href="' . esc_url( $preview_url ) . '" class="button button-primary load-customize hide-if-no-customize"><span aria-hidden="true">' . esc_html__( 'Live Preview', 'splash' ) . '</span><span class="screen-reader-text">' . sprintf( esc_html__( 'Live Preview &#8220;%s&#8221;', 'splash' ), $name ) . '</span></a>';
				}
			}
			...
			<a href="' . wp_nonce_url( $install_link, 'install-theme_' . $theme['id'] ) . '" class="button button-primary">
				<span aria-hidden="true">' . esc_html__( 'Install', 'splash' ) . '</span>
				<span class="screen-reader-text">' . sprintf( esc_html__( 'Install %s', 'splash' ), $name ) . '</span>
			</a>';
		}
				...
				<div class="envato-card-top">
					...
					<div class="column-name">
						<h4>
							...
							<span class="version" aria-label="<?php esc_attr_e( 'Version %s', 'splash' ); ?>"><?php echo esc_html( sprintf( esc_html__( 'Version %s', 'splash' ), $version ) ); ?></span>
						</h4>
					</div>
					<div class="column-description">
						...
						<p class="author">
							<cite><?php esc_html_e( 'By', 'splash' ); ?> <?php echo wp_kses_post( $author_link ); ?></cite>
						</p>
					</div>
	...
	foreach ( $premium as $slug => $plugin ) :
		...
		if ( 'active' === $group ) {
			...
			<a href="' . wp_nonce_url( $deactivate_link, 'deactivate-plugin_' . $slug ) . '" class="button">
				<span aria-hidden="true">' . esc_html__( 'Deactivate', 'splash' ) . '</span>
				<span class="screen-reader-text">' . sprintf( esc_html__( 'Deactivate %s', 'splash' ), $name ) . '</span>
			</a>';
		} elseif ( 'installed' === $group ) {
			if ( ! is_multisite() && current_user_can( 'delete_plugins' ) ) {
				...
				<a href="' . wp_nonce_url( $delete_link, 'bulk-plugins' ) . '" class="button-delete">
					<span aria-hidden="true">' . esc_html__( 'Delete', 'splash' ) . '</span>
					<span class="screen-reader-text">' . sprintf( esc_html__( 'Delete %s', 'splash' ), $name ) . '</span>
				</a>';
			}
			...
			if ( ! is_multisite() && current_user_can( 'activate_plugins' ) ) {
				...
				<a href="' . wp_nonce_url( $activate_link, 'activate-plugin_' . $slug ) . '" class="button">
					<span aria-hidden="true">' . esc_html__( 'Activate', 'splash' ) . '</span>
					<span class="screen-reader-text">' . sprintf( esc_html__( 'Activate %s', 'splash' ), $name ) . '</span>
				</a>';
			}
			...
			if ( is_multisite() ) {
				if ( current_user_can( 'manage_sites' ) ) {
					...
					<a href="' . esc_url( admin_url( wp_nonce_url( 'plugins.php?action=activate&amp;plugin=' . urlencode( $slug ), 'activate-plugin_' . $slug ) ) ) . '" class="button">
						<span aria-hidden="true">' . esc_html__( 'Site Activate', 'splash' ) . '</span>
						<span class="screen-reader-text">' . sprintf( esc_html__( 'Site Activate %s', 'splash' ), $name ) . '</span>
					</a>';
				}
				...
				if ( current_user_can( 'manage_network_plugins' ) ) {
					...
					<a href="' . esc_url( network_admin_url( wp_nonce_url( 'plugins.php?action=activate&amp;plugin=' . urlencode( $slug ), 'activate-plugin_' . $slug ) ) ) . '" class="button">
						<span aria-hidden="true">' . esc_html__( 'Network Activate', 'splash' ) . '</span>
						<span class="screen-reader-text">' . sprintf( esc_html__( 'Network Activate %s', 'splash' ), $name ) . '</span>
					</a>';
				}
			}
			// @codeCoverageIgnoreEnd
			...
			<div class="<?php echo esc_attr( implode( ' ', $classes ) ); ?>">
				<div class="envato-card-top">
					...
					<div class="column-name">
						<h4>
							<a href="<?php echo esc_url( $url ); ?>"><?php echo esc_html( $name ); ?></a>
							<span class="version" aria-label="<?php esc_attr_e( 'Version %s', 'splash' ); ?>"><?php echo esc_html( sprintf( esc_html__( 'Version %s', 'splash' ), ( isset( $plugins[ $slug ] ) ? $plugins[ $slug ]['Version'] : $version ) ) ); ?></span>
						</h4>
					</div>
					<div class="column-description">
						<div class="description">

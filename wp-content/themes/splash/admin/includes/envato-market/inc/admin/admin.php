...
if ( ! class_exists( 'Envato_Market_Admin' ) && class_exists( 'Envato_Market' ) ) :
	...
	class Envato_Market_Admin {
		...
		public function __clone() {
			_doing_it_wrong( __FUNCTION__, esc_html__( 'Cheatin&#8217; huh?', 'splash' ), '1.0.0' );
		}
		...
		public function maybe_delete_transients() {
			if ( isset( $_POST[ envato_market()->get_option_name() ] ) ) {
				...
				if ( isset( $_POST['_wpnonce'] ) && ! wp_verify_nonce( $_POST['_wpnonce'], envato_market()->get_slug() . '-options' ) ) {
		 			wp_die( esc_html__( 'You do not have sufficient permissions to delete transients.', 'splash' ) );
				}
		...
		public function admin_enqueue_script() {
			...
			$exports = array(
				...
				'i18n' => array(
					'save' => esc_html__( 'Save', 'splash' ),
					'remove' => esc_html__( 'Remove', 'splash' ),
					'cancel' => esc_html__( 'Cancel', 'splash' ),
					'error' => esc_html__( 'An unknown error occurred. Try again.', 'splash' ),
				),
			);
		...
		public function render_templates() {
			...
			<script type="text/html" id="tmpl-envato-market-auth-check-button">
				<a href="<?php echo esc_url( add_query_arg( array( 'authorization' => 'check' ), envato_market()->get_page_url() ) ); ?>" class="button button-secondary auth-check-button" style="margin:0 5px"><?php esc_html_e( 'Test API Connection', 'splash' ); ?></a>
			</script>

			<script type="text/html" id="tmpl-envato-market-item">
				<li data-id="{{ data.id }}">
					<span class="item-name"><?php esc_html_e( 'ID', 'splash' ); ?>: {{ data.id }} - {{ data.name }}</span>
					<button class="item-delete dashicons dashicons-dismiss">
						<span class="screen-reader-text"><?php esc_html_e( 'Delete', 'splash' ) ?></span>
					</button>
			...
			<script type="text/html" id="tmpl-envato-market-dialog-remove">
				<div id="envato-market-dialog-remove" title="<?php esc_html_e( 'Remove Item', 'splash' ) ?>">
					<p><?php esc_html_e( 'You are about to remove the connection between the Envato Market API and this item. You cannot undo this action.', 'splash' ) ?></p>
				</div>
			</script>

			<script type="text/html" id="tmpl-envato-market-dialog-form">
				<div id="envato-market-dialog-form" title="<?php esc_html_e( 'Add Item', 'splash' ) ?>">
					<form>
						<fieldset>
							<label for="token"><?php esc_html_e( 'Token', 'splash' ) ?></label>
							<input type="text" name="token" class="widefat" value="" />
							<p class="description"><?php esc_html_e( 'Enter the Envato API Personal Token.', 'splash' ); ?></p>
							<label for="id"><?php esc_html_e( 'Item ID', 'splash' ) ?></label>
							<input type="text" name="id" class="widefat" value="" />
							<p class="description"><?php esc_html_e( 'Enter the Envato Item ID.', 'splash' ); ?></p>
						...
						<div class="envato-card-top">
							...
							<div class="column-name">
								<h4>
									...
									<span class="version" aria-label="<?php esc_attr_e( 'Version %s', 'splash' ); ?>"><?php esc_html_e( 'Version', 'splash' ); ?> {{ data.version }}</span>
								</h4>
							</div>
							<div class="column-description">
								...
								<p class="author">
									<cite><?php esc_html_e( 'By', 'splash' ); ?> {{ data.author }}</cite>
								</p>
							</div>
						</div>
						<div class="envato-card-bottom">
							<div class="column-actions">
								<a href="{{{ data.install }}}" class="button button-primary">
									<span aria-hidden="true"><?php esc_html_e( 'Install', 'splash' ); ?></span>
									<span class="screen-reader-text"><?php esc_html_e( 'Install', 'splash' ); ?> {{ data.name }}</span>
								</a>
							</div>
						</div>
					</div>
		...
		public function register_settings() {
			...
			add_settings_section(
				...
				__( 'Global OAuth Personal Token', 'splash' ),
			...
			add_settings_section(
				...
				__( 'Single Use OAuth Personal Tokens', 'splash' ),
		...
		public function install_plugin( $plugin ) {
			if ( ! current_user_can( 'install_plugins' ) ) {
				$msg = '
				<div class="wrap">
					<h1>' . esc_html__( 'Installing Plugin...', 'splash' ) . '</h1>
					<p>' . esc_html__( 'You do not have sufficient permissions to install plugins on this site.', 'splash' ) . '</p>
					<a href="' . esc_url( 'admin.php?page=' . envato_market()->get_slug() . '&tab=plugins' ) . '">' . esc_html__( 'Return to Plugin Installer', 'splash' ) . '</a>
				</div>';
				wp_die( $msg );
			}
			...
			if ( empty( $array_api ) ) {
				...
				<div class="wrap">
					<h1>' . esc_html__( 'Installing Plugin...', 'splash' ) . '</h1>
					<p>' . esc_html__( 'An error occurred, please check that the item ID is correct.', 'splash' ) . '</p>
					<a href="' . esc_url( 'admin.php?page=' . envato_market()->get_slug() . '&tab=plugins' ) . '">' . esc_html__( 'Return to Plugin Installer', 'splash' ) . '</a>
				</div>';
				wp_die( $msg );
			}

			$title = sprintf( esc_html__( 'Installing Plugin: %s', 'splash' ), $api->name . ' ' . $api->version );
		...
		public function install_theme( $theme ) {
			if ( ! current_user_can( 'install_themes' ) ) {
				$msg = '
				<div class="wrap">
					<h1>' . esc_html__( 'Installing Theme...', 'splash' ) . '</h1>
					<p>' . esc_html__( 'You do not have sufficient permissions to install themes on this site.', 'splash' ) . '</p>
					<a href="' . esc_url( 'admin.php?page=' . envato_market()->get_slug() . '&tab=themes' ) . '">' . esc_html__( 'Return to Theme Installer', 'splash' ) . '</a>
				</div>';
				wp_die( $msg );
			}
			...
			if ( empty( $array_api ) ) {
				...
				<div class="wrap">
					<h1>' . esc_html__( 'Installing Theme...', 'splash' ) . '</h1>
					<p>' . esc_html__( 'An error occurred, please check that the item ID is correct.', 'splash' ) . '</p>
					<a href="' . esc_url( 'admin.php?page=' . envato_market()->get_slug() . '&tab=themes' ) . '">' . esc_html__( 'Return to Plugin Installer', 'splash' ) . '</a>
				</div>';
				wp_die( $msg );
			}
			...
			$title = sprintf( esc_html__( 'Installing Theme: %s', 'splash' ), $api->name . ' ' . $api->version );
		...
		public function ajax_upgrade_theme() {
			...
			if ( $theme_data->exists() && $theme_data->get( 'Version' ) ) {
				$status['oldVersion'] = sprintf( esc_html__( 'Version %s', 'splash' ), $theme_data->get( 'Version' ) );
			}

			if ( ! current_user_can( 'update_themes' ) ) {
				$status['error'] = esc_html__( 'You do not have sufficient permissions to update themes for this site.', 'splash' );
			...
			if ( is_array( $result ) && ! empty( $result[ $theme ] ) ) {
				...
				if ( $theme_data->exists() && $theme_data->get( 'Version' ) ) {
					$status['newVersion'] = sprintf( esc_html__( 'Version %s', 'splash' ), $theme_data->get( 'Version' ) );
				}
				...
				$status['error'] = esc_html__( 'Unable to connect to the filesystem. Please confirm your credentials.', 'splash' );
		...
		public function ajax_add_item() {
			if ( ! check_ajax_referer( self::AJAX_ACTION, 'nonce', false ) ) {
				...
				wp_send_json_error( array( 'message' => esc_html__( 'The Token is missing.', 'splash' ) ) );
			} elseif ( empty( $_POST['id'] ) ) {
				wp_send_json_error( array( 'message' => esc_html__( 'The Item ID is missing.', 'splash' ) ) );
			}
			...
			if ( false === $request ) {
				wp_send_json_error( array( 'message' => esc_html__( 'The Token or Item ID is incorrect.', 'splash' ) ) );
			}

			if ( false === envato_market()->api()->download( $_POST['id'], $args ) ) {
				wp_send_json_error( array( 'message' => esc_html__( 'The item cannot be downloaded.', 'splash' ) ) );
			}
			...
			if ( isset( $type ) ) {
				...
				if ( 'theme' === $type ) {
					...
					$request['install'] = wp_nonce_url( $install_link, 'install-theme_' . sanitize_text_field($_POST['id']) );
				}
				...
				if ( 'plugin' === $type ) {
					...
					$request['install'] = wp_nonce_url( $install_link, 'install-plugin_' . sanitize_text_field($_POST['id']) );
				}
			...
			wp_send_json_error( array( 'message' => esc_html__( 'An unknown error occurred.', 'splash' ) ) );
		}

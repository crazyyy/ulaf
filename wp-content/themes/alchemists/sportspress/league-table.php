...
if ( $title ) {
	$output .= '<header class="card__header">';
		...
		if ( $show_full_table_link ) {
			$output .= '<a href="' . get_permalink( $id ) . '" class="' . esc_attr( implode( ' ', $card_header_btn_classes ) ) . '">' . esc_html__( 'View full table', 'alchemists' ) . '</a>';
		}
	$output .= '</header>';
}
...
$output .= '<table class="table table-hover table-standings sp-league-table sp-league-table-' . ( $id ) . ' sp-data-table' . ( $sortable ? ' sp-sortable-table' : '' ) . ( $responsive ? ' sp-responsive-table ' . $identifier : '' ). ( $scrollable ? ' sp-scrollable-table' : '' ) . ( $paginated ? ' sp-paginated-table' : '' ) . '" data-sp-rows="' . $rows . '">' . '<thead>' . '<tr>';
...
$output .= '<th class="data-rank">' . esc_attr__( 'Pos', 'alchemists' ) . '</th>';

	...
	if ( $title ) {
		...
		$output .= '<header class="card__header">';
			...
			if ( $show_all_players_link ):
				$output .= '<a href="' . get_permalink( $id ) . '" class="' . esc_attr( implode( ' ', $card_header_btn_classes ) ) . '">' . esc_html__( 'View all players', 'alchemists' ) . '</a>';
			endif;
...
foreach ( $groups as $group ) :
	...
	if ( ! is_array( $labels ) || array_key_exists( 'number', $labels ) ) :
		if ( in_array( $orderby, array( 'number', 'name' ) ) ) :
			...
			$thead .= '<th class="data-rank">' . esc_html__( 'Rank', 'alchemists' ) . '</th>';
		endif;
	endif;
	...
	foreach ( $data as $player_id => $row ) :
		...
		if ( $show_player_photo ):
			if ( has_post_thumbnail( $player_id ) ):
				$logo = get_the_post_thumbnail( $player_id, 'alchemists_player-xxs' );
			else :
				$logo = '<img src="' . get_theme_file_uri( '/assets/images/player-placeholder-40x40.jpg' ) . '" srcset="' . get_theme_file_uri( '/assets/images/player-placeholder-40x40@2x.jpg' ) . ' 2x" alt="">';
			endif;

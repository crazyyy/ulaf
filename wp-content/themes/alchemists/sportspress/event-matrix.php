	...
	<div class="card__content">
		...
		<div class="sp-table-wrapper">
			<table class="sp-event-matrix sp-data-table<?php if ( $scrollable ) { ?> sp-scrollable-table<?php } ?>">
				<thead>
					<th class="sp-event-matrix-home-label"><?php esc_html_e( 'Home', 'alchemists' ); ?> \ <?php esc_html_e( 'Away', 'alchemists' ); ?></th>
					<?php foreach ( $rows as $team_id => $row ) { ?>
						<th class="sp-event-matrix-label">
							...
							if ( $link_teams ) {
								echo '<a href="' . get_post_permalink( $team_id ) . '" title="' . esc_attr( $team_names[ $team_id ] ) . '">' . esc_html( $name ) . '</a>';
							} else {
								echo '<span title="' . esc_attr( $team_names[ $team_id ] ) . '">' . esc_html( $name ) . '</span>';
							}
							?>
						</th>
					<?php } ?>
								...
								<?php } else { ?>
									<td class="sp-event-matrix-cell">
										...
										foreach ( $events as $event ) {
											...
											if ( $link_events ) {
												if ( $outcome ) {
													$result = '<a class="alc-event-matrix-event alc-event-matrix-event-' . $outcome . ' btn btn-xs btn-primary btn-block" href="' . get_post_permalink( $event, false, true ) . '">' . $result . '</a>';
												} else {
													$result = '<a class="alc-event-matrix-future-event btn btn-xs btn-link btn-block" href="' . get_post_permalink( $event, false, true ) . '">' . $result . '</a>';
												}
											} else {
												$result = '<span class="alc-event-matrix-event alc-event-matrix-event-' . $outcome . ' btn btn-xs btn-default btn-outline btn-block">' . $result . '</span>';
											}
		...
		if ( $id && $show_all_events_link )
			echo '<div class="sp-calendar-link sp-view-all-link"><a class="btn btn-sm btn-default" href="' . get_permalink( $id ) . '">' . esc_html__( 'View all events', 'alchemists' ) . '</a></div>';
		?>
	</div>
	</div>
</div>

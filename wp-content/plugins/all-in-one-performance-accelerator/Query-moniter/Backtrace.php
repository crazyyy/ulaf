<?php
$utiltrace = ABSPATH."wp-content/plugins/all-in-one-performance-accelerator/Query-moniter/Util.php";
require_once $utiltrace;

if ( ! class_exists( 'Backtrace' ) ) {
class Backtrace {
	...
	public function get_component() {
		...
		foreach ( \Util::get_file_dirs() as $type => $dir ) {
	...
	public static function get_frame_component( array $frame ) {
			try {
				...
				return \Util::get_file_component( $file );
	...
	public function get_filtered_trace() {
		...
		if ( ! isset( $this->filtered_trace ) ) {
			...
			if ( empty( $trace ) && ! empty( $this->trace ) ) {
				$lowest                 = $this->trace[0];
				$file                   = \Util::standard_dir( $lowest['file'], '' );
	...
	public function filter_trace( array $frame ) {
		...
		if ( isset( $frame['class'] ) ) {
			if ( isset( self::$ignore_class[ $frame['class'] ] ) ) {
				...
				$return['display'] = \Util::shorten_fqn( $frame['class'] . $frame['type'] . $frame['function'] ) . '()';
			}
		} else {
			if ( isset( self::$ignore_func[ $frame['function'] ] ) ) {
				...
				if ( 'dir' === $show ) {
					if ( isset( $frame['args'][0] ) ) {
						$arg = \Util::standard_dir( $frame['args'][0], '' );
						$return['id']      = $frame['function'] . '()';
						$return['display'] = \Util::shorten_fqn( $frame['function'] ) . "('{$arg}')";
					}
				} else {
					$args = array();
					for ( $i = 0; $i < $show; $i++ ) {
						if ( isset( $frame['args'][ $i ] ) ) {
							if ( is_string( $frame['args'][ $i ] ) ) {
								...
								$args[] = \Util::display_variable( $frame['args'][ $i ] );
							}
						}
					}
					$return['id']      = $frame['function'] . '()';
					$return['display'] = \Util::shorten_fqn( $frame['function'] ) . '(' . implode( ',', $args ) . ')';
				}
			} else {
				$return['id']      = $frame['function'] . '()';
				$return['display'] = \Util::shorten_fqn( $frame['function'] ) . '()';
			}
		}

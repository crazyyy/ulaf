...
class enhancerCache
{
    ...
    {
        $_min_ext = '';
        if ( apply_filters( 'smack_optimize_filter_cache_url_add_min_ext', false ) ) {
            $_min_ext = '.min';
        }
   
        $this->cachedir = SMACK_OPTIMIZE_DIR;
       // $this->nogzip   = AUTOPTIMIZE_CACHE_NOGZIP;
        ...
        // } else {
            if ( in_array( $ext, array( 'js', 'css' ) ) ) {
                $this->filename = $ext . '/' . 'enhancer_' . $md5 . $_min_ext . '.' . $ext;
            } else {
                $this->filename = 'enhancer_' . $md5 . $_min_ext . '.' . $ext;
            }
        //}
    }
    ...
    public function cache( $data, $mime )
    {
        ...
        //if ( apply_filters( 'smack_optimize_filter_cache_checkdirs_on_write', false ) ) {
       ...
       // }
        ...
        if ( false === $this->nogzip ) {
            ...
            $file    = 'default.php';
            $phpcode = file_get_contents( SMACK_OPTIMIZE_PLUGIN_DIR . 'config/' . $file );
            ...
            // if ( self::do_fallback() && strpos( $this->filename, '_snippet_' ) === false && strpos( $this->filename, '_single_' ) === false ) {
                if ( strpos( $this->filename, '_snippet_' ) === false && strpos( $this->filename, '_single_' ) === false ) {
                $_extension     = pathinfo( $this->filename, PATHINFO_EXTENSION );
                $_fallback_file = 'enhancer_' . 'fallback.' . $_extension;
            ...
            if ( apply_filters( 'smack_optimize_filter_cache_create_static_gzip', false ) ) {
        ...
        do_action( 'smack_optimize_action_cache_file_created', $this->cachedir . $this->filename );
    }
    ...
    public function getname()
    {
        ...
        apply_filters( 'smack_optimize_filter_cache_getname', SMACK_OPTIMIZE_CACHE_URL . $this->filename );
    ...
    /**
     * Clears contents of SMACK_OPTIMIZE_DIR.
     *
     * @return void
     */
    protected static function clear_cache_classic()
    {
        ...
        foreach ( $contents as $name => $files ) {
            $dir = rtrim( SMACK_OPTIMIZE_DIR . $name, '/' ) . '/';
        ...
        @unlink( SMACK_OPTIMIZE_DIR . '/.htaccess' ); // @codingStandardsIgnoreLine
    }
    ...
    /**
     * Clears contents of SMACK_OPTIMIZE_DIR by renaming the current
    ...
    {
        return apply_filters( 'smack_optimize_filter_cache_clear_advanced', false );
    }
    ...
    {
        $pathname = WP_CONTENT_DIR . '/cache/smack-optimize/';
        return $pathname;
    }
    ...
    public static function clearall( $propagate = true )
    {
        ...
        delete_transient( 'smack_optimize_stats' );

        // Cache was just purged, clear page cache and allow others to hook into our purging...
        if ( true === $propagate ) {
            if ( ! function_exists( 'smack_optimize_do_cachepurged_action' ) ) {
                function smack_optimize_do_cachepurged_action() {
                    do_action( 'smack_optimize_action_cachepurged' );
                }
            }
            add_action( 'shutdown', 'smack_optimize_do_cachepurged_action', 11 );
            add_action( 'smack_optimize_action_cachepurged', array( 'enhancerCache', 'flushPageCache' ), 10, 0 );
        }

        // Warm cache (part of speedupper)!
        if ( apply_filters( 'smack_optimize_filter_speedupper', true ) && false == get_transient( 'smack_optimize_cache_warmer_protector' ) ) {
            set_transient( 'smack_optimize_cache_warmer_protector', 'I shall not warm cache for another 10 minutes.', 60 * 10 );
            $url   = site_url() . '/?ao_speedup_cachebuster=' . rand( 1, 100000 );
            $url   = apply_filters( 'smack_optimize_filter_cache_warmer_url', $url );
    ...
    protected static function get_cache_contents()
    {
        ...
        foreach ( array( '', 'js', 'css' ) as $dir ) {
            $contents[ $dir ] = scandir( SMACK_OPTIMIZE_DIR . $dir );
        }
    ...
    {
        $stats = get_transient( 'enhancer_stats' );
        ...
        if ( ! is_array( $stats ) ) {
            ...
            if ( $count > 100 ) {
                ...
                set_transient(
                    'enhancer_stats',
                    $stats,
                    apply_filters( 'smack_optimize_filter_cache_statsexpiry', HOUR_IN_SECONDS )
                );
            }
        }
        return $stats;
    ...
    {
        if ( false === enhancerCache::check_and_create_dirs() ) {
            return false;
        }

        // Using .htaccess inside our cache folder to overrule wp-super-cache.
        $htaccess = SMACK_OPTIMIZE_DIR . '/.htaccess';
        if ( ! is_file( $htaccess ) ) {
            /**
...
<IfModule !mod_authz_core.c>
    ...
    public static function check_fallback_php() {
        ...
        if ( ! file_exists( $_fallback_php ) ) {
            $_fallback_php_contents = file_get_contents( SMACK_OPTIMIZE_PLUGIN_DIR . 'config/' . $_fallback_filename );
            $_fallback_php_contents = str_replace( '<?php exit;', '<?php', $_fallback_php_contents );
            $_fallback_php_contents = str_replace( '<!--ao-cache-dir-->', SMACK_OPTIMIZE_DIR, $_fallback_php_contents );
            if ( apply_filters( 'smack_optimize_filter_cache_fallback_log_errors', false ) ) {
    ...
    //         $_do_fallback = (bool) apply_filters( 'smack_optimize_filter_cache_do_fallback', autoptimizeOptionWrapper::get_option( 'autoptimize_cache_fallback', '' ) );
    //     }
    ...
    public static function wordpress_notfound_fallback() {
        ...
        $original_request = strtok( $server_array['REQUEST_URI'], '?' );
        if ( strpos( $original_request, wp_basename( WP_CONTENT_DIR ) . '/cache/smack-optimize' ) !== false && is_404() ) {
            ...
            $fallback_path = SMACK_OPTIMIZE_DIR . $js_or_css . '/autoptimize_fallback.' . $js_or_css;
    ...
    public static function check_and_create_dirs() {
        ...
        foreach ( array( '', 'js', 'css' ) as $dir ) {
            if ( ! self::check_cache_dir( SMACK_OPTIMIZE_DIR . $dir ) ) {
                return false;
            }
        }
        return true;
    ...
    public static function flushPageCache()
    {
        ...
        if ( function_exists( 'wp_cache_clear_cache' ) ) {
            ...
            if ( apply_filters( 'smack_optimize_flush_wpengine_aggressive', false ) ) {
            ...
            $wpe_methods = apply_filters( 'smack_optimize_flush_wpengine_methods', $wpe_methods );
        ...
        } elseif ( array_key_exists( 'KINSTA_CACHE_ZONE', $server_array ) ) {

	...
	<form method="post" action="<?php echo esc_attr($formActionUrl); ?>">
		...
		<table class="form-table">
			...
			<tr>
				...
				<td>
					<label>
						<input type="checkbox" name="node_detection_enabled" value="1"
							<?php checked($settings['node_detection_enabled']); ?>>
						Detect and record new toolbar items while browsing the site.
					</label>
					<p class="description">
						Some toolbar items only appear in the front-end or on specific pages, so
						the Toolbar Editor doesn't know about them by default. Enable this option
						to automatically detect new items and add them to your custom toolbar configuration.
						For performance, detection is only done when you're logged in as an admin.
					</p>
				</td>
			</tr>
			</tbody>

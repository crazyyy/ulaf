...
$mgwpp_gallery_id = isset($_GET['gallery_id']) ? absint($_GET['gallery_id']) : 0;

// Properly handle nonce
$mgwpp_nonce = isset($_GET['_wpnonce']) ? sanitize_key(wp_unslash($_GET['_wpnonce'])) : '';

if (!$mgwpp_gallery_id ||
    !wp_verify_nonce($mgwpp_nonce, 'mgwpp_preview')
) {
    status_header(403);
...
<head>
    ...
    <title><?php printf(esc_html__('Preview: %s', 'mini-gallery'), esc_html(get_the_title($mgwpp_gallery_id))); ?></title>

...
class CodeProfilerPro_Table_Connections extends WP_List_Table {
	...
	function __construct( $section, $id, $profile_path ) {
		...
		if (
			empty( $cp_options['truncate_name'] ) ||
		...
		if (
			empty( $cp_options['truncate_queries'] )
			|| ! preg_match('/^\d+$/', ( $cp_options['truncate_queries'] ) ) )
		{
		...
		if (
			empty( $cp_options['table_max_rows'] ) ||
		...
		parent::__construct( array(
			'singular'  => esc_html__('connection', 'code-profiler-pro'),
			'plural'    => esc_html__('connections', 'code-profiler-pro'),
			'ajax'      => false
		));
	}
	...
	function get_columns(){
		return [
			'url'		=> esc_html__('URL', 'code-profiler-pro'),
			'time'	=> esc_html__('Time (sec)', 'code-profiler-pro'),
			'code'	=> esc_html__('Code', 'code-profiler-pro'),
	...
	function single_row( $a_comment ) {
		...
		$this->echo_url_cell(  $a_comment, $columns['url'] );
		$this->echo_time_cell( $a_comment, $columns['time'] );
		$this->echo_code_cell( $a_comment, $columns['code'] );
		...
		$backtrace_buffer = json_decode( $a_comment['backtrace'], true );

		// Sorting order
		echo '<ol reversed>';

		foreach( $backtrace_buffer as $k => $v ) {

			// Absolute or relative paths
			if ( $this->show_paths == 'relative' ) {
				$file = ltrim( str_replace( $this->abspath, '', $v['file'] ), '\\/');
			} else {
				$file = $v['file'];
			}
			// Function/method
			...
			if (! file_exists( $v['file'] ) ) {
				...
				echo "<li>". esc_html( $v['file'] ) ."</li>";
			} else {
				printf(
					'<li><a style="cursor:pointer" onClick=\'cpjspro_file_view("%1$s","%2$s","",%3$d);\' title="%4$s">%5$s@%3$d</a>%6$s</li>',
					base64_encode( $v['file'] ),
					esc_attr( $this->nonce ),
					(int) $v['line'],
					esc_attr__('Click to view', 'code-profiler-pro'),
					esc_html( $file ),
					$function
				);
			}
	 ...
	 */
	function echo_code_cell( $a_comment, $name ) {

		// Connection error
		if ( $a_comment['code'] == 'ERROR' ) {
			$a_comment['code'] = 'HTTP Error';
			$color = 'red';

		// 4xx and 5xx error
		} elseif ( preg_match('/^[45]/', $a_comment['code'] ) ) {
			$a_comment['code'] = (int) $a_comment['code'];
			$color = 'red';

		// 3xx error
		} elseif ( preg_match('/^3/', $a_comment['code'] ) ) {
			$a_comment['code'] = (int) $a_comment['code'];
			$color = 'orange';
		...
		printf(
			'<td class="retcode column-retcode" data-colname="%1$s" title="%2$s" style="cursor:help"><font color="%3$s">%4$s</font></td>',
			$name,
			esc_attr( $a_comment['msg'] ),
			$color,
			esc_html( $a_comment['code'] )
		);
	}
	 ...
	 * Create the "URL" cell
	 */
	function echo_url_cell( $a_comment, $name ) {

		if ( strlen( $a_comment['url'] ) > $this->truncate_queries ) {
			$conn = mb_substr( $a_comment['url'], 0, $this->truncate_queries, 'utf-8') .' [...]';
		} else {
			$conn = $a_comment['url'];
		}

		$actions['view'] =
				'<a style="cursor:pointer" onClick="cpjspro_toggle_caller(\''.
				$this->row_count .'\', 4)">'.
				esc_html__('View backtrace', 'code-profiler-pro') .'</a>';

		printf(
			'<td class="url column-url column-primary" data-colname="%3$s">%1$s %2$s</td>',
			// Beware of invalid code unit sequences
			htmlspecialchars( $conn, ENT_SUBSTITUTE ),
	...
	function echo_time_cell( $a_comment, $name ) {
		...
		echo "<td class='time column-time' data-colname='$name'>".
				"<div class='cp-list-pc' style='cursor:help' title='".
					sprintf(
						esc_attr__('%s%% of all HTTP requests', 'code-profiler-pro'),
					$pc
				).	"'><div class='cp-list-pc-bar' style='width:{$pc}%'></div><center>". esc_html( $time ) .
				"</center></div></td>";
	...
	function echo_name_cell( $a_comment, $name ) {
		// Core or unknown script
		if ( empty( $a_comment['name'] ) ) {
			$a_comment['name'] = '-';
		}
	...
	function prepare_items() {
		...
		// Fetch our data
		$profile = $this->fetch_connections();
	...
	function fetch_connections() {
		...
		if (! file_exists("{$this->profile_path}.connections.profile") ) {
			return $buffer;
		}
		$lines = file_get_contents( "{$this->profile_path}.connections.profile", FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES );
		if ( $lines === false ) {
			wp_die( sprintf(
				esc_html__('Cannot open profile file: %s', 'code-profiler-pro'),
				"{$this->profile_path}.connections.profile"
			));
		}
		...
		$connections = json_decode( $lines, true );
		...
		foreach( $connections as $index => $connection ) {
			...
			if (! empty( $_REQUEST['s'] ) ) {
				if ( $search(
					$connection['url'] . $connection['msg'] . $connection['code'] .
					$connection['slug'] . $connection['name'],
					$_REQUEST['s'] ) === false
				) {
					// We must get the total time for all records, not only the filtered ones
					$this->total_time += $connection['time'];
					continue;
				}
			}
			$this->total_time          	+= $connection['time'];
			$buffer[ $index ]['time']		= $connection['time'];
			$buffer[ $index ]['url']		= $connection['url'];
			$buffer[ $index ]['code']		= $connection['code'];
			$buffer[ $index ]['msg']		= $connection['msg'];

			// Display full name or slug
			if ( $this->display_name == 'full' ) {
				$buffer[ $index ]['name']	= $connection['name'];
			} else {
				$buffer[ $index ]['name']	= $connection['slug'];
			}
			// Truncate name if needed
			if ( strlen( $buffer[ $index ]['name'] ) > $this->truncate_name ) {
				$buffer[ $index ]['name'] = mb_substr( $buffer[ $index ]['name'], 0, $this->truncate_name , 'utf-8') .'...';
			}
			$buffer[ $index ]['slug'] = $connection['slug'];

			$buffer[ $index ]['backtrace'] = json_encode( $connection['backtrace'] );
		}
		return $buffer;
	}

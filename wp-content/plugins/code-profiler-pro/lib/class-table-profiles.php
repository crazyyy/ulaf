...
class CodeProfilerPro_Table_Profiles extends WP_List_Table {
	...
	function __construct() {
		...
		parent::__construct([
			'singular' => esc_html__('profile', 'code-profiler-pro'),
			'plural'   => esc_html__('profiles', 'code-profiler-pro'),
			'ajax'     => false
		]);
    }
	 ...
	 */
	function no_items() {
		esc_html_e('No profile found.', 'code-profiler-pro');
		$this->is_empty = 1;
	}
	...
	function column_default( $item, $column_name ) {
		...
		/* 'time' is namaged by column_time() */
	...
	function get_columns(){
		return [
			...
			'profile'	=> esc_html__('Profile', 'code-profiler-pro'),
			'date'		=> esc_html__('Date', 'code-profiler-pro'),
			'items'		=> esc_html__('Items', 'code-profiler-pro'),
			'time'		=> esc_html__('Time', 'code-profiler-pro'),
			'mem'			=> esc_html__('Memory', 'code-profiler-pro'),
			'io'			=> esc_html__('File I/O', 'code-profiler-pro'),
			'queries'	=> esc_html__('SQL', 'code-profiler-pro'),
			'http'		=> esc_html__('HTTP', 'code-profiler-pro')
		];
    }
	...
	/**
	 * Time column + tooltip warning.
	 */
	function column_time( $item ) {
		if ( isset( $item['backtrace'] ) ) {
			// Display a warning if the backtrace option was enabled
			return $item['time'] . 's <span class="code-profiler-pro-tip code-profiler-pro-tip-info" data-tip="'.
			esc_attr__('The execution time for this profile may have taken longer than expected because the PHP backtrace for methods and functions option was enabled.', 'code-profiler-pro') .'"></span>';
		}
		return $item['time'] .'s';
	}
	...
	function column_profile( $item ) {
		...
		$actions = [
			'view'   => sprintf(
				'<a href="?page=code-profiler-pro&cptab=profiles_list&action=%s&id=%s&section=1">%s</a>',
				'view_profile',
				esc_attr( $item['ID'] ),
				esc_html__('View')
			),
			'edit'   => sprintf(
				'<a style="cursor:pointer" onClick="cpjspro_toggle_name(\'%s\')">%s</a>',
				esc_attr( $this->row_count ),
				esc_html__('Quick Edit')
			),
			'delete' => sprintf(
				'<a href="?page=code-profiler-pro&cptab=profiles_list&action=%s&profiles[]=%s&_wpnonce=%s&orderby=%s&order=%s" '.
				'onclick="return cpjspro_delete_profile();">%s</a>',
				'delete_profiles',
				esc_attr( $item['ID'] ),
		...
		$profile_name = sprintf(
			...
			'<div id="profile_div_%1$s" style="display:none">'.
				...
				'<p>'.
					'<input type="button" class="button-primary button button-small" value="%4$s" onClick="cpjspro_edit_name(\'%3$s\', \'%1$s\', \'%6$s\', \'%1$s\')"/>'.
					'&nbsp;'.
					'<input type="button" class="button-secondary button button-small" value="%5$s" onClick="cpjspro_toggle_name(\'%1$s\')"/>'.
					'&nbsp;'.
	...
	function prepare_items() {
		...
		usort( $profile, [ &$this, 'usort_reorder'] );
	...
	function fetch_profiles() {
		...
		$profiles = [];
		$glob = glob( CODE_PROFILER_PRO_UPLOAD_DIR .'/*.slugs.profile');
			...
			foreach( $glob as $path ) {
				...
				if ( preg_match('`^(\d{10}\.\d+)\.(.+?)\.(?:slugs)\.profile$`', $file, $match ) ) {
					...
					foreach( $this->default_files as $pname ) {
						...
						if ( in_array( $pname, $this->optional_files ) ) { continue; }
						if ( file_exists( CODE_PROFILER_PRO_UPLOAD_DIR ."/{$match[1]}.{$match[2]}.$pname.profile") ) {
							$fsize += filesize( CODE_PROFILER_PRO_UPLOAD_DIR ."/{$match[1]}.{$match[2]}.$pname.profile");
						} else {
							$error = 1;
							break;
						}
					...
					if ( $error ) {
						foreach( $this->default_files as $pname ) {
							if ( file_exists( CODE_PROFILER_PRO_UPLOAD_DIR ."/{$match[1]}.{$match[2]}.$pname.profile") ) {
								unlink( CODE_PROFILER_PRO_UPLOAD_DIR ."/{$match[1]}.{$match[2]}.$pname.profile");
							}
						}
						continue;
					}
					...
					if (! empty( $_REQUEST['s'] ) ) {
						foreach( $this->default_files as $pname ) {
							// Ignore these ones, there aren't mandatory
							if ( in_array( $pname, $this->optional_files ) ) { continue; }
							$search = $this->search_profile_file( CODE_PROFILER_PRO_UPLOAD_DIR ."/{$match[1]}.{$match[2]}.$pname.profile", $_REQUEST['s'] );
							if ( $search === true ) {
								break;
							}
						}
					...
					if ( file_exists( CODE_PROFILER_PRO_UPLOAD_DIR ."/{$match[1]}.{$match[2]}.summary.profile") ) {
						$summary = json_decode(
							file_get_contents( CODE_PROFILER_PRO_UPLOAD_DIR ."/{$match[1]}.{$match[2]}.summary.profile"), true
						);
					}
					...
					if (! empty( $summary['backtrace'] ) ) {
						$profiles[$count]['backtrace'] = 1;
					}

					if (! isset( $summary['connections'] ) ) {
						// CP <1.6
						$profiles[$count]['http'] = '-';
					} elseif (! empty( $summary['connections'] ) ) {
						$profiles[$count]['http'] = $summary['connections'];
					} else{
						$profiles[$count]['http'] = 0;
					}
	 ...
	 */
	 public function delete_profiles( $profiles ) {
		...
		foreach( $profiles as $name ) {
			$profile_name = code_profiler_pro_get_profile_path( $name );

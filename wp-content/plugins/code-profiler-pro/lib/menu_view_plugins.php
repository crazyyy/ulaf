...
$buffer = code_profiler_pro_get_profile_data( $profile_path, 'slugs');
...
if (! empty( $cp_options['warn_composer'] ) ) {
	$composer_warning = code_profiler_pro_composer_warning( $profile_path, $cp_options['display_name'] );
}

$label		= '';
$data			= '';
$theme		= esc_attr__('theme', 'code-profiler-pro');
$total_time	= 0;
$hidden		= 0;
$count		= 0;
// Sort by value
...
<?php
	...
	echo "<table>\n".
		"\t<tr>\n".
			"\t\t<td>".	esc_html__('Component name', 'code-profiler-pro')		."</td>".
			"<td>".		esc_html__('Time in seconds', 'code-profiler-pro')		."</td>".
			"<td>".		esc_html__('Time in percentage', 'code-profiler-pro')	."</td>\n";
...
function code_profiler_pro_composer_warning( $profile_path, $display_name ) {
	...
	$msg = esc_html__('Code Profiler has detected that the following components, sorted by execution order, are using Composer dependency manager:', 'code-profiler-pro') . "<br />$list<br />".
	sprintf(
		esc_html__('As that may increase the execution time of %s, make sure to consult the following FAQ: %s%s%s', 'code-profiler-pro'),
		$first,
		'<a href="?page=code-profiler-pro&cptab=faq#composerwarning" target="_blank" rel="noopener noreferrer">',
		esc_html__('Why does Code Profiler warn me that I have multiple plugins using Composer?', 'code-profiler-pro'),
		'</a>'
	);

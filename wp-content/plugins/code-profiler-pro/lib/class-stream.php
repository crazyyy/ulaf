...
class CodeProfilerPro_Stream {
	...
	public static function start() {
		...
		stream_wrapper_register( self::$protocol, CodeProfilerPro_Stream::class );
	}
	...
	public function stream_read( $count ) {
		...
		if ( $this->script ) {
			...
			if ( ftell( $this->resource ) == 0 ) {
				self::start();
				$read = fread( $this->resource, $count - CODE_PROFILER_PRO_LENGTH );
				self::$io_read += strlen( $read );
				return preg_replace(
					'/<\?php/i',
					'<?php declare(ticks='. CODE_PROFILER_PRO_TICKS .');',
					$read,
					1
				);
			}
	...
	public function stream_stat() {
		...
		if ( $this->script ) {
			$res['size']	+= CODE_PROFILER_PRO_LENGTH;
			$res[7]			+= CODE_PROFILER_PRO_LENGTH;
		}
		return $res;
    }

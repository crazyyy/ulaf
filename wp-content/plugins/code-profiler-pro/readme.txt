=== Code Profiler Pro - WordPress Performance Profiling and Debugging Made Easy ===
Contributors: bruandet, nintechnet
Tags: profile, debug, optimize, performance, seo, benchmark, statistics, debugging, speed, profiling
...
* WP-CLI: Added support for HTTP basic authentication with the `--u` and `--p` parameters. Run `wp code-profiler-pro help` for more details.
* WP-CLI: Added support for WordPress user authentication with the `--user` parameter. Run `wp code-profiler-pro help` for more details.
...
* Added support for WP-CLI: you can run the profiler from a terminal. Enter `wp code-profiler-pro help` to display the available command line options. WP-CLI integration can be turned on/off in the plugin "Settings" page.

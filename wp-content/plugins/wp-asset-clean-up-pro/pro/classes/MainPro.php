...
class MainPro
{
	...
	public static function getTaxonomyValuesAssocToPostType($postType, $assetType = '', $handle = '')
    {
	    ...
	    if ( $assetType && $handle &&
             isset( $existingListAll[ $assetType ]['post_type_via_tax'][ $postType ] [ $handle ] ['values'] ) &&
	...
	public function filterAssetsUnloadList($list, $assetType, $filterType)
    {
        ...
        if ($filterType === 'unload') {
            ...
            foreach (array('taxonomy', 'author') as $bulkType) {
                if ( isset( $bulkUnloads['bulk_unloaded'][$bulkType][ $assetType ] ) && ( ! empty( $bulkUnloads['bulk_unloaded'][$bulkType][ $assetType ] ) ) ) {

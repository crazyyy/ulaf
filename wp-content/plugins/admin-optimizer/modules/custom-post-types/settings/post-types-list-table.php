...
class Post_Types_List_Table extends \WP_List_Table {
	...
	/**
	 * Post types to display in the table
	 *
	...
	public function __construct() {
		$this->post_types = get_option( Post_Types::OPTION_NAME, [] );

		parent::__construct(
			array(
				'plural'   => 'custom_post_types',
				'singular' => 'custom_post_type',
				'ajax'     => true,
			)
		);
	}
	...
	/**
	 * Details for the checkbox column
	 *
		...
		$output            = sprintf(
			...
			$this->generate_link(
				[
					'action' => 'edit',
					'slug'   => $item['slug'],
					'nonce'  => wp_create_nonce( 'edit-post-type_' . $item['slug'] ),
				]
			),
			$name
		);
		...
		$actions['delete'] = sprintf(
			...
			$this->generate_link(
				[
					'action' => 'delete',
					'slug'   => $item['slug'],
					'nonce'  => wp_create_nonce( 'delete-post-type_' . $item['slug'] ),
				]
			)
		);
	 ...
	 *
	 * @param array $query Item to render.
	 *
	 * @return string
	 */
	public function generate_link( array $query ) {
		$default = [
			'page'   => Post_Types::MENU_SLUG,
			'action' => 'add',
			'slug'   => '',
			'nonce'  => wp_create_nonce( 'adminoptimizer-post-types' ),
		];
	 ...
	 *
	 * @param array $post_type Item to be rendered.
	 *
	 * @return void
	 */
	public function single_row( $post_type ) {
		$item['slug']        = $post_type['slug'];
		$item['name']        = $post_type['labels']['name'];
		$item['description'] = $post_type['description'] ?? '';
		?>
		<tr id="poststatus-<?php echo esc_attr( $post_type['slug'] ); ?>">

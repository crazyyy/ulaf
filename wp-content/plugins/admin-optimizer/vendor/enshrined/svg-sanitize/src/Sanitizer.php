<?php
namespace Yipresser\AdminOptimizer\Vendor\enshrined\svgSanitize;

use Yipresser\AdminOptimizer\Vendor\enshrined\svgSanitize\data\AllowedAttributes;
use Yipresser\AdminOptimizer\Vendor\enshrined\svgSanitize\data\AllowedTags;
use Yipresser\AdminOptimizer\Vendor\enshrined\svgSanitize\data\AttributeInterface;
use Yipresser\AdminOptimizer\Vendor\enshrined\svgSanitize\data\TagInterface;
use Yipresser\AdminOptimizer\Vendor\enshrined\svgSanitize\data\XPath;
use Yipresser\AdminOptimizer\Vendor\enshrined\svgSanitize\ElementReference\Resolver;
...
class Sanitizer
{
    ...
    protected function cleanAttributesOnWhitelist(\DOMElement $element)
    {
        for ($x = $element->attributes->length - 1; $x >= 0; $x--) {
            ...
            /**
             * This is used for when a namespace Yipresser\AdminOptimizer\Vendor\isn't imported properly.
             * Such as xlink:href when the xlink namespace Yipresser\AdminOptimizer\Vendor\isn't imported.
    ...
    protected function cleanXlinkHrefs(\DOMElement $element)
    {
        foreach ($element->attributes as $attribute) {
            // remove attributes with unexpected namespace Yipresser\AdminOptimizer\Vendor\prefix, e.g. `XLinK:href` (instead of `xlink:href`)

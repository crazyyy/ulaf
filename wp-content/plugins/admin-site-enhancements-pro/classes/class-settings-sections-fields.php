...
class Settings_Sections_Fields {
	...
	function register_sections_fields() {
		...
		$nonpublic_post_type_names = get_post_types( array( 'public' => false ), 'names' );
        ...
        if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
        	...
        	$gutenberg_not_applicable_types[] = 'asenha_options_page';
        	$gutenberg_not_applicable_types[] = 'options_page_config';
        	$gutenberg_not_applicable_types[] = 'asenha_code_snippet';
        	$gutenberg_not_applicable_types[] = 'formbuilder-styles';
        }

		$rest_api_enabled_post_type_names = get_post_types( array( 'show_in_rest' => true ), 'objects' );
		...
		if ( is_array( $asenha_all_post_types ) ) {
			$inapplicable_post_types = array(
				...
				'asenha_redirect', // ASE Redirect Manager
			);
		...
		if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
			$field_description = __( 'Limit media files visibility so only administrators can see all media files. Other restricted roles will only see media files they uploaded themselves in the media library and in the modal window to add/insert media.', 'admin-site-enhancements' );
			$field_options_wrapper = true;
			$field_options_moreless = true;
		}
		...
		if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
			$field_id = 'media_files_visibility_control_for_roles';
			$field_slug = 'media-files-visibility-control-for-roles';

			$roles_with_upload_files = array();
			$default_roles = array();
			if ( is_array( $roles ) ) {
				foreach ( $roles as $role_slug => $role_label ) {
					...
					$roles_with_upload_files[ $role_label ] = $role_slug;
					$default_roles[] = $role_slug;
				}
			}

			if ( ! empty( $roles_with_upload_files ) ) {
				add_settings_field(
					$field_id,
					__( 'Restrict media files visibility for:', 'admin-site-enhancements' ),
					[ $render_field, 'render_checkboxes_subfield' ],
					ASENHA_SLUG,
					'main-section',
					array(
						'field_id'		=> $field_id,
						'field_name'	=> ASENHA_SLUG_U . '[' . $field_id . '][]',
						'field_options'	=> $roles_with_upload_files,
						'field_default'	=> $default_roles,
						'layout'		=> 'vertical',
						'class'			=> 'asenha-checkboxes margin-top-8 content-management ' . $field_slug,
					)
				);
			...
			$field_id = 'media_files_visibility_control_notes';
			$field_slug = 'media-files-visibility-control-notes';
			...
			add_settings_field(
				...
				array(
					'option_name'			=> ASENHA_SLUG_U,
						'field_description'	=> '<div class="asenha-warning">' . __( 'Only user roles with <code>upload_files</code> capability can access the media library, and they are listed above.', 'admin-site-enhancements' ) . '</div>',
        ...
        if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
			...
			$field_id = 'media_categories_disabled_for_roles';
			$field_slug = 'media-categories-disabled-for-roles';
			...
			if ( is_array( $roles ) ) {
				foreach ( $roles as $role_slug => $role_label ) {
					...
					$has_manage_categories = $role_object->has_cap( 'manage_categories' );

					$label = $role_label;
					if ( $has_manage_categories ) {
						$label .= ' <span class="faded">(' . sprintf(
							/* translators: %s: capability name */
							__( 'has <code>%s</code> capability', 'admin-site-enhancements' ),
							'manage_categories'
						) . ')</span>';
					}

					// Default: disable Media Categories for upload-capable roles that do not have manage_categories.
					if ( ! $has_manage_categories ) {
						$default_roles[] = $role_slug;
					}

					$roles_with_upload_files[ $label ] = $role_slug;
				}
			}
			...
			if ( ! empty( $roles_with_upload_files ) ) {
				add_settings_field(
					$field_id,
					__( 'Disable media categories features for:', 'admin-site-enhancements' ),
					[ $render_field, 'render_checkboxes_subfield' ],
					ASENHA_SLUG,
					'main-section',
					array(
				...
				$field_id = 'media_categories_disabled_for_roles_notes';
				$field_slug = 'media-categories-disabled-for-roles-notes';
				...
				add_settings_field(
					...
					array(
						'option_name'		=> ASENHA_SLUG_U,
						'field_description'	=> '<div class="asenha-warning">' . __( 'Only user roles with <code>upload_files</code> capability can access the media library, and they are listed above.', 'admin-site-enhancements' ) . '</div>',
						'class'				=> 'asenha-description content-management ' . $field_slug,
					)
				);
			}
        }
		...
		if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
			$field_id   = 'enable_external_permalinks_type';
			$field_slug = 'enable-external-permalinks-type';
			...
			add_settings_field(
				...
				array(
					...
					'field_radios'   => array(
						__( 'Enable only on', 'admin-site-enhancements' )           => 'only-on',
						__( 'Enable except on', 'admin-site-enhancements' )         => 'except-on',
						__( 'Enable on all post types', 'admin-site-enhancements' ) => 'all-post-types',
					),
					'field_default'  => 'only-on',
					'class'          => 'asenha-radio-buttons bold-label shift-up asenha-th-border-bottom content-management ' . $field_slug,
				)
			);
		}
		...
		if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
			$field_label .= '<br /><span class="faded asenha-disable-comments-forced-note">' . esc_html__( '(Currently removed by the Disable Comments module)', 'admin-site-enhancements' ) . '</span>';
		}
        ...
        if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
			...
			if ( ! empty( $workable_nodes ) ) {
				...
				foreach( $workable_nodes as $node_id => $node ) {
					...
					// Prefer extracting readable text from the rendered title HTML.
					if ( isset( $node['title'] ) ) {
						$node_title = $common_methods->extract_readable_text_from_html( $node['title'] );
					}

					// Fall back to the tooltip-like meta title if available (often plain text).
					if ( empty( $node_title ) && isset( $node['meta'] ) && is_array( $node['meta'] ) && isset( $node['meta']['title'] ) ) {
						$node_title = $common_methods->extract_readable_text_from_html( $node['meta']['title'] );
					}
					...
					add_settings_field(
						...
						array(
							...
							'field_label'			=> esc_html( $node_title ) . ' <span class="faded">(' . esc_html( $node_id ) . ')</span>',
							'class'					=> 'asenha-checkbox asenha-hide-th admin-interface ' . $field_slug . ' ' . $node_id,
						)
					);
				}

				$field_id   = 'rescan_plugins_admin_bar_items';
				$field_slug = 'rescan-plugins-admin-bar-items';

				$button_html  = '<div class="asenha-admin-bar-rescan-wrap">';
				$button_html .= '<input type="button" id="asenha-rescan-extra-admin-bar-elements" class="button button-secondary asenha-admin-bar-rescan" value="' . esc_attr__( 'Rescan Extra Elements', 'admin-site-enhancements' ) . '" />';
				$button_html .= '<span class="asenha-admin-bar-rescan-status"></span>';
				$button_html .= '</div>';
				...
				add_settings_field(
					...
					array(
						'html'  => $button_html,
						'class' => 'asenha-custom-html admin-interface ' . $field_slug,
					)
				);
				...
				add_settings_field(
					...
					array(
						'option_name'			=> ASENHA_SLUG_U,
						/* translators: 1: URL to "All Pages" 2: URL to "All Posts" */
						'field_description'		=> sprintf( __( 'Please visit at least one admin page or one of your frontend <a href="%1$s">pages</a> or <a href="%2$s">posts</a> while logged-in, to start detecting extra admin bar elements and return here to see them listed here for removal.', 'admin-site-enhancements' ), $all_pages_url, $all_posts_url ),
						'class'					=> 'asenha-description utilities ' . $field_slug,
					)
				);
		...
		if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
			...
			$field_id = 'hide_admin_notices_menu_for_nonadmins';
			$field_slug = 'hide-admin-notices-menu-for-nonadmins';
			...
			add_settings_field(
				...
				array(
					...
					'field_id'				=> $field_id,
					'field_name'			=> ASENHA_SLUG_U . '[' . $field_id . ']',
					'field_label'			=> __( "Hide the 'Notices' menu in the admin bar for non-administrators.", 'admin-site-enhancements' ),
					'class'					=> 'asenha-checkbox asenha-hide-th admin-interface ' . $field_slug,
				)
			);
		...
		if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
			...
			$field_id = 'hide_admin_bar_always_show_for_admins_on_backend';
			$field_slug = 'hide-admin-bar-always-show-for-admins-on-backend';
			...
			add_settings_field(
				...
				array(
					...
					'field_id'				=> $field_id,
					'field_name'			=> ASENHA_SLUG_U . '[' . $field_id . ']',
					'field_label'			=> __( 'Always show the admin bar for administrators on the backend. Useful for when an administrator has multiple user roles, and the admin bar is set to be hidden for the other user role(s).', 'admin-site-enhancements' ),
					'class'					=> 'asenha-checkbox asenha-hide-th asenha-th-border-top admin-interface ' . $field_slug,
				)
			);
		}
		...
		if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
			...
			add_settings_field(
				...
				array(
					...
					'field_hidden_inputs'			=> array(
						array(
							'option_key'	=> 'admin_logo_image_width',
							'field_id'		=> 'admin-logo-image-width',
							'field_name'	=> ASENHA_SLUG_U . '[admin_logo_image_width]',
						),
						array(
							'option_key'	=> 'admin_logo_image_height',
							'field_id'		=> 'admin-logo-image-height',
							'field_name'	=> ASENHA_SLUG_U . '[admin_logo_image_height]',
						),
					),
			...
			$field_id   = 'admin_logo_replace_admin_bar_home_icon';
			$field_slug = 'admin-logo-replace-admin-bar-home-icon';

			$site_icon_link = sprintf(
				'<a href="%1$s">%2$s</a>',
				esc_url( admin_url( 'options-general.php' ) ),
				esc_html__( 'site icon', 'admin-site-enhancements' )
			);

			/* translators: %s: linked text “site icon” pointing to Settings → General. */
			$replace_admin_bar_home_icon_label = sprintf(
				__( 'Replace admin bar home icon with the %s', 'admin-site-enhancements' ),
				$site_icon_link
			);
			...
			add_settings_field(
				...
				array(
					...
					'field_id'		=> $field_id,
					'field_name'	=> ASENHA_SLUG_U . '[' . $field_id . ']',
					'field_label'	=> $replace_admin_bar_home_icon_label,
					'class'			=> 'asenha-checkbox asenha-hide-th admin-interface ' . $field_slug,
				)
			);
		...
		if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
			$field_id = 'custom_login_menu_label';
			$field_slug = 'custom-login-menu-label';

			add_settings_field(
				$field_id,
				__( 'Custom "Log In" Label', 'admin-site-enhancements' ) . ' <span class="faded">(' . __( 'only for dynamic log in/out menu item', 'admin-site-enhancements' ) . ')</span>',
				[ $render_field, 'render_text_subfield' ],
				ASENHA_SLUG,
				'main-section',
				array(
					...
					'field_description'		=> '',
					'field_placeholder'		=> '',
					'class'					=> 'asenha-text margin-top-12 width-75 login-logout ' . $field_slug,
				)
			);

			$field_id = 'custom_logout_menu_label';
			$field_slug = 'custom-logout-menu-label';

			add_settings_field(
				$field_id,
				__( 'Custom "Log Out" Label', 'admin-site-enhancements' ) . ' <span class="faded">(' . __( 'only for dynamic log in/out menu item', 'admin-site-enhancements' ) . ')</span>',
				[ $render_field, 'render_text_subfield' ],
				ASENHA_SLUG,
				'main-section',
				array(
			...
			$field_id = 'custom_login_logout_label_description';
			$field_slug = 'custom-login-logout-label-description';
	    	$field_description = __( 'These custom labels will override the "Navigation Label" set for the menu item.', 'admin-site-enhancements' );
        ...
        if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
			...
			$field_id = 'code_snippets_disable_custom_role';
			$field_slug = 'code-snippets-disable-custom-role';
			...
			add_settings_field(
				...
				array(
					...
					'field_id'		=> $field_id,
					'field_name'	=> ASENHA_SLUG_U . '[' . $field_id . ']',
					'field_label'	=> __( 'Disable the Code Snippets Editor role', 'admin-site-enhancements' ),
					'class'			=> 'asenha-checkbox asenha-hide-th custom-code ' . $field_slug,
				)
			);

			$field_id = 'code_snippets_notes';
			$field_slug = 'code-snippets-notes';
			...
			add_settings_field(
				...
				array(
					'option_name'			=> ASENHA_SLUG_U,
					'field_description'		=> '<div class="asenha-warning">If one of your PHP snippets triggers a pervasive fatal error and you no longer can access your website and/or wp-admin, you can set <code>CSM_SAFE_MODE</code> to <code>true</code> in wp-config.php via FTP to <strong>activate safe mode</strong>, which will stop the execution of all PHP snippets.<br /><br />This will allow you to regain access to wp-admin, deactivate the problematic PHP snippet, see the error stack trace for the fatal error, and fix the error. Afterwards, you can reactivate the snippet and deactivate safe mode by setting <code>CSM_SAFE_MODE</code> to <code>false</code>, to re-enable the execution of PHP snippets.</div>',
					'class'					=> 'asenha-description custom-code ' . $field_slug,
				)
			);
        }
		...
		if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
			...
			add_settings_field(
				...
				array(
					...
					// 'field_description'		=> sprintf(
					// 	'<span class="asenha-disable-comments-all-post-types-note">%s</span>',
					// 	esc_html__( 'This will also remove \'Comments\' in the admin menu, and remove the comments counter/link in the admin bar.', 'admin-site-enhancements' )
					// ),
					'field_description'		=> '',
					'field_default'			=> 'only-on',
					'class'					=> 'asenha-radio-buttons bold-label shift-up asenha-th-border-bottom disable-components ' . $field_slug,
		...
		if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
			$field_id = 'disable_comments_all_cpts_description';
			$field_slug = 'disable-comments-all-cpts-description';
			...
			add_settings_field(
				...
				array(
					'option_name'			=> ASENHA_SLUG_U,
					'field_description'		=> '<div class="asenha-warning">' . __( 'This will also remove \'Comments\' in the admin menu, and remove the comments counter/link in the admin bar.', 'admin-site-enhancements' ) . '</div>',
					'class'					=> 'asenha-description disable-comments ' . $field_slug,
				)
			);			
		}
		...
		add_settings_field(
			...
			array(
				...
				'field_title'			=> $field_title,
				'field_name'			=> ASENHA_SLUG_U . '['. $field_id .']',
				'field_description'		=> __( 'Prevent your site content from being embedded on other sites.', 'admin-site-enhancements' ) . ' ' . sprintf( /* translators: %s is URL of WP Dev Resource page */ __( 'Prevent embedding of non-<a href="%s" target="_blank">whitelisted</a> sites in your site.', 'admin-site-enhancements' ), $oembed_providers_doc ) . ' ' . __( 'Disable all, related JavaScript.', 'admin-site-enhancements' ) . ' ' . __( 'Removes support for the embed block.', 'admin-site-enhancements' ),
				'field_options_wrapper'	=> true,
				'field_options_moreless'	=> true,
				'class'					=> 'asenha-toggle disable-components ' . $field_slug,
			)
        ...
        if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
        	...
        	$field_description .= '</li>
        			<li><strong>[obfuscate email="info@orange.com" link="mailto"]</strong> '; 
        	$field_description .= __( 'to show or return the plain \'mailto:obfuscated@email.address\' string, e.g. as the URL for a "Contact Us" button, widget, element or component that accepts a shortcode for the URL field.' , 'admin-site-enhancements' ) ;
        	$field_description .= '</li>
        		</ul>';
        } else {
        ...
        if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
			$field_id = 'image_upload_control_only_media_library_uploads';
			$field_slug = 'image-upload-control-only-media-library-uploads';
			...
			add_settings_field(
				...
				array(
					...
					'field_id'				=> $field_id,
					'field_name'			=> ASENHA_SLUG_U . '[' . $field_id . ']',
					'field_label'			=> __( 'Only apply resize and conversion to media library and block editor uploads when creating or editing a page or a post.', 'admin-site-enhancements' ),
					'class'					=> 'asenha-checkbox asenha-hide-th optimizations ' . $field_slug,
				)
			);
		...
		if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
			...
			// File Manager
			$field_id = 'file_manager';
			$field_slug = 'file-manager';
			$field_title = __( 'File Manager', 'admin-site-enhancements' );
			$module_description = __( 'A comprehensive file manager for WordPress with folder tree navigation, file and folder operations, and code editing capabilities.', 'admin-site-enhancements' )  . ' ' . sprintf( 
							/* translators: %s is URL to the Redirect Manager settings page */
							__( 'Once enabled, you can find the <a href="%s">File Manager</a> item under the Tools menu.', 'admin-site-enhancements' ), 
							admin_url( 'admin.php?page=asenha-file-manager' )					
						);
			...
			$field_id = 'file_manager_notes';
			$field_slug = 'file-manager-notes';
			...
			add_settings_field(
				...
				array(
					'option_name'			=> ASENHA_SLUG_U,
					'field_description'		=> '<div class="asenha-warning">When <code>FM_READ_ONLY</code> or <code>DISALLOW_FILE_MODS</code> is set to <code>true</code> in wp-config.php, <strong>read-only mode</strong> is activated across all files and folders and no modifications are allowed.<br /><br />When <code>DISALLOW_FILE_EDIT</code> is set to <code>true</code>, read-only mode is activated in the /plugins, /mu-plugins and /themes folders.</div>',
					'class'					=> 'asenha-description utilities ' . $field_slug,
				)
			);
        ...
        if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
			$field_id = 'redirect_manager';
			$field_slug = 'redirect-manager';
			$field_title = __( 'Redirect Manager', 'admin-site-enhancements' );
			$module_description = __( 'Easily manage various types of redirects with support for wildcards and regular expressions (regex).', 'admin-site-enhancements' )  . ' ' . sprintf( 
							/* translators: %s is URL to the Redirect Manager settings page */
							__( 'Once enabled, you can find the <a href="%s">Redirect Manager</a> item under the Tools menu.', 'admin-site-enhancements' ), 
							admin_url( 'edit.php?post_type=asenha_redirect' )					
						);

...
// File: /Applications/MAMP/htdocs/wordpress/wp-content/plugins/gpt3-ai-content-generator/lib/vector-stores/file-upload/qdrant/fn-upload-file-and-upsert.php
// Status: MODIFIED

namespace WPAICG\Lib\VectorStores\FileUpload\Qdrant;
...
/**
 * Handles the logic for ajax_upload_file_and_upsert_to_qdrant.
 ...
 * @param array $qdrant_config
 * @param AIPKit_Vector_Store_Qdrant_Ajax_Handler $handler_instance The AJAX handler instance for logging.
 * @return array|WP_Error An array including data for logging by the caller, or WP_Error.
 */
function _aipkit_qdrant_ajax_upload_file_and_upsert_logic(
    AIPKit_Vector_Store_Manager $vector_store_manager,
    AIPKit_AI_Caller $ai_caller,
    array $qdrant_config,
    AIPKit_Vector_Store_Qdrant_Ajax_Handler $handler_instance
): array|\WP_Error {
    // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce is checked by the calling handler method.
    $target_collection_name = isset($_POST['target_store_id']) ? sanitize_text_field(wp_unslash($_POST['target_store_id'])) : '';
    // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce is checked by the calling handler method.
    ...
    if (empty($target_collection_name)) {
        return new WP_Error('missing_target_collection_qdrant_lib', __('Target Qdrant collection name is required.', 'gpt3-ai-content-generator'), ['status' => 400]);
    }
    // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce is checked by the calling handler method.
    if (!isset($_FILES['file_to_upload'])) {
        return new WP_Error('no_file_qdrant_upload_lib', __('No file provided for Qdrant upload.', 'gpt3-ai-content-generator'), ['status' => 400]);
    }
    if (empty($embedding_provider_key) || empty($embedding_model)) {
        return new WP_Error('missing_embedding_config_qdrant_lib', __('Embedding provider and model are required for Qdrant.', 'gpt3-ai-content-generator'), ['status' => 400]);
    }
    ...
    $allowed_mime_types_for_qdrant = \WPAICG\Includes\AIPKit_Upload_Utils::get_vector_upload_allowed_mime_types();
    $validation_error = \WPAICG\Includes\AIPKit_Upload_Utils::validate_upload_file(
        $file_data,
        $allowed_mime_types_for_qdrant, // Pass allowed types
        $upload_limits['limit_bytes']
    );
    if (is_wp_error($validation_error)) {
        $validation_error->add_data([
            'where' => 'qdrant_upload:validation',
    ...
    if (is_wp_error($extracted_text_or_error)) {
        return new WP_Error('file_extraction_failed_qdrant_lib', 'File extraction failed: ' . $extracted_text_or_error->get_error_message(), [
            'status' => 500,
            'log_data' => [
                'vector_store_id' => $target_collection_name, 'vector_store_name' => $target_collection_name,
    ...
    if ($file_content_for_embedding === false || empty(trim($file_content_for_embedding))) {
        return new WP_Error('file_read_error_qdrant_lib', __('Could not read file content or file is empty.', 'gpt3-ai-content-generator'), ['status' => 500, 'log_data' => [
            'vector_store_id' => $target_collection_name, 'vector_store_name' => $target_collection_name,
            'status' => 'content_error', 'message' => 'Could not read file content or file is empty for Qdrant.',
            'embedding_provider' => $embedding_provider_key, 'embedding_model' => $embedding_model,
            'indexed_content' => sanitize_file_name($file_data['name']),
            'post_title' => sanitize_file_name($file_data['name']),
            'source_type_for_log' => 'file_upload_global_form'
    ...
    if (!isset($provider_map[$embedding_provider_key])) {
        return new WP_Error(
            'invalid_embedding_provider_qdrant_lib',
            __('Invalid embedding provider for Qdrant upload.', 'gpt3-ai-content-generator'),
            [
                'status' => 400,
                'details' => [
    ...
    $embedding_provider_norm = $provider_map[$embedding_provider_key];
    ...
    foreach ($chunks as $c) {
        ...
        if (is_wp_error($embedding_result) || empty($embedding_result['embeddings'][0])) {
            ...
            return new WP_Error('embedding_failed_qdrant_file_lib', __('Failed to generate vector for file content.', 'gpt3-ai-content-generator'), ['status' => 500, 'log_data' => [
                'vector_store_id' => $target_collection_name, 'vector_store_name' => $target_collection_name,
    ...
    $upsert_result = $vector_store_manager->upsert_vectors('Qdrant', $target_collection_name, ['points' => $points_to_upsert], $qdrant_config);

    if (is_wp_error($upsert_result)) {
        return new WP_Error('upsert_failed_qdrant_lib', 'Upsert to Qdrant failed: ' . $upsert_result->get_error_message(), ['status' => 500, 'log_data' => [
            'vector_store_id' => $target_collection_name, 'vector_store_name' => $target_collection_name,
            ...
            'post_title' => sanitize_file_name($file_data['name']),
            'source_type_for_log' => 'file_upload_global_form'
        ], 'details' => [ 'stage' => 'upsert', 'chunk_count' => count($points_to_upsert) ]]);
    }
    ...
    if ($handler_instance && method_exists($handler_instance, '_log_vector_data_source_entry')) {
        ...
        $batch_id = 'qdrant_file_' . md5(sanitize_file_name($file_data['name']) . '|' . (string) wp_rand()) . '_' . time();
        foreach ($chunk_records as $rec) {
            ...
            $handler_instance->_log_vector_data_source_entry([
                'vector_store_id' => $target_collection_name,
                'vector_store_name' => $target_collection_name,
                'status' => 'indexed',
    ...
    return [
        'message' => __('File content embedded and upserted to Qdrant successfully.', 'gpt3-ai-content-generator'),

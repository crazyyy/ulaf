...
use WPAICG\Lib\Chat\Frontend\Ajax\Handlers\ChatFileUploadClaudeHandler;
...
class ChatFileUploadHandler
{
    ...
    public function handle_upload(array $post_data, array $files_data): array|WP_Error
    {
        if (!class_exists(aipkit_dashboard::class) || !aipkit_dashboard::is_pro_plan()) {
            return new WP_Error('pro_feature_required', __('File upload for chat requires a Pro plan.', 'gpt3-ai-content-generator'), ['status' => 403]);
        }
        ...
        switch ($provider_slug) {
            ...
            case 'qdrant':
                ...
                break;
            case 'claude_files':
                if (!class_exists(ChatFileUploadClaudeHandler::class)) {
                    return new WP_Error('handler_class_missing', __('Claude file processing component is missing.', 'gpt3-ai-content-generator'), ['status' => 500]);
                }
                $provider_handler = new ChatFileUploadClaudeHandler($this->config_loader, $this->logger);
                $provider_config = $provider_handler->get_provider_api_config();
                break;
            default:

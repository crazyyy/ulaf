...
class ChatFileUploadQdrantHandler
{
    ...
    public function process_upload(array $file_data, ?int $user_id, ?string $session_id, array $qdrant_config, int $bot_id): array|WP_Error
    {
        ...
        $embedding_provider_lookup = strtolower((string) $embedding_provider_key);
        if (!isset($provider_map[$embedding_provider_lookup])) {
            $this->logger->log_event(array_merge($log_data_base, ['status' => 'failed', 'message' => 'Invalid embedding provider for Qdrant chat upload.']));
            return new WP_Error('invalid_embedding_provider_qdrant_chat_process', __('Invalid embedding provider for Qdrant chat upload.', 'gpt3-ai-content-generator'), ['status' => 400]);
        }
        $embedding_provider_norm = $provider_map[$embedding_provider_lookup];

...
class AIPKit_Action_Call_Webhook {
    ...
    public function execute(array $payload, array $context_data, int $bot_id): array|WP_Error {
        ...
        if (!isset($processed_headers[$content_type_key]) && in_array($http_method, ['POST', 'PUT'], true)) {
             $processed_headers[$content_type_key] = 'application/json; charset=utf-8';
        }
        ...
        if (in_array($http_method, ['POST', 'PUT']) && !empty($body_template)) {
            ...
            if (isset($processed_headers[$content_type_key]) && stripos($processed_headers[$content_type_key], 'application/json') !== false) {

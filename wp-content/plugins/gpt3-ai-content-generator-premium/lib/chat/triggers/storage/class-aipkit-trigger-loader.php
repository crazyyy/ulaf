...
class AIPKit_Trigger_Loader {
    ...
    public function get_triggers(int $bot_id): array {
        ...
        if ($json_error !== JSON_ERROR_NONE || !is_array($triggers_array)) {

            // Attempt a targeted repair for unescaped quotes in body_template (common legacy issue).
            ...
            if (!empty($repair_count)) {
                $triggers_array = json_decode($repaired, true);
                if (json_last_error() === JSON_ERROR_NONE && is_array($triggers_array)) {
                    // Persist repaired JSON so runtime and UI stay in sync.
                    update_post_meta(
                        $bot_id,
                        \WPAICG\Lib\Chat\Triggers\AIPKit_Trigger_Storage::META_KEY,
                        wp_json_encode($triggers_array, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES)
                    );
                }
            }
        }

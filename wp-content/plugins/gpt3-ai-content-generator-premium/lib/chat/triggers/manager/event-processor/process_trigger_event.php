...
function process_trigger_event_logic(AIPKit_Trigger_Event_Processor $processorInstance, int $bot_id, string $event_name, array $context_data): array {
    ...
    foreach ($active_triggers as $trigger) {
        $event_params = $trigger['event_params'] ?? [];
        if (!is_array($event_params) || empty($event_params)) {
            $filtered_triggers[] = $trigger;
            continue;
        }

        $params_match = true;
        if ($event_name === 'form_submitted' && !empty($event_params['form_id_is'])) {
            $form_id = isset($context_data['form_id']) ? (string) $context_data['form_id'] : '';
            $expected_form_id = (string) $event_params['form_id_is'];
            if ($form_id === '' || $form_id !== $expected_form_id) {
                $params_match = false;
            }
        }
        ...
        if ($params_match) {
            ...
            log_trigger_event_logic(
                ...
                $base_log_data_for_processing,
                'trigger_skipped',
                sprintf("Trigger '%s' (ID: %s) skipped: Event parameters did not match.", $trigger['name'] ?? 'Unnamed Trigger', $trigger['id'] ?? 'unknown_trigger_id'),
                [
                    'trigger_id' => $trigger['id'] ?? 'unknown_trigger_id',
                    'trigger_name' => $trigger['name'] ?? 'Unnamed Trigger',
                    'event_name_processed' => $event_name,
                    'reason' => 'Event parameters did not match',
                ]
            );
        }
    }

    $active_triggers = $filtered_triggers;
    if (empty($active_triggers)) {
        return $result_accumulator;
    }
    ...
    foreach ($active_triggers as $trigger) {
        ...
        if ($conditions_met) {
            ...
            $context_for_action = $result_accumulator['modified_context_data'];
            $context_for_action['current_trigger_id'] = $trigger_id;
            $context_for_action['current_trigger_name'] = $trigger_name;
            $context_for_action['event_type'] = $event_name;

            $action_result = $action_executor->execute_action($action_config, $context_for_action, $bot_id);
            ...
            if (is_wp_error($action_result)) {
                ...
                } elseif ($executed_action_type === 'call_webhook' || $executed_action_type === 'webhook_sent') {

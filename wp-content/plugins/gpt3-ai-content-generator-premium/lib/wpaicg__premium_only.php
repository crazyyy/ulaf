...
if (class_exists('\WPAICG\aipkit_dashboard') && \WPAICG\aipkit_dashboard::is_pro_plan()) {
    ...
    if (!empty($module_settings['content_writer']) || !empty($module_settings['autogpt'])) {
        ...
        add_action('init', function () {
            ...
            // RSS Feed Fetcher
            $rss_fetch_action_path = WPAICG_LIB_DIR . 'content-writer/ajax/actions/class-aipkit-content-writer-fetch-rss-action.php';
            if (file_exists($rss_fetch_action_path)) {
                require_once $rss_fetch_action_path;
                if (class_exists('\WPAICG\Lib\ContentWriter\Ajax\Actions\AIPKit_Content_Writer_Fetch_Rss_Action')) {
                    $rss_fetch_handler = new \WPAICG\Lib\ContentWriter\Ajax\Actions\AIPKit_Content_Writer_Fetch_Rss_Action();
                    add_action('wp_ajax_aipkit_content_writer_fetch_rss', [$rss_fetch_handler, 'handle']);
                }
            }
        }, 20);
...
// --- NEW: Conditional Loading for Chatbot Triggers ---
if (class_exists('\WPAICG\aipkit_dashboard') && \WPAICG\aipkit_dashboard::is_pro_plan()) {
...
if (class_exists('\WPAICG\aipkit_dashboard') && \WPAICG\aipkit_dashboard::is_pro_plan()) {
    ...
    $claude_handler_path = $chat_file_upload_base_path . 'handlers/class-chat-file-upload-claude-handler.php';
    ...
    $files_to_load_for_upload = [
        ...
        $openai_handler_path, $pinecone_handler_path, $qdrant_handler_path, $claude_handler_path,
...
// --- WhatsApp integration (disabled by default) ---

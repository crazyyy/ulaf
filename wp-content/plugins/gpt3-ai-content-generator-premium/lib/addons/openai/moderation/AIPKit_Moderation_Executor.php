...
class AIPKit_Moderation_Executor
{
     ...
     */
    public static function execute(string $text, array $bot_settings = []): string|false|null
    {
        ...
        if (!AIPKit_Moderation_IsRequired::check($bot_settings)) {
        ...
        if ($moderation_result === true) { // Message Flagged
            ...
            return AIPKit_Moderation_MessageProvider::get($bot_settings); // Return the message to display to the user
        } else { // Message Passed

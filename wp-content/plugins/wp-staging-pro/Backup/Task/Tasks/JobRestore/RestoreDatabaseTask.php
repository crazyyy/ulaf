...
class RestoreDatabaseTask extends RestoreTask
{
    ...
    public function execute()
    {
        ...
        $queriesPerSecond = ($this->stepsDto->getCurrent() - $before) / (microtime(true) - $start);
        $queriesPerSecond = (int)$queriesPerSecond;
        $this->logger->info(sprintf('Executed %s/%s queries (%s queries per second)', number_format_i18n($this->stepsDto->getCurrent()), number_format_i18n($this->stepsDto->getTotal()), number_format_i18n((int)$queriesPerSecond)));

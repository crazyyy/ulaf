...
class RestoreRequirementsCheckTask extends RestoreTask
{
    ...
    protected function cannotRestoreIfThereIsNotEnoughFreeDiskSpaceForTheDatabase()
    {
        ...
        if (!file_exists($tmpFile) && !touch($tmpFile)) {
            throw new RuntimeException(__(sprintf('The backup restore could not write to the temporary file %s.', $tmpFile)));
        }
        ...
        while ($writtenBytes < $estimatedSizeNeeded) {
            ...
            if ($writtenNow === 0) {
                ...
                throw new RuntimeException(__(sprintf('It seems there is not enough free disk space to restore this backup. The backup restore needs %s of free disk space to proceed, therefore the restore will not continue.', size_format($estimatedSizeNeeded))));
            } else {
            ...
            if ($timesWritten++ >= 5) {
                if ($this->isThreshold()) {
                    ...
                    throw ThresholdException::thresholdHit(__(sprintf('Checking if there is enough free disk space to restore... (%d%%)', $percentage), 'wp-staging'));
                }
                $timesWritten = 0;
            }
        }
    ...
    protected function cannotRestoreIfBackupGeneratedOnNewerWPStagingVersion()
    {
        ...
        if (version_compare($this->jobDataDto->getBackupMetadata()->getVersion(), WPStaging::getVersion(), '>')) {
            throw new RuntimeException('This backup was generated on a newer version of WP STAGING. Please upgrade WP STAGING to restore this Backup.');
        }
    }
    ...
    {
        if (is_multisite()) {
            throw new RuntimeException('Cannot Restore! Free Version doesn\'t support restore of multisite backups.');
        }
    }
    ...
    protected function cannotMigrate()
    {
        if (!$this->jobDataDto->getIsSameSiteBackupRestore()) {
            throw new RuntimeException('Cannot Restore! Free Version doesn\'t support site migration.');
        }
    }
    ...
    protected function cannotRestoreMultipartBackup()
    {
        if ($this->jobDataDto->getBackupMetadata()->getIsMultipartBackup()) {
            throw new RuntimeException('Cannot Restore! Free Version doesn\'t support restore of multipart backups.');
        }
    }
}

...
class Meow_MWCODE_Modules_Snippet
{
    ...
    private function get_function_snippet_data( &$snippet )
    {
        ...
        foreach ( $functions_snippet as $function_snippet ) {
            if ( $function_snippet['snippetId'] == $snippetId ) {
                ...
                if ( !isset( $function_snippet['behavior'] ) || empty( $function_snippet['behavior'] ) ) {
                    $snippet['functionBehavior'] = 'dynamic';
                }
                else {
                    $snippet['functionBehavior'] = $function_snippet['behavior'];
                }
                if ( !isset( $function_snippet['target'] ) || empty( $function_snippet['target'] ) ) {
                    $snippet['functionTarget'] = 'PHP';
                }
                else {
    ...
    public function create_or_update_function_snippet( $params )
    {
        ...
        foreach ( $params['functionArgs'] as $argName ) {
            if ( !empty( $argName ) ) {
                ...
                $snippet['args'][] = [
                    'name' => $argName,
                    'desc' => $argData['description'] ?? $argData['desc'] ?? '',  // Support both 'description' and 'desc'
    ...
    {
        $valide_scopes = ['backend', 'frontend', 'function', 'persistent', 'scheduled', 'content_php', 'content_js'];
        $errors = [];
    ...
    private function formatParamsForFront( $params )
    {
        ...
        $scopes = ['backend', 'frontend', 'function', 'persistent', 'scheduled', 'content_php', 'content_js'];
    ...
    {
        $scopes = ['function', 'scheduled', 'global', 'content_php', 'content_js'];

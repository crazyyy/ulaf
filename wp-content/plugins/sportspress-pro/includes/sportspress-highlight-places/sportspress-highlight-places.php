...
class SportsPress_Highlight_Places {
	...
	public function __construct() {
		...
		add_action( 'sportspress_before_template', array( $this, 'add_inline_css' ), 10, 4 );
		add_action( 'sportspress_after_template', array( $this, 'add_highlight_places_info' ), 10, 4 );
	...
	private function define_constants() {
		if ( !defined( 'SP_HIGHLIGHT_PLACES_VERSION' ) )
			define( 'SP_HIGHLIGHT_PLACES_VERSION', '2.7.26' );
	 ...
	 * Add inline css code
	 */
	public function add_inline_css( $template_name, $template_path, $located, $args ) {
		if ( 'league-table.php' === $template_name ) {
			...
			if ( is_array( $sp_highlight_places ) ) {
				...
				foreach ( $sp_highlight_places as $place => $info) { 
					$sp_inline_css .= '
					table.sp-league-table-'.$id.' tr.sp-row-no-'.( $place-1 ).$rank_css.'{
	...
	/**
	 * Add extra info below the table to explain what each color means
	 */
	public function add_highlight_places_info( $template_name, $template_path, $located, $args ) {
		if ( 'league-table.php' === $template_name ) {
			if ( isset( $args['id'] ) ) {
				$id = $args['id'];
			}else{
			...
			if ( is_array( $sp_highlight_places ) ) {
				...
				foreach ( $sp_highlight_places as $place => $info) { 
					...
					if ( empty( $info["desc"] ) )
						continue;
					if ( $info["color"] == $highlight_last_color )
						continue;
					...
					$highlight_last_color = $info["color"];
				}
				$sp_extra_info .= '</div>';
				echo $sp_extra_info;
			}

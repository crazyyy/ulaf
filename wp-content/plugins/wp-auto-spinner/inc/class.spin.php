...
class wp_auto_spin_spin {
	...
	function spin_openai() {
		...
		if (trim ( $wp_auto_spinner_openai_api_key ) != ''  ) {
			...
			// message field 
			$default_message = "Replace words with other synonyms and change sentences structure but keep HTML tags and format as-is";
			...
			// call the api
			try {
				$api_response = $this->api_call ( 'rewrite', $args );
			} catch ( Exception $e ) {

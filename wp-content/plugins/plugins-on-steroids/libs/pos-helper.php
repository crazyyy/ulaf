...
add_filter( 'site_option_active_sitewide_plugins', 'pos_plugin_filter' );
		...
		if ( is_array( $blocked_urls ) && count( $blocked_urls ) > 0 ) {
			if ( isset( $blocked_urls[$absolute_slug] ) ) {
				foreach ( $plugins as $index => $plugin ) {
					$parts_single = explode( '/', $plugin );
					$parts_multi = explode( '/', $index );
					$needle_single = $parts_single[0];
					$needle_multi = $parts_multi[0];
					if ( in_array( $needle_single, $blocked_urls[$absolute_slug] ) || in_array( $needle_multi, $blocked_urls[$absolute_slug] ) ) {
						unset( $plugins[$index] );
					}
				}
			}

...
class EazyPluginManager
{
	...
	{
		if (!is_network_admin() && get_option('pos_activation_redirect', false) == true) {
	...
	{
		define('POS_VERSION', (defined('POS_DEV_MODE') && POS_DEV_MODE) == true ? time() : get_plugin_data(__FILE__, false, false)['Version']);
	...
	function install_plugin_from_slug()
	{
		...
		// Check capability early
		if (! defined('ABSPATH') || ! is_user_logged_in() || ! current_user_can('install_plugins')) {
			wp_send_json_error(['message' => 'Unauthorized'], 403);
		}

		// Verify nonce for security
		if (empty($_REQUEST['nonce']) || ! wp_verify_nonce($_REQUEST['nonce'], 'pos')) {
		...
		if (empty($slug)) {
			wp_send_json_error(['message' => 'Plugin slug is required'], 400);
		}

		try {
			$posp = new POS_Plugins();
			$result = $posp->installFromSlug($slug, $version);
	...
	function load_admin_assets($hook)
	{
		...
		if (is_multisite()) {
			$active_plugins = array_unique(array_merge($active_plugins, array_keys(get_site_option('active_sitewide_plugins'))));
		}
		$current_user = wp_get_current_user();
		...
		$pos['active_plugins']   = json_encode($active_plugins);
		...
		if ('plugins.php' == $hook) {
			...
			wp_enqueue_script('alpine', plugin_dir_url(__FILE__) . 'assets/js/alpine.min.js', ['jquery', 'collapse'], '3.14.8', true);
		...
		if ('plugin-install.php' == $hook) {
			...
			wp_enqueue_script('alpine', plugin_dir_url(__FILE__) . 'assets/js/alpine.min.js', ['jquery', 'pos-bookmark', 'alpine-collapse.js'], '3.14.8', true);
		...
		if ('toplevel_page_pos-settings' == $hook) {
			...
			wp_enqueue_script('alpine', plugin_dir_url(__FILE__) . 'assets/js/alpine.min.js', ['jquery', 'pos-admin', 'collapse'], '3.14.8', true);

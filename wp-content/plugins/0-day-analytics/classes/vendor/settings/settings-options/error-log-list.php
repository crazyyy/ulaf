...
Settings::set_current_options( $settings );
		...
		if ( System_Status::environment_info()['wp_debug_mode'] ) {
			...
			Settings::build_option(
				array(
					...
					'type'    => 'checkbox',
					'hint'    => \esc_html__( 'Check this if you want plugin to try to set its own debugging (if there is no other option like wp-confing is not writable, no other plugin set that, or file system is unaccessible otherwise). IMPORTANT: if you enable this, the "Keep monitoring" option must be enabled too!', '0-day-analytics' ),
	...
	Settings::build_option(
		array(
			'name'    => \esc_html__( 'Do not use plugin exception handler', '0-day-analytics' ),
			'id'      => 'plugin_exception_handler_disable',
			'type'    => 'checkbox',
			'hint'    => \esc_html__( 'Check this if you do not want plugin to use it own exception handler. It is in use by default, but that interferes with error capturing mechanism of the WordPress - enable this setting it that gives you trouble.', '0-day-analytics' ),
			'default' => Settings::get_option( 'plugin_exception_handler_disable' ),
		)
	);
	...
	$options   = array(
		'-1' => esc_html__( 'Never', '0-day-analytics' ),
	);

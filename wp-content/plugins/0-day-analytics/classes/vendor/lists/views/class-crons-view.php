	...
	class Crons_View {
		...
		public static function analytics_cron_page() {
			...
			if ( ! \current_user_can( 'manage_options' ) ) {
				\wp_die( \esc_html__( 'You do not have permission to manage cron jobs.', '0-day-analytics' ) );
			}
			\add_thickbox();
				...
				<div class="wrap">
					...
					if ( false === $cron ) {
						...
						<form method="post" action="<?php echo \esc_url( \admin_url( 'admin-post.php' ) ); ?>">
							...
							printf(
								'<input type="hidden" name="plugin" value="%s"/>',
								\esc_attr( isset( $_REQUEST['plugin'] ) ? \sanitize_text_field( \wp_unslash( $_REQUEST['plugin'] ) ) : '' )
							);
							printf(
								'<input type="hidden" name="site" value="%s"/>',
								\esc_attr( isset( $_REQUEST['site'] ) ? \sanitize_text_field( \wp_unslash( $_REQUEST['site'] ) ) : '' )
							);
							?>
							...
							<table class="form-table">
								<tbody>
									...
									<tr>
										...
										<td>
											<textarea class="large-text code" name="cron_args" id="transient-editor" style="height: 302px; padding-left: 35px; max-width:100%;"><?php echo esc_textarea( $arguments ); ?></textarea>
											<?php
											printf(
											/* translators: 1, 2, and 3: Example values for an input field. */
												\esc_html__( 'Use a JSON encoded array, e.g. %1$s, %2$s, or %3$s', '0-day-analytics' ),
												'<code>[25]</code>',
												'<code>["asdf"]</code>',
		...
		public static function update_cron() {
			// Capability guard: only allow administrators (or users with equivalent capability).
			if ( ! \current_user_can( 'manage_options' ) ) {
				\wp_die( \esc_html__( 'You do not have permission to manage crons.', '0-day-analytics' ) );
			}
		...
		/**
		 * Handles plugin dropdown filter submission for cron list.
		 *
		 * @return void
		 */
		public static function plugin_filter_action() {
			if ( isset( $_REQUEST['plugin_top'] ) || isset( $_REQUEST['plugin_bottom'] ) ) {
				if ( \check_admin_referer( Crons_List::PLUGIN_FILTER_ACTION, Crons_List::PLUGIN_FILTER_ACTION . 'nonce' ) ) {
					$plugin = isset( $_REQUEST['plugin_top'] ) ? \sanitize_text_field( \wp_unslash( $_REQUEST['plugin_top'] ) ) : \sanitize_text_field( \wp_unslash( $_REQUEST['plugin_bottom'] ) );
						...
						\remove_query_arg(
							...
							\add_query_arg(
								array(
									...
									'site'       => ( isset( $_REQUEST['site'] ) ? rawurlencode( sanitize_text_field( wp_unslash( $_REQUEST['site'] ) ) ) : '' ),
									'event_type' => ( isset( $_REQUEST['event_type'] ) ? \sanitize_text_field( \wp_unslash( $_REQUEST['event_type'] ) ) : '' ),
									'schedules_filter' => ( isset( $_REQUEST['schedules_filter'] ) ? \sanitize_text_field( \wp_unslash( $_REQUEST['schedules_filter'] ) ) : '' ),
								),
								\admin_url( 'admin.php' )
							)
						)
		...
		/**
		 * Handles site filter submission for cron list.
		 *
		 * @return void
		 */
		public static function site_filter_action() {
			// Only network admins can filter by site.
			if ( ! ( function_exists( 'is_multisite' ) && is_multisite() && current_user_can( 'manage_network' ) ) ) {
				return;
			}
			if ( isset( $_REQUEST['site_top'] ) || isset( $_REQUEST['site_bottom'] ) ) {
				if ( \check_admin_referer( Crons_List::SITE_FILTER_ACTION, Crons_List::SITE_FILTER_ACTION . 'nonce' ) ) {
					$site = isset( $_REQUEST['site_top'] ) ? \sanitize_text_field( \wp_unslash( $_REQUEST['site_top'] ) ) : \sanitize_text_field( \wp_unslash( $_REQUEST['site_bottom'] ) );
					\wp_safe_redirect(
						\remove_query_arg(
							array( 'deleted' ),
							\add_query_arg(
								array(
									...
									'plugin'     => ( isset( $_REQUEST['plugin'] ) ? rawurlencode( sanitize_text_field( wp_unslash( $_REQUEST['plugin'] ) ) ) : '' ),
									'event_type' => ( isset( $_REQUEST['event_type'] ) ? \sanitize_text_field( \wp_unslash( $_REQUEST['event_type'] ) ) : '' ),
									'schedules_filter' => ( isset( $_REQUEST['schedules_filter'] ) ? \sanitize_text_field( \wp_unslash( $_REQUEST['schedules_filter'] ) ) : '' ),
								),
								\admin_url( 'admin.php' )

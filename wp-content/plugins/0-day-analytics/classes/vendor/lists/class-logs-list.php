...
if ( ! class_exists( '\ADVAN\Lists\Logs_List' ) ) {
	 ...
	 * @since 1.1.0
	 */
	class Logs_List extends Abstract_List {
			...
			while ( empty( $errors ) ) {
				...
				while ( $items > 0 ) {
					$result = Reverse_Line_Reader::read_file_from_end(
						$file,
						function( $line, $pos ) use ( &$collected_items, &$errors, &$position, $items, $write_temp ) {

							$position = $pos;

							// Prevent memory exhaustion from extremely large multi-line errors.
							if ( count( $collected_items ) > 500 ) { // Reasonable limit for stack traces.
								$collected_items = array_slice( $collected_items, -100 ); // Keep last 100 items.
								// if ( function_exists( 'error_log' ) ) {
								// 	error_log( 'Logs_List: Truncated collected_items due to size limit (' . count( $collected_items ) . ' items)' );
								// }
							}
							...
							if ( ! empty( $line ) && null !== Log_Line_Parser::parse_entry_with_stack_trace( $line ) ) {
								...
								if ( \is_array( $parsed_data ) && isset( $parsed_data['message'] ) ) {
									...
									// Prevent memory exhaustion from too many errors.
									if ( count( $errors ) >= $items * 2 ) { // Allow some buffer.
										return array(
											'close'     => true,
											'line_done' => true,
											'no_flush'  => false,
					...
					if ( ! empty( $last_error ) && ! $first_only ) {
						$plugin = Plugin_Theme_Helper::get_plugin_from_file_path( $last_error['message'] );
			...
			switch ( $column_name ) {
				case 'severity':
					if ( isset( $item['severity'] ) && ! empty( $item['severity'] ) ) {
						...
						if ( isset( Settings::get_option( 'severities' )[ $item['severity'] ] ) ) {
							...
							return '<span class="badge dark-badge" style="color: ' . \esc_attr( Settings::get_option( 'severities' )[ $item['severity'] ]['color'] ) . ' !important;">' . \esc_html( $item['severity'] ) . '</span>';
						} else {
							return '<span class="badge dark-badge">' . \esc_html( $item['severity'] ) . '</span>';
						}
					} else {
						return '<span class="badge dark-badge" style="color: ' . \esc_attr( Settings::get_option( 'severities' )['not set']['color'] ) . ' !important;">' . \esc_html( 'not set' ) . '</span>';
					}
					break;
				case 'timestamp':
					if ( 1 === $item['timestamp'] ) {
					...
					if ( isset( $item['sub_items'] ) && ! empty( $item['sub_items'] ) ) {
						...
						foreach ( $reversed_details as $val ) {
							...
							if ( isset( $val['file'] ) && ! empty( $val['file'] ) ) {
								...
								$source_link = ' <a href="' . \esc_url( $view_url ) . '" title="' . \esc_attr( $title ) . '" class="thickbox view-source">' . \esc_html( $query_array['error_file'] ) . ':' . \esc_html( (string) $query_array['error_line'] ) . '</a><br>';

							}

							$message .= ( isset( $val['call'] ) && ! empty( $val['call'] ) ) ? '<b><i>' . \esc_html( $val['call'] ) . '</i></b> - ' : '';

							if ( ! empty( $source_link ) ) {
								$message .= $source_link;
							} else {
								$message .= ( isset( $val['file'] ) && ! empty( $val['file'] ) ) ? \esc_html( $val['file'] ) . ' ' : '';
				...
				case 'plugin_theme':
					...
					if ( isset( $item['error_file'] ) && ! empty( $item['error_file'] ) ) {
						...
						$source_link = '<div> <a href="' . \esc_url( $view_url ) . '" title = "' . \esc_attr( $title ) . '" class="thickbox view-source gray_lab badge">' . __( 'view error source', '0-day-analytics' ) . '</a></div>';
					}

					$message = \esc_html( $item['message'] );

					if ( isset( $item['plugin'] ) && ! empty( $item['plugin'] ) ) {
						return __( 'Plugin: ', '0-day-analytics' ) . '<b>' . \esc_html( Plugin_Theme_Helper::get_sources()['plugins'][ $item['plugin'] ]['Name'] ) . '</b><br>' . \__( 'Current version: ', '0-day-analytics' ) . \esc_html( Plugin_Theme_Helper::get_sources()['plugins'][ $item['plugin'] ]['Version'] ) . $source_link;
					}

					$theme = Plugin_Theme_Helper::get_theme_from_file_path( $message );
					...
					if ( isset( $item['source'] ) ) {
						return \esc_html( $item['source'] ) . $source_link;
					} else {
						return '';
					}
				default:
		...
		public function extra_tablenav( $which ) {
			...
			if ( false !== $log_file ) {
				...
				if ( 'top' === $which && count( self::$items_collected ) > 0 ) {
					?>
					<div id="export-form" style="display:inline-block;">
						<div>
							<button id="start-export" class="button" data-type-export="logs" data-search="<?php echo \esc_attr( self::escaped_search_input() ); ?>" data-plugin-filter="<?php echo ( isset( self::$query_args['plugin_filter'] ) ) ? \esc_attr( self::$query_args['plugin_filter'] ) : '-1'; ?>">
			...
			if ( false !== $log_file ) {
				...
				<div class="">
					<?php
					if ( ! empty( Plugin_Theme_Helper::get_sources()['plugins'] ) ) {
						?>
							<select name="plugin_filter">
								<option value="-1"><?php \esc_html_e( '- Choose plugin to filter -', '0-day-analytics' ); ?></option>
								<?php
								foreach ( Plugin_Theme_Helper::get_sources()['plugins'] as $plugin_base => $plugin ) {
									...
									?>
								<option <?php echo $selected; ?> value="<?php echo \esc_attr( $plugin_base ); ?>"><?php echo \esc_html( $plugin['Name'] ); ?></option>
									<?php
								}
								?>
							</select>
				...
				<div style="margin: 1em;">
					<label for="single_severity_filter_<?php echo \esc_attr( $which ); ?>">
						<?php echo \esc_html__( 'or choose single severity to filter for: ', '0-day-analytics' ); ?>
					</label>
					<?php
					...
					<option value="none_selected"><?php \esc_html_e( '-- Choose --', '0-day-analytics' ); ?></option>
						...
						foreach ( $severities as $name => $severity ) {
							...
							?>
								<option <?php echo ( $selected ); ?> value="<?php echo \esc_attr( $name ); ?>"><?php echo \esc_html( $name ); ?></option>
								<?php
						}
						?>
					</select>
						...
						$dismissed = \get_user_meta( \get_current_user_id(), '_aadvana_filter_error_log_notice_dismissed', true );

						if ( ! $dismissed ) {
							?>

						<div id="advaa-filtering-notice" class="notice notice-info is-dismissible">
							<p>
								<?php \esc_html_e( 'Note: Filtering error log is expensive (in terms of resources) operation, so use it with care. Important info to keep in mind - if you don\'t use only ONE severity for filtering, you will see more severities, outside selected ones for filtering, because error log may contain all kinds of errors and it is impossible for any software to have\'em all, because there are no strict standards. It is implemented this way because during few severities filtering there may be more important ones (errors in log) which are not covered from the plugin, and it is best not to miss them.', '0-day-analytics' ); ?>
							</p>
						</div>
						<script>
							jQuery(document).on('click', '#advaa-filtering-notice .notice-dismiss', function(){
								jQuery.ajax({
									url: ajaxurl,
									type: 'GET',
									data: {
										action: 'advana_error_log_filtering_dismiss_notice'
									}
								});
							});
						</script>
		...
		public static function extract_last_item() {
			...
			if ( $event && ! empty( $event ) ) {
				...
				if ( isset( Settings::get_option( 'severities' )[ $event['severity'] ] ) ) {
					...
					$style = 'border-left: 5px solid ' . \esc_attr( Settings::get_option( 'severities' )[ $event['severity'] ]['color'] ) . ' !important;';
				}
		...
		public static function set_severity_status( \WP_REST_Request $request ) {
			// Require appropriate capability for changing severity visibility.
			if ( ! \current_user_can( 'manage_options' ) ) {
				return new \WP_Error(
					'rest_forbidden',
		...
		public static function get_notification_data(): array {
			...
			if ( $event && ! empty( $event ) ) {
				...
				if ( $last_transient_reported_data < $event['timestamp'] ) {
					...
					// Escape severity and message to avoid injecting markup from log content into notifications.
					$data['body']  = \esc_html( $event['severity'] ) . ' ' . \esc_html( \html_entity_decode( (string) $event['message'] ) );
					$data['title'] = $in;
					...
					$data['url']   = Miscellaneous::get_error_log_page_link();
		...
		public static function add_cron_job( $crons ) {
			return static::add_standard_cron_job(
				$crons,
				'advana_error_log_clear',
				ADVAN_PREFIX . 'error_log_clear',
				array( __CLASS__, 'truncate_error_log' )
			);
		}

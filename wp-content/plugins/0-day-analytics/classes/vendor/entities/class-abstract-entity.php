...
if ( ! class_exists( '\ADVAN\Entities\Abstract_Entity' ) ) {
	...
	abstract class Abstract_Entity {
		...
		/**
		 * Child classes must implement table creation.
		 *
		 * @param \wpdb $connection Connection instance.
		 *
		 * @return bool True on success.
		 *
		 * @since 4.1.1
		 */
		abstract protected static function create_table( \wpdb $connection ): bool;

		/**
		 * Holds default field values (override in child classes).
		 *
		 * @var array
		 *
		 * @since 4.1.1
		...
		/**
		 * Validates SQL identifiers like table and column names (letters, numbers, underscore).
		 *
		...
		/**
		 * Backtick-quote an identifier and escape embedded backticks.
		 *
		...
		/**
		 * Log and hook when a table is auto-recreated after a missing-table error.
		 *
		 * @param string $table_name Fully-qualified table name.
		 * @return void
		 */
		private static function log_table_recreated( string $table_name ): void {
			/**
			 * Fires when an ADVAN entity table is auto-recreated after being missing.
			 *
			 * @param string $table_name Table name that was recreated.
			 */
			\do_action( 'advan_entity_table_recreated', $table_name );
			// if ( \function_exists( 'error_log' ) ) {
			// \error_log( '[advanced-analytics] Recreated missing table: ' . $table_name ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
			// }
		}
		...
		public static function check_index_exists( string $index ): bool {
			...
			if ( '' === $index || ! self::is_valid_identifier( $index ) ) {
				return false;
			}

			$results = static::get_connection()->get_results( 'SHOW INDEX FROM ' . self::bt( self::get_table_name() ) . '', \ARRAY_A );
			...
			if ( '' !== $_wpdb->last_error ) {
				if ( 1146 === self::get_last_sql_error( $_wpdb ) ) {
					if ( ( static::class )::create_table( $_wpdb ) ) {
						self::log_table_recreated( self::get_table_name( $_wpdb ) );
						$count = 0;
					}
				}
			}
		...
		public static function duplicate_by_id( int $id, $connection ) {
			...
			$table_name = self::get_table_name( $_wpdb ); // phpcs:ignore Generic.Formatting.MultipleStatementAlignment.NotSameWarning
			$all_fields = array_keys( self::get_fields() ); // phpcs:ignore Generic.Formatting.MultipleStatementAlignment.NotSameWarning
			$insert_list = static::get_duplicate_fields( false );
			$select_list = static::get_duplicate_fields( true );

			$insert_cols  = array();
			$select_parts = array();

			foreach ( $insert_list as $f ) {
				if ( is_string( $f ) && self::is_valid_identifier( $f ) && in_array( $f, $all_fields, true ) ) {
					$insert_cols[] = self::bt( $f );
				}
			}

			foreach ( $select_list as $f ) {
				if ( is_numeric( $f ) ) {
					$select_parts[] = (string) $f;
					continue;
				}
				if ( is_string( $f ) && self::is_valid_identifier( $f ) && in_array( $f, $all_fields, true ) ) {
					$select_parts[] = self::bt( $f );
				}
			}

			if ( empty( $insert_cols ) || empty( $select_parts ) || count( $insert_cols ) !== count( $select_parts ) ) {
				return false;
			}

			$sql = 'INSERT INTO ' . self::bt( $table_name ) . ' (' . implode( ',', $insert_cols ) . ') SELECT ' . implode( ',', $select_parts ) . ' FROM ' . self::bt( $table_name ) . ' WHERE id = %d';

			$_wpdb->suppress_errors( true );
			$result = $_wpdb->query( $_wpdb->prepare( $sql, $id ) );
			if ( '' !== $_wpdb->last_error ) {
				if ( 1146 === self::get_last_sql_error( $_wpdb ) ) {
					if ( ( static::class )::create_table( $_wpdb ) ) {
						self::log_table_recreated( self::get_table_name( $_wpdb ) );
		 ...
		 * @param \wpdb  $connection - The DB connection object.
		 *
		 * @return array
		 *
		 * @since 3.0.1
		...
		/**
		 * Returns single variable from the provided query.
		 *
		 * @param string $query - The SQL query to execute.
		 * @param \wpdb  $connection - The DB connection object.
		 ...
		 * @since 3.0.1
		 */
		public static function get_var( string $query, $connection = null, $x = 0, $y = 0 ) {
			if ( null !== $connection ) {
				if ( $connection instanceof \wpdb ) {
					$_wpdb = $connection;
				} else {
		...
		/**
		 * Returns all column names which belong to the table
		 *
		 * @return array
		 *
		 * @since 3.8.0
		 */
		public static function get_all_columns(): array {
			return ( static::class )::$fields;
		}
	}
}

	...
	class WP_Helper {
		...
		private static function get_altis_vendor_name( string $file ): string {
			if ( ! defined( 'Altis\\ROOT_DIR' ) ) {
				return __( 'Dependency: unknown', '0-day-analytics' );
			}
			$plug = str_replace( constant( 'Altis\\ROOT_DIR' ) . '/vendor/', '', $file );
			$plug = explode( '/', $plug, 3 );
			$plug = $plug[0] . '/' . $plug[1];
		...
		public static function get_file_dirs(): array {
			...
			self::$file_dirs = array(
				...
				'vip-client-mu-plugin' => defined( 'WPCOM_VIP_CLIENT_MU_PLUGIN_DIR' ) ? constant( 'WPCOM_VIP_CLIENT_MU_PLUGIN_DIR' ) : null,
				'altis-vendor'         => defined( 'Altis\\ROOT_DIR' ) ? constant( 'Altis\\ROOT_DIR' ) . '/vendor' : null,
				'theme'                => null,
		...
		public static function check_cron_status() {
			...
			if ( defined( 'DISABLE_WP_CRON' ) && DISABLE_WP_CRON ) {
				$url = \add_query_arg( array( 'page' => Settings::SETTINGS_MENU_SLUG ), \network_admin_url( 'admin.php' ) ) . '#aadvana-options-tab-cron-list';
				$url = \esc_url( $url );
				return new \WP_Error(
					'advana_cron_info',
					sprintf(
					/* translators: %s: The name of the PHP constant that is set. %s The url to the cron settings */
						__( 'The %1$s constant is set to true. WP-Cron spawning is disabled. Try to enable it in the settings - %2$s', '0-day-analytics' ),
						'DISABLE_WP_CRON',
						'<a href="' . $url . '">' . __( 'here', '0-day-analytics' ) . '</a>',
					)
				);
			}
		}
		...
		public static function check_debug_status() {
			...
			if ( ! Settings::get_option( 'keep_reading_error_log' ) ) {
				if ( ! defined( 'WP_DEBUG' ) || ! \WP_DEBUG ) {
					$url = \add_query_arg( array( 'page' => Settings::SETTINGS_MENU_SLUG ), network_admin_url( 'admin.php' ) ) . '#aadvana-options-tab-error-log-list';
					$url = \esc_url( $url );
					return new \WP_Error(
						'debug_off',
						sprintf(
		...
		public static function time_formatter( array $item, string $label_string ): string {
			...
			if ( $late ) {
				...
				return sprintf(
					...
					esc_html( $ago ),
					$time,
				) . '<br><span class="badge red-badge">' . \esc_html( $label_string ) . '</span>';
			}
		...
		public static function get_mysql_time_zone(): string {
			...
			if ( ! $timezone_string instanceof \DateTimeZone ) {
				$timezone_string = new \DateTimeZone( (string) $timezone_string );
			}

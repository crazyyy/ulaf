	...
	class Upgrade_Notice {
		...
		public static function init() {
			...
			if ( 'plugins' === WP_Helper::get_wp_screen()->id ) {
				...
				// Restrict privileged actions to users who can activate plugins (tighten authorization surface).
		...
		private static function get_upgrade_notice( $version ) {
			// Sanitize version for transient key safety.
			$sanitized_version = preg_replace( '/[^0-9\\.]/', '', (string) $version );

			$transient_name = 'advan_upgrade_notice_' . $sanitized_version;
		...
		private static function parse_update_notice( $content, $new_version ) {
			...
			// Specify the delimiter in preg_quote to comply with WP coding standards.
				...
				if ( preg_match( $notice_regexp, $content, $matches ) ) {
					...
					if ( version_compare( trim( $matches[1] ), $check_version, '=' ) ) {
						...
						foreach ( $notices as $line ) {
							// Convert markdown-style links to safe anchors.
							$upgrade_notice .= preg_replace_callback(
								'~\\[([^\\]]*)\\]\\(([^\\)]*)\\)~',
								static function( $m ) {
		...
		public static function extract_plugin_versions( string $plugin_slug ) {
			...
			// Sanitize slug input defensively.
			...
			if ( is_array( $plugin_information->versions ) && ! empty( $plugin_information->versions ) ) {
				...
				foreach ( $plugin_information->versions as $version => $download_link ) {
					$key = array_search( $download_link, $versions, true );
			...
			if ( ! empty( $versions ) ) {
				if ( ! array_key_exists( 'errors', $versions ) ) {
					$option_html = "<option value='' >" . esc_html__( 'Select Version', '0-day-analytics' ) . '</option>';
					foreach ( $versions as $version ) {
						$option_html .= '<option value="' . esc_attr( $version ) . '">' . esc_html( $version ) . '</option>';
					}
					return $option_html;
				} else {
		...
		public static function version_switch( $plugin_slug, $version ) {
			...
			$available = self::extract_plugin_versions( $plugin_slug );
			if ( \is_wp_error( $available ) ) {
				return $available; // Propagate error.
			}
			$found = false;
			if ( is_string( $available ) ) {
				if ( preg_match_all( '/<option[^>]*value=["\']([^"\']+)["\']/', $available, $matches ) ) {
					foreach ( $matches[1] as $opt_version ) {
			...
			if ( ! $found ) {
				return new \WP_Error( 'plugin_version_not_allowed', __( 'Selected version not in allowed list.', '0-day-analytics' ) );
			}
		...
		public static function upgrade( string $plugin_name, $version ) {
			...
			// Sanitize inputs.
			$plugin_name = \sanitize_key( (string) $plugin_name );
			$version     = preg_replace( '/[^0-9\.]/', '', (string) $version );

			if ( empty( $version ) ) {
				return new \WP_Error( 'plugin_version_invalid', __( 'Invalid version.', '0-day-analytics' ) );
			}

			// Verify plugin slug exists among installed plugins directories.
			...
			foreach ( array_keys( $installed ) as $file ) {
				if ( dirname( $file ) === $plugin_name ) {
					$slug_found = true;
					break;
				}
			}
			if ( ! $slug_found ) {
				return new \WP_Error( 'plugin_not_installed', __( 'Plugin slug not found among installed plugins.', '0-day-analytics' ) );
			}
		...
		public static function load_custom_wp_admin_style( $hook ) {
			// Use $hook to ensure function parameter is utilized and restrict script enqueue context if needed.
			if ( 'plugins.php' !== $hook ) {
				return; // Only output on plugins screen.
			}
			?>
				<script>
					window.addEventListener("load", () => {
						jQuery( '.switch_plugin_version' ).on( 'click', function(e) {
							var data = {
								'action': '<?php echo \esc_attr( ADVAN_PREFIX ); ?>extract_plugin_versions',
							...
							jQuery.post(
								...
								if ( data && data.data && jQuery.trim( data.data ) ) {
									jQuery('#<?php echo \esc_attr( ADVAN_PREFIX ); ?>' + jQuery(that).data('plugin-slug')).html(data.data).show();
									jQuery('#<?php echo \esc_attr( ADVAN_PREFIX ); ?>switch_plugin_to_version_' + jQuery(that).data('plugin-slug')).show();
									that.remove();
								} else {

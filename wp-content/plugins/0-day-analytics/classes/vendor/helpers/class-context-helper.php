...
if ( ! class_exists( '\ADVAN\Helpers\Context_Helper' ) ) {
	/**
	...
	class Context_Helper {
		...
		private static function is_rest_request(): bool {
			if (
				...
				|| \filter_has_var( INPUT_GET, 'rest_route' )
				) {
				return true;
			}
		...
		private static function is_login_request(): bool {
			...
			// Check for interim-login via GET or POST without directly accessing superglobals.
			if ( \filter_has_var( INPUT_GET, 'interim-login' ) || \filter_has_var( INPUT_POST, 'interim-login' ) ) {
				return true;
			}
			 ...
			 */
			// Use basename of SCRIPT_NAME and avoid direct superglobal usage side effects.
			...
			if ( empty( self::$action_callbacks ) ) {
				self::$action_callbacks = array(
					...
					'current_screen'    => function ( \WP_Screen $screen ): void {
						$screen->in_admin() && self::reset_and_force( self::BACKOFFICE );
					},
				);
			}

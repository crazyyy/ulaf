...
if ( ! class_exists( '\ADVAN\Helpers\WP_Error_Handler' ) ) {
	...
	class WP_Error_Handler {
		...
		/**
		 * Keys considered sensitive and subject to redaction when logging.
		 *
		 * @since 3.9.0
		 * @var array
		 */
		...
		/**
		 * Redacts sensitive values from arbitrary data structures prior to logging.
		 *
		...
		/**
		 * Sanitizes a message string for safe external transport/logging.
		 *
		...
		private static function sanitize_message( $message ) {
			...
			// Normalize whitespace & limit extremely long messages to prevent log flooding.
		...
		public static function log_rest_api_errors( $result, $server, $request ) {
			if ( $result->is_error() ) {
				$params = self::redact_data( $request->get_params() );
				// Do not leak potentially sensitive request params; keep a compact representation.
				$encoded_params = var_export( $params, true );
				error_log(
					sprintf(
						'REST API request: %s:',
						$request->get_route(),
				...
				error_log(
					sprintf(
						'REST API %s: %s.',
						isset( $result->get_data()['code'] ) ? self::sanitize_message( (string) $result->get_data()['code'] ) : 'unknown_code',
						isset( $result->get_data()['message'] ) ? self::sanitize_message( (string) $result->get_data()['message'] ) : 'unknown_message',
					) . \PHP_EOL .
		...
		public static function shutdown() {
			...
			if ( null !== $error && ( \in_array( $error['type'], array( 1, 4 ), true ) ) ) {
				...
				$sanitized_errstr  = self::sanitize_message( $errstr );
				$public_path       = self::clean_file_path( $errfile );
				$notification_body = WP_Helper::get_blog_domain() . "\n" . self::error_code_to_string( $errno ) . ' ' . $sanitized_errstr . ' ' . $public_path . ' ' . (int) $errline . "\n" . __( 'Recovery URL not sent for security.', '0-day-analytics' );
				if ( Slack::is_set() ) {
					// Send sanitized error to Slack (without sensitive recovery token).
					Slack_API::send_slack_message_via_api( null, null, $notification_body );
				}

				if ( Telegram::is_set() ) {
					// Send sanitized error to Telegram.
					Telegram_API::send_telegram_message_via_api( null, null, $notification_body );
				}

				if ( Settings::get_option( 'fatals_module_enabled' ) ) {
					WP_Fatals_Entity::prepare_fatality_to_store(
						array(
							'message'   => $error['message'],
							'file'      => $error['file'],
							'line'      => $error['line'],
							'severity'  => self::error_code_to_string( $error['type'] ),
							'backtrace' => '',
						)
					);
				}
		...
		public static function exception_handler( $e ) {
			...
			$sanitized_message = self::sanitize_message( $e->getMessage() );
			$public_path       = self::clean_file_path( $e->getFile() );
			$notification_body = WP_Helper::get_blog_domain() . "\n" . $error . ' ' . $sanitized_message . ' ' . $public_path . ' ' . (int) $e->getLine() . "\n" . __( 'Recovery URL not sent for security.', '0-day-analytics' );
			if ( Slack::is_set() ) {
				// Send sanitized error to Slack.
				Slack_API::send_slack_message_via_api( null, null, $notification_body );
			}
			...
			if ( $previous instanceof \Error || $previous instanceof \Exception ) {
				$out .= ' (Previous Error: ' . $previous->getMessage() . ' in ' . $previous->getFile() . ' on line ' . $previous->getLine() . ')';
			}
			...
			for ( $i = 0; $i < $counter; $i++ ) {
				...
				if ( ! $main_shown && isset( $trc[ $i + 3 ]['args'] ) && ! empty( $trc[ $i + 3 ]['args'] ) ) {
					$args = ' Arguments ' . \htmlentities( (string) \wp_json_encode( $trc[ $i + 3 ]['args'] ) );
				} else {
					$args = '';
				}
			...
			if ( $previous ) {
				...
				if ( $previous instanceof \Error || $previous instanceof \Exception ) {
					...
					$counter = count( $previous->getTrace() );
					for ( $i = 0; $i < $counter; $i++ ) {
						$sf    = (object) \shortcode_atts( $defaults, $previous->getTrace()[ $i ] );
						$index = $i;
						$file  = $sf->file;
		...
		public static function log_wp_error( \WP_Error $error, ?string $additional_message = '' ) {
			...
			$out = sprintf(
				'WP_Error error: %s: %s',
				self::sanitize_message( (string) $error->get_error_code() ),
				self::sanitize_message( (string) $error->get_error_message() ),
			) . \PHP_EOL .
			$error_data . self::sanitize_message( (string) $additional_message ) . PHP_EOL . 'Stack trace:' . PHP_EOL;
			self::trace_log( $out );
		 ...
		 *
		 * @param string $out - String with the lead error log line.
		 * @param bool   $just_return - Whether to just return the trace string instead of logging it.
		 *
		 * @return string
		 *
		 * @since 2.4.2.1
		 * @since 3.8.0 - Added just return the output instead of logging it.
		 */
		private static function trace_log( string $out, bool $just_return = false ) {
			...
			if ( $counter <= 0 ) {
				// Looks like WE are the source - act accordingly.
				$counter  = count( $trace );
				$how_back = 0;
			}
			for ( $i = 1; $i < $counter; $i++ ) {
				...
				if ( ! $main_shown && isset( $trace[ $i + $how_back ]['args'] ) && ! empty( $trace[ $i + $how_back ]['args'] ) ) {
					$args = ' Arguments ' . \htmlentities( \wp_json_encode( $trace[ $i + $how_back ]['args'] ) );
				} else {
					$args = '';
				}
				...
				self::log_wp_error(
					...
					// translators: %1$s is the URL of the request, %2$s are the arguments of the request.
						...
						\implode(
							...
							\array_map(
								function( $key, $value ) {
									return sprintf( '%s: %s', $key, \is_array( $value ) ? \wp_json_encode( $value ) : var_export( $value, true ) );
								},

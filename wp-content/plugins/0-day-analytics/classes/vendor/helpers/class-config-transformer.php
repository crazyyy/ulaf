...
if ( ! class_exists( '\ADVAN\Helpers\Config_Transformer' ) ) {
	...
	class Config_Transformer {
		...
		/**
		 * Performs internal check and inits the class with defaults from the plugin.
		 *
		 * @return void
		 *
		 * @since 1.1.0
		...
		public static function add( $type, $name, $value, array $options = array() ) {
			...
			$anchor_default = self::is_wp_engine() ? "# That's It. Pencils down" : "/* That's all, stop editing!";

			$defaults = array(
				'raw'       => false, // Display value in raw format without quotes.
				'anchor'    => $anchor_default, // Config placement anchor string (may vary by host).
		...
		/**
		 * Detect if the current environment is WP Engine.
		 *
		 * Uses a few heuristics: known WP Engine constants, environment variables,
		 * or the `$_SERVER['SERVER_SOFTWARE']` string.

		 * @return bool True if WP Engine is detected, false otherwise.
		 *
		 * @since 1.2.0
		 */
		protected static function is_wp_engine() {
			if ( defined( 'WPENGINE' ) || defined( 'WPE_APIKEY' ) || defined( 'WPE_CLUSTER' ) ) {
				return true;
			}

			if ( getenv( 'WPENGINE' ) || getenv( 'WPE_APIKEY' ) || getenv( 'IS_WPE' ) ) {
				return true;
			}

			$server_software = $_SERVER['SERVER_SOFTWARE'] ?? '';
			if ( $server_software && false !== stripos( $server_software, 'wp engine' ) ) {
				return true;
			}

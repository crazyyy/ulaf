...
if ( ! class_exists( '\ADVAN\Helpers\Log_Line_Parser' ) ) {
	...
	class Log_Line_Parser {
		...
		public static function parse_php_error_log_line( string $line ) {
			...
			$pattern = '/
                ^(?:\[(?P<timestamp>[\w \-+:\/]{6,50}?)\]\ )?
                (?P<message>
                    (?:(?:(?P<source>PHP|WordPress\ database|REST\ API|WP_Error)\ )?(?P<severity>[\w+ ]{3,40}?):?\ )?
                .+)$
            /x';
			...
			if ( preg_match( $pattern, $line, $matches ) ) {
				...
				*/
				if ( preg_match( '/(?:in\s+([^\s]+(?:[\\\\\/][^\s]+)*)\s+on\s+line\s+(\d+)|([^\s]+(?:[\\\\\/][^\s:]+)*)[:](\d+))/', $message, $matches ) ) {
					 ...
					 */
					if ( @\is_file( $error_file ) ) { // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged
		...
		public static function parse_php_error_log_stack_line( $message, $is_last_line = false ) {
			...
			if ( preg_match(
				'@(?:
    \#(?P<index>\d+)\s+                         # Stack frame index line starting with
    (?:
        (?P<source>
            \[internal\sfunction\]
            |
            ...
            \((?P<line>\d{1,6})?\)              # Line number (optional)
        )
        :
        |
        (?P<main>{main})\s*?$
    ...
    |
    ((?P<previous>
        Previous\s+[^:]+:\s+                   # “Previous Error:” or similar prefix
        .*?                                    # The error message (non-greedy)
        \s+in\s+)
        (?P<prev_file>
            (?:phar://)?                       # Optional PHAR prefix
            (?:[a-zA-Z]:)?                     # Optional drive letter
            [^:()]+                            # File path
        )
        \(
            (?P<prev_line>\d{1,6})             # Line number
        \)
    )
)@x',
				$message,
				...
				if ( ! empty( $matches['previous'] ) ) {
					$item['call'] = $matches['previous'];
				}

				if ( ! empty( $matches['prev_line'] ) ) {
					$item['line'] = $matches['prev_line'];
				}
				if ( ! empty( $matches['prev_file'] ) ) {
					$item['file'] = $matches['prev_file'];
				}
		...
		public static function store_last_parsed_timestamp() {
			if ( null !== self::$last_timestamp ) {
				...
				if ( false === self::get_last_parsed_timestamp() || (int) self::$last_timestamp > (int) self::get_last_parsed_timestamp() ) {
					self::$last_parsed_timestamp = (int) self::$last_timestamp;
					\set_transient( self::TIMESTAMP_TRANSIENT, (int) self::$last_timestamp, 600 );
				}

				self::get_lines_to_show_interface();

...
if ( ! class_exists( '\ADVAN\Helpers\Crons_Helper' ) ) {
	...
	class Crons_Helper {
						...
						if ( ! is_array( $cron ) ) {
							...
							foreach ( $cron as $hook => $events ) {
								foreach ( $events as $event ) {
									...
									// Site metadata for multisite clarity.
									$cron_item['site_id']   = function_exists( 'get_current_blog_id' ) ? get_current_blog_id() : 1;
									$cron_item['site_name'] = ( function_exists( 'get_bloginfo' ) ) ? get_bloginfo( 'name' ) : '';

									// Derive all plugin slugs from callback files (multi-plugin aggregation).
									$callbacks    = self::get_cron_callbacks( $hook );
									$plugin_slugs = array();
									if ( ! empty( $callbacks ) ) {
										foreach ( $callbacks as $cb ) {
											if ( isset( $cb['callback']['file'] ) && ! empty( $cb['callback']['file'] ) ) {
												$slug = Plugin_Theme_Helper::get_plugin_from_file_path( (string) $cb['callback']['file'] );
												if ( false !== $slug && ! in_array( $slug, $plugin_slugs, true ) ) {
													$plugin_slugs[] = (string) $slug;
												}
											}
										}
									}
									// Backward compatibility single slug (first) & full list.
									$cron_item['plugin_slug']  = isset( $plugin_slugs[0] ) ? $plugin_slugs[0] : '';
									$cron_item['plugin_slugs'] = $plugin_slugs; // empty array signifies core/unknown.
		...
		/**
		 * Returns the cron events for a specific site in a multisite installation.
		 * Safely switches to the blog, collects events, then restores.
		 *
		 * @param int $blog_id Site (blog) ID.
		 * @return array
		 */
		public static function get_events_for_site( int $blog_id ): array {
			...
			// Switch, clear cache, collect, restore and clear again to avoid bleed.
			switch_to_blog( $blog_id );
			self::$events = null;
			$events      = self::get_events();
			restore_current_blog();
			self::$events = null; // ensure subsequent calls on original blog re-fetch.

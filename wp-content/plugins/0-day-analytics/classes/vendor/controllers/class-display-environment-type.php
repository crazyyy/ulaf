...
if ( ! class_exists( '\ADVAN\Controllers\Display_Environment_Type' ) ) {
	...
	class Display_Environment_Type {
		...
		public static function add_toolbar_item( $admin_bar ) {
			...
			if ( ! empty( $env_type ) ) {
				...
				$admin_bar->add_node(
					array(
						'id'     => 'det-wp-debug',
						'title'  => self::show_label_value( 'WP_DEBUG', ( \defined( 'WP_DEBUG' ) && WP_DEBUG ? 'true' : 'false' ) ),
						'parent' => 'det_env_type',
					)
				);
				...
				$admin_bar->add_node(
					array(
						'id'     => 'det-wp-debug-log',
						'title'  => self::show_label_value( 'WP_DEBUG_LOG', ( \defined( 'WP_DEBUG_LOG' ) && WP_DEBUG_LOG ? 'true' : 'false' ) ),
						'parent' => 'det_env_type',
						'meta'   => array(
							'title' => \esc_attr( (string) ( \defined( 'WP_DEBUG_LOG' ) ? ( \is_string( WP_DEBUG_LOG ) ? WP_DEBUG_LOG : ( WP_DEBUG_LOG ? 'true' : 'false' ) ) : '' ) ),
						),
					)
				);
				...
				$admin_bar->add_node(
					array(
						...
						'title'  => self::show_label_value( 'WP_DEBUG_DISPLAY', ( \defined( 'WP_DEBUG_DISPLAY' ) && WP_DEBUG_DISPLAY ? 'true' : 'false' ) ),
						'parent' => 'det_env_type',
					)
				);
				...
				$admin_bar->add_node(
					array(
						...
						'title'  => self::show_label_value( 'SCRIPT_DEBUG', ( \defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? 'true' : 'false' ) ),
						'parent' => 'det_env_type',
					)
				);
		...
		private static function show_label_value( $label, $value ): string {
			$html  = '';
			$html .= '<span class="ei-label">' . \esc_html( (string) $label ) . '</span>';
			$html .= '<span class="ei-value">' . \esc_html( (string) $value ) . '</span>';
		...
		public static function enqueue_block_editor_assets() {
			...
			\wp_enqueue_script(
				'advan-det-block-editor',
				ADVAN_PLUGIN_ROOT_URL . 'js/block-editor.js',
				array(),
				ADVAN_VERSION,
				true
			);
			...
			\wp_enqueue_style(
				'advan-det-block-editor-styles',
				ADVAN_PLUGIN_ROOT_URL . 'css/block-editor.css',
				array(),
				ADVAN_VERSION
			);
			...
			\wp_localize_script(
				...
				array(
					...
					'wpDebugLog'  => ( \defined( 'WP_DEBUG_LOG' ) && WP_DEBUG_LOG ? 'true' : 'false' ),
					'wpDebugDisplay' => ( \defined( 'WP_DEBUG_DISPLAY' ) && WP_DEBUG_DISPLAY ? 'true' : 'false' ),

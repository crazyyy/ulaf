...
if ( ! class_exists( '\ADVAN\Controllers\Mail_SMTP_Settings' ) ) {
	...
	class Mail_SMTP_Settings {
		...
		public static function deliver_email_via_smtp( $phpmailer ) {
			// Fetch and minimally validate settings before applying to PHPMailer.
			$smtp_host = trim( (string) Settings::get_option( 'smtp_host' ) );
			// Ensure port is an integer in the valid range; default to 587 if invalid.
			$smtp_port_raw = (string) Settings::get_option( 'smtp_port' );
			$smtp_port     = (int) \absint( $smtp_port_raw );
			if ( $smtp_port < 1 || $smtp_port > 65535 ) {
				$smtp_port = 587;
			}

			// Normalize encryption value; map 'none' to empty string.
			$smtp_security_raw = strtolower( trim( (string) Settings::get_option( 'encryption_type' ) ) );
			$smtp_security     = ( 'none' === $smtp_security_raw ) ? '' : $smtp_security_raw;

			$smtp_username                = trim( (string) Settings::get_option( 'smtp_username' ) );
			$smtp_password                = (string) Settings::get_option( 'smtp_password' );
			$smtp_authentication          = ( '' !== $smtp_username && '' !== $smtp_password );
			$smtp_bypass_ssl_verification = (bool) Settings::get_option( 'smtp_bypass_ssl_verification' );
			...
			// Validate/sanitize sender fields.
			$from_email = \sanitize_email( (string) Settings::get_option( 'from_email' ) );
			if ( ! empty( $from_email ) && \is_email( $from_email ) ) {
				$phpmailer->From = $from_email;
				$phpmailer->Sender = $from_email;
			}
			$from_email_name = \sanitize_text_field( (string) Settings::get_option( 'from_email_name' ) );
			...
			if ( ! empty( $smtp_host ) && ! empty( $smtp_port ) ) {
				...
				// Validate SMTP security value before assigning.
				$allowed_smtp_security = array( 'ssl', 'tls', '' );
				if ( in_array( $smtp_security, $allowed_smtp_security, true ) ) {

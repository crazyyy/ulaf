...
use ADVAN\Helpers\System_Analytics;
...
if ( ! class_exists( '\ADVAN\Advanced_Analytics' ) ) {
	 ...
	 * Main plugin class.
	 *
	 * Responsible for initializing and managing all core modules of the Advanced Analytics plugin,
	 * including logs, requests, tables, transients, fatals, WP mail, cron jobs, pointers, settings,
	 * environment type display, migration, AJAX handling, and admin UI enhancements.
	 *
	 * Handles plugin activation, redirection, admin notices, screen options, and integration with
	...
	class Advanced_Analytics {
		...
		public static function init() {
			if ( \is_admin() && ! \wp_doing_ajax() ) {
				...
				// Initialize hooks capture list admin features.
				Hooks_Capture_List::init_admin_hooks();
				...
				\add_filter( 'set-screen-option', array( Fatals_List::class, 'set_screen_option' ), 10, 3 );

				\add_filter( 'set-screen-option', array( Snippets_List::class, 'set_screen_option' ), 10, 3 );

				\add_filter( 'set-screen-option', array( Hooks_Capture_List::class, 'set_screen_option' ), 10, 3 );

				\add_filter( 'set-screen-option', array( Hooks_Management_List::class, 'set_screen_option' ), 10, 3 );

				\add_filter( 'plugin_action_links', array( __CLASS__, 'add_settings_link' ), 10, 2 );
				\add_filter( 'plugin_row_meta', array( __CLASS__, 'plugin_meta' ), 10, 2 );

				// Placeholder: Review_Plugin::init() removed intentionally.
				...
				\add_filter( 'update_footer', array( Miscellaneous::class, 'admin_footer_version_text' ), PHP_INT_MAX );
		...
		/**
		 * Add Settings link to plugin list (in plugin.php page)
		 *
		...
		public static function add_settings_link( $links, $file ) {
			if ( ADVAN_PLUGIN_BASENAME === $file ) {
				...
				if ( ( Settings::get_option( 'wp_mail_module_enabled' ) ) ) {
					$settings_link = '<a href="' . \esc_url( Miscellaneous::get_wp_mail_page_link() ) . '">' . \esc_html__( 'Mails', '0-day-analytics' ) . '</a>';
					array_unshift( $links, $settings_link );
				}
				if ( ( Settings::get_option( 'snippets_module_enabled' ) ) ) {
					$settings_link = '<a href="' . \esc_url( Miscellaneous::get_snippets_page_link() ) . '\">' . \esc_html__( 'Snippets', '0-day-analytics' ) . '</a>';
					array_unshift( $links, $settings_link );
				}
				if ( ( Settings::get_option( 'requests_module_enabled' ) ) ) {
					$settings_link = '<a href="' . \esc_url( Miscellaneous::get_requests_page_link() ) . '">' . \esc_html__( 'Requests', '0-day-analytics' ) . '</a>';
					array_unshift( $links, $settings_link );
				}
				if ( ( Settings::get_option( 'tables_module_enabled' ) ) ) {
					$settings_link = '<a href="' . \esc_url( Miscellaneous::get_tables_page_link() ) . '">' . \esc_html__( 'Tables', '0-day-analytics' ) . '</a>';
					array_unshift( $links, $settings_link );
				}
				if ( ( Settings::get_option( 'transients_module_enabled' ) ) ) {
					$settings_link = '<a href="' . \esc_url( Miscellaneous::get_transients_page_link() ) . '">' . \esc_html__( 'Transients', '0-day-analytics' ) . '</a>';
					array_unshift( $links, $settings_link );
				}
				if ( ( Settings::get_option( 'cron_module_enabled' ) ) ) {
					$settings_link = '<a href="' . \esc_url( Miscellaneous::get_crons_page_link() ) . '">' . \esc_html__( 'Cron Jobs', '0-day-analytics' ) . '</a>';
					array_unshift( $links, $settings_link );
				}
				if ( ( Settings::get_option( 'file_editor_module_enabled' ) ) ) {
					$settings_link = '<a href="' . \esc_url( Miscellaneous::get_file_editor_page_link() ) . '">' . \esc_html__( 'File Editor', '0-day-analytics' ) . '</a>';
					array_unshift( $links, $settings_link );
				}
				if ( ( Settings::get_option( 'fatals_module_enabled' ) ) ) {
					$settings_link = '<a href="' . \esc_url( Miscellaneous::get_fatals_page_link() ) . '">' . \esc_html__( 'PHP Errors View', '0-day-analytics' ) . '</a>';
					array_unshift( $links, $settings_link );
				}
				$settings_link = '<a href="' . \esc_url( Miscellaneous::get_settings_page_link() ) . '">' . \esc_html__( 'Error Logs', '0-day-analytics' ) . '</a>';
				array_unshift( $links, $settings_link );
			}
		...
		public static function plugin_meta( $links, $file ) {
			...
			if ( false !== strpos( $file, 'advanced-analytics.php' ) ) {
				$links = array_merge(
					$links,
					array(
						'<a target="_blank" rel="noopener noreferrer" href="' . \esc_url( 'https://wordpress.org/support/plugin/0-day-analytics' ) . '">' . \esc_html__( 'Support', '0-day-analytics' ) . '</a>',
					)
				);
			}
		...
		/**
		 * Remove all non plugin notices from our plugin pages.
		 *
		 * @since 1.8.4
		 */
		...
		/**
		 * Remove all notices from our plugin pages based on the provided action hook.
		 *
		 * @param string $action The name of the action hook. Expected values: 'user_admin_notices', 'admin_notices', 'all_admin_notices', 'network_admin_notices'.
		 *
		 * @since 1.8.4
		 */
		public static function remove_unrelated_actions( $action ) {
			...
			if ( ! isset( $wp_filter[ $action ] ) || empty( $wp_filter[ $action ]->callbacks ) || ! is_array( $wp_filter[ $action ]->callbacks ) ) {
				return;
			}
		...
		public static function plugin_redirect() {
			if ( \get_option( self::REDIRECT_OPTION_NAME, false ) ) {
				...
				if ( ! isset( $_GET['activate-multi'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Redirect only, no action taken.

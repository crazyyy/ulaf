...
$window.on('load', function () {
    ...
    jQuery('.widgets-chooser-sidebars').find('li').each(function () {
        ...
        if (/^Page #\d+ - Section #\d+$/.test(thistext)) {
            $thisElem.addClass('aadvana-chooser-section').hide();
        }
    });
    ...
    if (selected_item == 'template-authors.php') {
        jQuery('#aadvana-page-template-authors').show();
    }

    jQuery($pageTemplateAttr).change(function () {
        var selected_item = jQuery($pageTemplateAttr).val();
        jQuery('.aadvana-page-templates-options').hide();
        if (selected_item == 'template-authors.php') {
            jQuery('#aadvana-page-template-authors').show();
        }
    });
...
function attachAllDynamicSelects() {
    ...
    jQuery('.aadvana-custom-posts-selector select').select2();
    jQuery('.aadvana-custom-posts-selector select').select2({
        width: 'resolve',
        ajax: {
            ...
            data: function (params) {
                ...
                return {
                    ...
                    block: activeBlock,
                    action: 'aadvana_get_posts' // AJAX action for admin-ajax.php
                };
            },
    ...
    jQuery('.aadvana-custom-posts-selector select').each(function (index) {
...
function attachAllDynamicPostTypes() {
    jQuery('.aadvana-custom-post-selector input:not([type=hidden])').autocomplete({
...
$doc.ready(function () {
	...
	jQuery('#awefpanel-darkskin').change(function(){
		...
		switchTo = $html.hasClass('aadvana-darkskin') ? 'light' : 'dark';

		$html.toggleClass('aadvana-darkskin');

		if( 'undefined' != typeof localStorage ){
			localStorage.setItem( 'aadvana-backend-skin', switchTo );
		}
	});
    ...
    jQuery('.aadvana-toggle-option').each(function () {
        var $thisElement = jQuery(this),
            elementType = $thisElement.attr('type'),
            toggleItems = $thisElement.data('aadvana-toggle');
    ...
    ------------------------------------------------------------------------------------------ */
    jQuery('#aadvana-reset-settings').click(function () {
    ...
    if (jQuery('.widgets-php div[id*="figaropost-"]').length) {
        jQuery('#aadvana-show-sections-sidebars-wrap').show();

        jQuery('.widgets-php div[id*="figaropost-"]').parent().addClass('aadvana-sidebar-section');
        jQuery('#aadvana-show-sections-sidebars').change(function () {
            jQuery('.aadvana-sidebar-section, .aadvana-chooser-section').toggle();
        });
    }

    /* OPTIONS SEARCH
    ------------------------------------------------------------------------------------------ */
    $searchSettings = jQuery('#aadvana-panel-search'),
    $searchList0 = jQuery('#aadvana-search-list');
    ...
    $searchSettings.on('keyup', function () {
        ...
        if (valThis == '') {
            ...
            jQuery('.aadvana-label').each(function () {
                var $thisElem = jQuery(this),
                    thistext = $thisElem.text();
                ...
                if (thistext.toLowerCase().indexOf(valThis) >= 0) {
                    ...
                    $searchList0.append('<li><a href="#" data-section="' + thistextid + '" data-url="' + thistextparentid + '"><strong>' + thistextparent + '</strong> / ' + thistext + '</a></li>');
                }
                else {
    ...
    $searchList0.on('click', 'a', function () {
        var $thisElem = jQuery(this),
            tabId = $thisElem.data('url'),
            tabsection = $thisElem.data('section');

        jQuery('.aadvana-panel-tabs ul li').removeClass('active');
        jQuery('.aadvana-panel-tabs').find('.' + tabId).addClass('active');
    ...
    $doc.mouseup(function (e) {
        var container = jQuery('#aadvana-options-search-wrap');
    ...
    $doc.on('click', '#aadvana-builder-wrapper .aadvana-options a', function () {
    ...
    var figaroBlocksColorsOptions = {
        change: function (event, ui) {
            ...
            jQuery(this).closest('.block-item').find('.aadvana-block-head').attr('style', 'background-color: ' + newColor).removeClass('block-head-light block-head-dark').addClass('block-head-' + getContrastColor(newColor));
        },
        clear: function () {
            jQuery(this).closest('.block-item').find('.aadvana-block-head').attr('style', '').removeClass('block-head-light block-head-dark');
        }
    };
    ...
    var figaroTitleColorsOptions = {
        change: function (event, ui) {
            var newColor = ui.color.toString();
            jQuery(this).closest('.block-item').find('.aadvana-block-head .block-preview-title').attr('style', 'color: ' + newColor).removeClass('block-head-light block-head-dark').addClass('block-head-' + getContrastColor(newColor));
        },
        clear: function () {
            jQuery(this).closest('.block-item').find('.aadvana-block-head .block-preview-title').attr('style', '').removeClass('block-head-light block-head-dark');
        }
    };
    ...
    $doc.on('click', '.toggle-section', function () {
        var $thisElement = jQuery(this).closest('.aadvana-builder-container');
        $thisElement.find('.aadvana-builder-section-inner').slideToggle('fast');
        $thisElement.toggleClass('aadvana-section-open');
        return false;
    });
    ...
    $doc.on('click', '.edit-block-icon', function () {
        ...
        $figaroBody.addClass('has-overlay');
        $thisElement.find('> .aadvana-builder-content-area').fadeIn('fast');

        // Disable the Sortable and Draggable if the PopUp is open
        jQuery('#aadvana-builder-wrapper, .aadvana-builder-blocks-wrapper').sortable('disable');
    ...
    $doc.on('click', '.blocks-settings-tabs a', function () {
        var $thisButtonTab = jQuery(this),
            $blockContent = $thisButtonTab.closest('.aadvana-builder-content-area'),
    ...
    $doc.on('click', '#aadvana_featured_posts_style a', function () {
    ...
    aadvana_builder_dragdrop();


    /* IMAGE UPLOADER PREVIEW */
    jQuery('.aadvana-img-path').each(function () {
        aadvana_image_uploader_trigger(jQuery(this));
    });


    /* Font Uploader */
    jQuery('.aadvana-font-path').each(function () {
        aadvana_set_font_uploader(jQuery(this));
    });


    /* CHECKBOXES */
    var checkInputs = Array.prototype.slice.call(document.querySelectorAll('.aadvana-js-switch'));
    ...
    jQuery('.tab-sortable').each(function () {
        aadvana_sortable_tabs_trigger(jQuery(this));
    });

    /* Widget Posts order option  */
    jQuery('.aadvana-posts-order-option').each(function () {
        aadvana_widget_posts_order(jQuery(this));
    });
    ...
    $doc.on('widget-added', function (event, widgetContainer) {
        ...
        aadvana_sortable_tabs_trigger($thisTabs);

        // ------
        var $thisOption = widgetContainer.find('.aadvana-posts-order-option');
        aadvana_widget_posts_order($thisOption);
    });
    ...
    /* DISMISS NOTICES
    ------------------------------------------------------------------------------------------ */
    $doc.on('click', '.aadvana-notice .notice-dismiss', function () {

        jQuery('#aadvana-page-overlay').hide();

        jQuery.ajax({
            url: ajaxurl,
            type: 'post',
            data: {
                pointer: jQuery(this).closest('.aadvana-notice').attr('id'),
    ...
    jQuery('#aadvana_form').on( 'submit' , function () {
        ...
        jQuery('form#aadvana_form input, form#aadvana_form textarea, form#aadvana_form select').each(function () {
            if (!jQuery(this).val()) {
        ...
        jQuery('form#aadvana_form input:disabled, form#aadvana_form textarea:disabled, form#aadvana_form select:disabled').attr('disabled', false);
    ...
    /* SETTINGS PANEL
    ------------------------------------------------------------------------------------------ */
    jQuery('.aadvana-panel, .aadvana-notice').css({ 'opacity': 1, 'visibility': 'visible' });

    var tabsHeight = jQuery('.aadvana-panel-tabs').outerHeight();
    jQuery('.tabs-wrap').hide();
    jQuery('.aadvana-panel-tabs ul li.aadvana-tabs:first').addClass('active').show();
    jQuery('.tabs-wrap:first').show();
    jQuery('.aadvana-panel-content').css({ minHeight: tabsHeight });

    jQuery('li.aadvana-tabs:not(.aadvana-not-tab)').click(function () {
        jQuery('.aadvana-panel-tabs ul li').removeClass('active');
    ...
    if (jQuery(currentTab).parent('#aadvana_form').length) {
    ...
    $doc.on('click', '.del-item', function () {
        ...
        if ($thisButton.hasClass('del-section')) {
            var widgets = $thisButton.closest('.parent-item').find('.aadvana-manage-widgets').data('widgets');
    ...
    $doc.on('click', '.del-img', function () {
        ...
        $img.fadeOut('fast', function () {
            $img.hide();
            $img.closest('.option-item').find('.aadvana-img-path').attr('value', '');
        });
    });
        ...
        if (customtext.length > 0) {
            ...
            jQuery('#customList').append('\
				<li class="parent-item">\
					<div class="aadvana-block-head">\
						'+ customtext + '\
						<input name="_aadvana_hlights_text['+ customnext + ']" type="hidden" value="' + customtext + '" />\
        ...
        if (source_name.length > 0) {
            ...
            if (source_link.length > 0) {
                source_code += '\
							...
							<input name="_aadvana_post_source['+ source_next + '][url]" type="hidden" value="' + source_link + '" />\
						';
            }
            else {
            ...
            source_code += '\
						<input name="_aadvana_post_source['+ source_next + '][text]" type="hidden" value="' + source_name + '" />\
						<a class="del-item dashicons dashicons-trash"></a>\
					</div>\
				</li>\
			';
        ...
        if (via_name.length > 0) {
            ...
            if (via_link.length > 0) {
                via_code += '\
							...
							<input name="_aadvana_post_via['+ via_next + '][url]" type="hidden" value="' + via_link + '" />\
						';
            }
            else {
                via_code += via_name;
            ...
            via_code += '\
						<input name="_aadvana_post_via['+ via_next + '][text]" type="hidden" value="' + via_name + '" />\
						<a class="del-item dashicons dashicons-trash"></a>\
					</div>\
				</li>\
			';
    ...
    /* VISUAL OPTIONS
    ------------------------------------------------------------------------------------------ */
    jQuery('ul.aadvana-options').each(function (index) {
        jQuery(this).find('input:checked').parent().addClass('selected');
    });

    $doc.on('click', 'ul.aadvana-options a', function () {
        var $thisBlock = jQuery(this),
            blockID = $thisBlock.closest('ul.aadvana-options').attr('id');
    ...
    jQuery('.predefined-skins-options select').change(function () {
        var skin = jQuery(this).val(),
            skin_colors = aadvana_skins[skin];

        jQuery('#aadvana-options-tab-styling').find('.figaroColorSelector').val('');
        jQuery('#aadvana-options-tab-styling').find('.wp-color-result').attr('style', '');
...
------------------------------------------------------------------------------------------ */
function aadvana_sortable_tabs_trigger($thisTabs) {
...
/* IMAGE UPLOADER PREVIEW
------------------------------------------------------------------------------------------ */
function aadvana_image_uploader_trigger($thisElement) {
    ...
    if ($thisElement.hasClass('aadvana-background-path')) {
        thisElementID = thisElementID.replace('-img', '');
        uploaderTypeStyles = true;
    }

    aadvana_set_uploader(thisElementID, uploaderTypeStyles);
}
...
------------------------------------------------------------------------------------------ */
function aadvana_builder_dragdrop() {

    jQuery('#aadvana-builder-wrapper').sortable({
        placeholder: 'aadvana-state-highlight aadvana-state-sections',
        activate: function (event, ui) {

            var $sortableWrap = ui.item,
                outerHeight = $sortableWrap.outerHeight() + 40;

            jQuery('.aadvana-state-sections').css('min-height', outerHeight);
        },
    });

    jQuery('.tabs_cats').sortable({ placeholder: 'aadvana-state-highlight' });
    ...
    jQuery('.block-item').draggable({
        ...
        cursor: 'move',
        zIndex: 100,
        connectToSortable: '.aadvana-builder-blocks-wrapper',
        revert: true,
        revertDuration: 0,
    ...
    jQuery('.aadvana-builder-blocks-wrapper').sortable({
        placeholder: 'aadvana-state-highlight',
        items: '> .block-item',
        cursor: 'move',
        distance: 2,
        ...
        receive: function (event, ui) {
            ...
            ui.item.find('[name^=aadvana_home_cats]').each(function () {
                var newName = jQuery(this).attr('name').replace(/aadvana_home_cats\[(\w+)\]/g, 'aadvana_home_cats\[' + sectionID + ']');
                jQuery(this).attr('name', newName);
            });
        },

        activate: function (event, ui) {
            jQuery('.aadvana-builder-blocks-wrapper').css('min-height', '65px');
            var $sortableWrap = ui.item.closest('.aadvana-builder-blocks-wrapper'),
                outerHeight = ($sortableWrap.outerHeight() > 0) ? $sortableWrap.outerHeight() + 40 : '65px';

            $sortableWrap.css('min-height', outerHeight);
            jQuery('.aadvana-builder-container').addClass('aadvana-block-hover');
        },

        deactivate: function () {
            jQuery('.aadvana-builder-container').removeClass('aadvana-block-hover');
            jQuery('.aadvana-builder-blocks-wrapper').css('min-height', '');
        },
    }).sortable('option', 'connectWith', '.aadvana-builder-container');
...
/* IMAGE UPLOADER FUNCTIONS
------------------------------------------------------------------------------------------ */
function aadvana_set_uploader(field, styling) {
    var aadvana_bg_uploader;

    $doc.on('click', '#upload_' + field + '_button', function (event) {

        event.preventDefault();
        aadvana_bg_uploader = wp.media.frames.aadvana_bg_uploader = wp.media({
            title: 'Choose Image',
            library: { type: 'image' },
            button: { text: 'Select' },
            multiple: false
        ...
        aadvana_bg_uploader.on('select', function () {
            var selection = aadvana_bg_uploader.state().get('selection');
...
function aadvana_color_picker() {
    ...
    jQuery('.figaroColorSelector').each(function () {
        ...
        jQuery('<div class="aadvana-alpha-container"><div class="slider-alpha"></div><div class="transparency"></div></div>').appendTo($control.parents('.wp-picker-container'));

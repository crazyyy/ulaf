<?php

namespace Rollbar\WordPress\Html\Input;

use Rollbar\WordPress\Html\Template;
// Exit if accessed directly
// phpcs:disable PSR1.Files.SideEffects.FoundWithSymbols
\defined('ABSPATH') || exit;
// phpcs:enable
/**
 * Select Input Class
 *
 * Renders a select dropdown input for a setting.
 *
 * @since 3.0.0
 * @internal
 */
class SelectInput extends \Rollbar\WordPress\Html\Input\AbstractInput
{
    /**
     * Constructor for the SelectInput class.
     *
     * @param string $id The input ID.
     * @param string|array $name The input name.
     * @param mixed $value The input value.
     * @param mixed $default The default value.
     * @param string $label The label for the input.
     * @param string $helpText The help text for the input.
     * @param bool $disabled Whether the input is disabled.
     * @param bool $showReset Whether to show the reset button.
     * @param array<string, string> $attributes Additional attributes for the input.
     * @param array<string, string> $options The value, label pairs of possible options.
     */
    public function __construct(protected string $id, protected string|array $name, protected $value = null, protected $default = null, protected string $label = '', protected string $helpText = '', protected bool $disabled = \false, protected bool $showReset = \true, protected array $attributes = [], protected array $options = [])
    {
        parent::__construct($id, $name, $value, $default, $label, $helpText, $disabled, $showReset, $attributes);
    }
    /**
     * Renders the input as a string.
     *
     * @return string
     */
    public function __toString() : string
    {
        return Template::string(ROLLBAR_PLUGIN_DIR . '/templates/html/input/selectInput.php', ['input' => $this]);
    }
    /**
     * Returns the value, label pairs of possible options.
     *
     * @return array<string, string>
     */
    public function getOptions() : array
    {
        return $this->options;
    }
    /**
     * Returns the type of the input.
     *
     * @return string
     */
    public function getType() : string
    {
        return 'select';
    }
}

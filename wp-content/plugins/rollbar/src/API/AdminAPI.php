...
class AdminAPI extends AbstractSingleton
{
    ...
    {
        register_rest_route(route_namespace: self::API_NAMESPACE, route: '/test-php-logging', args: ['methods' => WP_REST_Server::CREATABLE, 'callback' => $this->handleTestPhpLogging(...), 'permission_callback' => function (WP_REST_Request $request) : bool {
            /**
             ...
             *
             */
            return apply_filters(hook_name: 'rollbar_api_admin_permission', value: is_user_logged_in() && current_user_can('manage_options'), route: 'test-php-logging', request: $request);
        }, 'args' => ['server_side_access_token' => ['required' => \true, 'sanitize_callback' => 'sanitize_text_field'], 'environment' => ['required' => \true, 'sanitize_callback' => 'sanitize_text_field'], 'included_errno' => ['required' => \true, 'sanitize_callback' => 'sanitize_text_field']]]);
    }
    /**
    ...
    public function handleTestPhpLogging(WP_REST_Request $request) : WP_REST_Response
    {
        ...
        try {
            $plugin->initPhpLogging(ignoreEnabledSetting: \true);
            $rollbarResponse = Rollbar::report(Level::INFO, 'Test message from Rollbar WordPress plugin using PHP: ' . 'integration with WordPress successful');
        ...
        $response = ['code' => $rollbarResponse->getStatus(), 'success' => $rollbarResponse->getStatus() === 200, 'message' => ''];

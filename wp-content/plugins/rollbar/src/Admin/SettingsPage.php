...
final class SettingsPage extends AbstractSingleton
{
    ...
    {
        add_submenu_page(parent_slug: 'options-general.php', page_title: 'Rollbar', menu_title: 'Rollbar', capability: 'manage_options', menu_slug: 'rollbar_wp', callback: $this->optionsPage(...));
    }
    /**
    ...
    public function addSettings() : void
    {
        ...
        register_setting(option_group: 'rollbar_wp', option_name: 'rollbar_wp');
        // SECTION: General
        add_settings_section(id: 'rollbar_wp_general', title: \false, callback: \false, page: 'rollbar_wp');
        // On/Off and Tokens
        add_settings_field(id: 'rollbar_wp_status', title: 'Status', callback: function () {
            Template::print(ROLLBAR_PLUGIN_DIR . '/templates/admin/settings-status.php', ['php_logging_enabled' => !empty($this->plugin->getSetting('php_logging_enabled')) ? 1 : 0, 'server_side_access_token' => $this->plugin->getSetting('server_side_access_token'), 'js_logging_enabled' => !empty($this->plugin->getSetting('js_logging_enabled')) ? 1 : 0, 'client_side_access_token' => $this->plugin->getSetting('client_side_access_token')]);
        }, page: 'rollbar_wp', section: 'rollbar_wp_general');
        ...
        add_settings_section(
            ...
            title: \false,
            // The title is created by the callback.
    ...
    public function enqueueAdminScripts(string $hook) : void
    {
        ...
        wp_enqueue_style('rollbar-admin-css', plugin_dir_url(ROLLBAR_PLUGIN_FILE) . '/public/admin/rollbar.css', \false, Plugin::VERSION);
        wp_enqueue_script('rollbar-admin-js', Plugin::getAssetUrl('/public/admin/rollbar.js'), [], Plugin::VERSION);
        wp_localize_script(handle: 'rollbar-admin-js', object_name: 'rollbarSettings', l10n: ['nonce' => wp_create_nonce('rollbar_wp_api_test_logging'), 'rest_nonce' => wp_create_nonce('wp_rest'), 'rest_root' => esc_url_raw(rest_url()), 'plugin_url' => plugin_dir_url(ROLLBAR_PLUGIN_FILE)]);
    }
    /**
    ...
    {
        add_settings_field(id: 'rollbar_wp_' . $setting->id, title: $setting->getLabelElement(), callback: $setting->render(...), page: 'rollbar_wp', section: $setting->section, args: $args);
    }
    /**
    ...
    {
        Template::print(ROLLBAR_PLUGIN_DIR . '/templates/admin/settings-section-header.php', ['id' => 'rollbar_settings_advanced_header', 'title' => 'Advanced Settings', 'description' => '<p>See the <a href="https://docs.rollbar.com/docs/php-configuration-reference" ' . 'target="_blank">configuration documentation</a> for more details on each of these settings.</p>']);
    }
    /**
     * Renders the header for the "Advanced" section of the settings page.
     *
    ...
    {
        Template::print(ROLLBAR_PLUGIN_DIR . '/templates/admin/settings-section-header.php', ['id' => 'rollbar_settings_telemetry_header', 'title' => 'Telemetry Settings', 'description' => '<p>See the <a href="https://docs.rollbar.com/docs/php-telemetry" target="_blank">Rollbar ' . 'telemetry documentation</a> and <a href="https://developer.wordpress.org/apis/hooks/action-reference/" ' . 'target="_blank">WordPress action reference</a> for more details on these settings.</p>']);
    }
    /**
    ...
    public static function restoreDefaultsAction() : void
    {
        ...
        \Rollbar\WordPress\Admin\FlashMessages::addMessage(message: 'Default Rollbar settings restored.');

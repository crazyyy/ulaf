...
class Listener extends AbstractSingleton
{
    ...
    protected function postInit() : void
    {
        ...
        foreach ($actions as $action => $acceptedArgs) {
            ...
            $this->instrumentAction(action: $action, acceptedArgs: $acceptedArgs, argsHandler: $handler);
        }
    }
    /**
    ...
    protected function getCustomHandlers() : array
    {
         ...
         * @since 3.0.0
         */
        return apply_filters('rollbar_telemetry_custom_handlers', ['after_setup_theme' => self::handlerAfterSetupTheme(...), 'plugins_loaded' => self::handlerPluginsLoaded(...)]);
    }
    /**
     ...
     */
    public function instrumentAction(string $action, int $priority = 10, int $acceptedArgs = 1, callable $argsHandler = null) : void
    {
        if (null === $argsHandler && $acceptedArgs > 1) {
            throw new InvalidArgumentException('If acceptedArgs is greater than 1, argsHandler must be provided.');
        }
        \add_action(hook_name: $action, callback: function (...$args) use($action, $argsHandler) {
            $message = $action;
            if (null !== $argsHandler) {
                $message = $argsHandler($action, ...$args);
            }
            $this->log('Action triggered: ' . $message);
        }, priority: $priority, accepted_args: $acceptedArgs);
    }
    /**

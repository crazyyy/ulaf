...
final class Plugin extends AbstractSingleton
{
     ...
     * @var Settings|null
     */
    private \Rollbar\WordPress\Settings|null $settings;
    /**
    ...
    {
        $this->settings = \Rollbar\WordPress\Settings::getInstance();
        $this->config = [];
        $this->hooks();
        $this->initPhpLogging();
    }
    ...
    public static function userCanViewAdmin() : bool
    {
         ...
         * @since 3.0.0
         */
        return apply_filters('rollbar_user_can_view_admin', current_user_can('manage_options')) === \true;
    }
    /**
     * @param string $path
     * @return string
     ...
     */
    public function settingsInstance() : \Rollbar\WordPress\Settings
    {
    ...
    {
        $levels = [\E_ERROR, \E_WARNING, \E_PARSE, \E_NOTICE, \E_CORE_ERROR, \E_CORE_WARNING, \E_COMPILE_ERROR, \E_COMPILE_WARNING, \E_USER_ERROR, \E_USER_WARNING, \E_USER_NOTICE, \E_STRICT, \E_RECOVERABLE_ERROR, \E_DEPRECATED, \E_USER_DEPRECATED, \E_ALL];
        $included_errno = 0;
    ...
    public function initPhpLogging(bool $ignoreEnabledSetting = \false) : void
    {
        ...
        try {
            ...
            FlashMessages::addMessage(message: 'Rollbar is misconfigured. Please, fix your configuration here: <a href="' . \admin_url('/options-general.php?page=rollbar_wp') . '">Rollbar Settings</a>.', type: 'error');
        }
    }
    /**
    ...
    public function buildPHPConfig() : array
    {
        ...
        if ($this->getSetting('enable_telemetry_listener')) {
            $config['telemetry'] = ['includeItemsInTelemetry' => $this->getSetting('include_items_in_telemetry'), 'includeIgnoredItemsInTelemetry' => $this->getSetting('include_ignored_items_in_telemetry')];
        }
        foreach ($config as $setting => $value) {
            if (SettingType::Boolean !== \Rollbar\WordPress\Settings::getSettingType($setting)) {
                continue;
            }
            $config[$setting] = \Rollbar\WordPress\Settings::toBoolean($value);
        }
        if ($config['enable_person_reporting'] && empty($config['person_fn']) && empty($config['person'])) {
            $config['person_fn'] = \Rollbar\WordPress\Settings::getPersonFunction(...);
        }
    ...
    {
        $config = ['accessToken' => $this->getSetting('client_side_access_token'), 'captureUncaught' => \true, 'payload' => ['environment' => $this->getSetting('environment')]];
        /**

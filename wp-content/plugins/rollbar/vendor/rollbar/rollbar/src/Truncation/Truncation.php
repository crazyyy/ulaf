...
class Truncation
{
     ...
     */
    const MAX_PAYLOAD_SIZE = 131072;
    // 128 * 1024
    /**
     * @var string[] The list of truncation strategies to apply in order.
     */
    protected static array $truncationStrategies = array(\Rollbar\Truncation\FramesStrategy::class, \Rollbar\Truncation\StringsStrategy::class);
    /**
    ...
    public function registerStrategy(string $type) : void
    {
        ...
        if (!\in_array(\Rollbar\Truncation\StrategyInterface::class, \class_implements($type))) {
            throw new Exception('Truncation strategy "' . $type . '" doesn\'t implement ' . \Rollbar\Truncation\StrategyInterface::class);
        }

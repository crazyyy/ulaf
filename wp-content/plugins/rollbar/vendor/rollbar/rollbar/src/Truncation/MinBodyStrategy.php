 ...
 * @internal
 */
class MinBodyStrategy extends \Rollbar\Truncation\AbstractStrategy
{
    /**
    ...
    public function execute(EncodedPayload $payload) : EncodedPayload
    {
        ...
        if (isset($traceData['exception'])) {
            ...
            $traceData['exception']['message'] = \substr($traceData['exception']['message'], 0, static::EXCEPTION_MESSAGE_LIMIT);
            $modified = \true;
        }
        // Limit trace frames
        if (!empty($traceData['frames'])) {
            $traceData['frames'] = \Rollbar\Truncation\FramesStrategy::selectFrames($traceData['frames'], static::EXCEPTION_FRAMES_RANGE);
            $modified = \true;
        }
        if ($modified) {
            $payload->encode($data);

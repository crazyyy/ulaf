 ...
 * @internal
 */
class FramesStrategy extends \Rollbar\Truncation\AbstractStrategy
{
    /**
    ...
    public static function selectFrames(array $frames, int $range = self::FRAMES_OPTIMIZATION_RANGE) : array
    {
        ...
        return \array_merge(\array_splice($frames, 0, $range), \array_splice($frames, -$range, $range));
    }
    /**
    ...
    public function applies(EncodedPayload $payload) : bool
    {
        ...
        if (isset($payload['data']['body']['trace_chain']) || isset($payload['data']['body']['trace']['frames'])) {
            return \true;
        }
        return \false;
    }

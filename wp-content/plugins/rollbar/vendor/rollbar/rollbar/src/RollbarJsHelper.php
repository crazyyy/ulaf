...
class RollbarJsHelper
{
     ...
     * @return string
     */
    public static function buildJs($config, $headers = null, $nonce = null, $customJs = "")
    {
    ...
    {
        return $this->scriptTag($this->configJsTag() . $this->jsSnippet() . ";" . $customJs, $headers, $nonce);
    }
    /**
    ...
    {
        return \file_get_contents($this->snippetPath());
    }
    /**
    ...
    {
        return $status == 200 && $this->isHtml($headers) && !$this->hasAttachment($headers);
        /**
    ...
    public function shouldAppendNonce($headers)
    {
        foreach ($headers as $header) {
            if (\str_contains($header, 'Content-Security-Policy') && \str_contains($header, "'unsafe-inline'")) {
                return \true;
            }
        }
        return \false;
    ...
    public function scriptTag($content, $headers = null, $nonce = null)
    {
        if ($headers !== null && $this->shouldAppendNonce($headers)) {
            if (!$nonce) {
                throw new \Exception('Content-Security-Policy is script-src ' . 'inline-unsafe but nonce value not provided.');
            }

...
class Config
{
    ...
    private static array $options = array('access_token', 'agent_log_location', 'allow_exec', 'endpoint', 'base_api_url', 'autodetect_branch', 'branch', 'capture_error_stacktraces', 'check_ignore', 'code_version', 'custom', 'custom_data_method', 'enabled', 'environment', 'error_sample_rates', 'exception_sample_rates', 'fluent_host', 'fluent_port', 'fluent_tag', 'handler', 'host', 'include_error_code_context', 'include_exception_code_context', 'included_errno', 'log_payload', 'log_payload_logger', 'person', 'person_fn', 'capture_ip', 'capture_username', 'capture_email', 'root', 'scrubber', 'scrub_fields', 'scrub_safelist', 'timeout', 'transmit', 'telemetry', 'custom_truncation', 'report_suppressed', 'use_error_reporting', 'proxy', 'send_message_trace', 'include_raw_request_body', 'local_vars_dump', 'max_nesting_depth', 'max_items', 'minimum_level', 'verbose', 'verbose_logger', 'raise_on_error', 'transformer');
     ...
     */
    private ?\Rollbar\TransformerInterface $transformer = null;
    /**
     * @var FilterInterface
     */
    private $filter;
    ...
    public function __construct(array $configArray)
    {
        ...
        $levels = array(\E_WARNING, \E_NOTICE, \E_USER_ERROR, \E_USER_WARNING, \E_USER_NOTICE, \E_RECOVERABLE_ERROR);
        // PHP 5.3.0
    ...
    private function setAccessToken(array $config) : void
    {
        ...
        $this->utilities()->validateString(input: $config['access_token'], name: "config['access_token']", len: [32, 96], allowNull: !$this->sender->requireAccessToken());
    ...
    {
        $this->transmit = $config['transmit'] ?? \Rollbar\Defaults::get()->transmit();
    }
    private function setLogPayload(array $config) : void
    {
        $this->logPayload = $config['log_payload'] ?? \Rollbar\Defaults::get()->logPayload();
    }
    private function setLogPayloadLogger(array $config) : void
    {
        $this->logPayloadLogger = $config['log_payload_logger'] ?? new Logger('rollbar.payload', array(new ErrorLogHandler()));
    ...
    {
        $this->verbose = $config['verbose'] ?? \Rollbar\Defaults::get()->verbose();
    }
    ...
    private function setDataBuilder(array $config) : void
    {
        ...
        $exp = \Rollbar\DataBuilderInterface::class;
        $def = \Rollbar\DataBuilder::class;
        $this->setupWithOptions($config, "dataBuilder", $exp, $def, \true);
    }
    private function setTransformer(array $config) : void
    {
        $expected = \Rollbar\TransformerInterface::class;
    ...
    private function setMinimumLevel(array $config) : void
    {
        ...
        if ($override instanceof Level) {
            ...
            $level = \Rollbar\LevelFactory::fromName($override);
            if ($level !== null) {
    ...
    {
        $this->setupWithOptions($config, "filter", \Rollbar\FilterInterface::class);
    }
    ...
    {
        $exp = \Rollbar\ScrubberInterface::class;
        $def = \Rollbar\Scrubber::class;
        $this->setupWithOptions($config, "scrubber", $exp, $def, \true);
    }
    ...
    {
        $telemetry = \key_exists('telemetry', $config) ? $config['telemetry'] : \true;
        $this->telemetry = \Rollbar\Defaults::get()->telemetry($telemetry);
    }
    ...
    public function verboseInteger() : int
    {
         ...
         * @psalm-suppress UndefinedClass
         */
        if (\is_a($level, 'RollbarWP\\Monolog\\Level')) {
            return $level->value;
        }
        return $level;
    }
    ...
    private function setAgentSenderOptions(array &$config, mixed $default) : mixed
    {
        ...
        if (\array_key_exists('agent_log_location', $config)) {
            $config['senderOptions'] = array('agentLogLocation' => $config['agent_log_location']);
        }
        return $default;
    }
    ...
    {
        $this->setupWithOptions($config, "responseHandler", \Rollbar\ResponseHandlerInterface::class);
    }
     ...
     */
    protected function setupWithOptions(array $config, string $keyName, string $expectedType, ?string $defaultClass = null, bool $passWholeConfig = \false) : void
    {
    ...
    public function getTelemetry(?Telemeter $telemeter = null) : ?Telemeter
    {
        ...
        $config = \array_intersect_key($config, ['maxTelemetryEvents' => Telemeter::MAX_EVENTS, 'filter' => null, 'includeItemsInTelemetry' => \false, 'includeIgnoredItemsInTelemetry' => \false]);
        if (null === $telemeter) {
    ...
    private function initTelemetryFilter(?string $filterClass) : ?TelemetryFilterInterface
    {
        ...
        if (!\class_exists($filterClass)) {
            throw new InvalidArgumentException("Telemetry filter class {$filterClass} does not exist");
        }
        $filter = new $filterClass($this->telemetry);
        if (!$filter instanceof TelemetryFilterInterface) {
            throw new InvalidArgumentException("Telemetry filter class {$filterClass} must implement TelemetryFilterInterface");
        }
        return $filter;
    }
    public function transform(Payload $payload, Level|string $level, mixed $toLog, array $context = array()) : Payload
    {
    ...
    public function checkIgnored(Payload $payload, $toLog, bool $isUncaught)
    {
        if (isset($this->checkIgnore)) {
            try {
                ...
                $this->verboseLogger()->error('Exception occurred in the custom checkIgnore logic:' . $exception->getMessage());
        ...
        if (!\is_null($this->filter)) {
            ...
            $this->verboseLogger()->debug("Custom filter result: " . \var_export($filter, \true));
            return $filter;
        }
        return \false;
    }
    public function internalCheckIgnored(string $level, mixed $toLog) : bool
    {
        ...
        if ($this->levelTooLow(\Rollbar\LevelFactory::fromName($level))) {
            $this->verboseLogger()->debug("Occurrence's level is too low");
            return \true;
        }
        if ($toLog instanceof \Rollbar\ErrorWrapper) {
     ...
     * @return bool
     */
    protected function shouldIgnoreErrorWrapper(\Rollbar\ErrorWrapper $toLog) : bool
    {
    ...
    public function shouldSuppress() : bool
    {
        ...
        if ($errorReporting === (\E_ERROR | \E_CORE_ERROR | \E_COMPILE_ERROR | \E_USER_ERROR | \E_RECOVERABLE_ERROR | \E_PARSE)) {
            return \true;
        }
        // PHP 7 or manually disabled case:
        return $errorReporting === 0;
    }
    public function send(EncodedPayload $payload, string $accessToken) : ?\Rollbar\Response
    {
        if ($this->transmitting()) {
            $response = $this->sender->send($payload, $accessToken);
        } else {
            $response = new \Rollbar\Response(0, "Not transmitting (transmitting disabled in configuration)");
            $this->verboseLogger()->warning($response->getInfo());
        }
        if ($this->loggingPayload()) {
            $this->logPayloadLogger()->debug('Sending payload with ' . \get_class($this->sender) . ":\n" . $payload);
        }
        return $response;
    }
    public function sendBatch(array $batch, string $accessToken) : ?\Rollbar\Response
    {
        if ($this->transmitting()) {
    ...
    public function handleResponse(Payload $payload, \Rollbar\Response $response) : void
    {
        if (!\is_null($this->responseHandler)) {
            $this->verboseLogger()->debug('Applying custom response handler: ' . \get_class($this->responseHandler));

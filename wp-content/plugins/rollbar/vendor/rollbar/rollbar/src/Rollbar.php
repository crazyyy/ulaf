...
class Rollbar
{
     ...
     */
    private static ?\Rollbar\RollbarLogger $logger = null;
    /**
     ...
     */
    public static function init(\Rollbar\RollbarLogger|array $configOrLogger, bool $handleException = \true, bool $handleError = \true, bool $handleFatal = \true) : void
    {
     ...
     * @throws Exception If the $configOrLogger argument is an array that has invalid configs.
     */
    private static function setLogger(\Rollbar\RollbarLogger|array $configOrLogger) : void
    {
        if ($configOrLogger instanceof \Rollbar\RollbarLogger) {
        ...
        self::$logger = new \Rollbar\RollbarLogger($configOrLogger);
        self::updateTelemeter();
    }
    /**
     ...
     */
    public static function logger() : ?\Rollbar\RollbarLogger
    {
        return self::$logger;
    }
    /**
     ...
     */
    public static function scope(array $config) : \Rollbar\RollbarLogger
    {
        if (\is_null(self::$logger)) {
            return new \Rollbar\RollbarLogger($config);
        }
     ...
     * @since 4.0.0
     */
    public static function report($level, string|Stringable $message, array $context = array()) : \Rollbar\Response
    {
        if (\is_null(self::$logger)) {
     ...
     * @since 3.0.0
     */
    public static function logUncaught(string|Level $level, Throwable $toLog, array $context = array()) : \Rollbar\Response
    {
        if (\is_null(self::$logger)) {
            return self::getNotInitializedResponse();
        }
        return self::$logger->report($level, $toLog, $context, isUncaught: \true);
    }
    /**
     ...
     * @since 4.1.0
     */
    public static function captureTelemetryEvent(EventType $type, EventLevel $level, array|TelemetryBody $metadata, ?string $uuid = null, ?int $timestamp = null) : ?TelemetryEvent
    {
        if (\is_null(self::$logger)) {
            return null;
        }
     ...
     * @return Response
     */
    private static function getNotInitializedResponse() : \Rollbar\Response
    {
        return new \Rollbar\Response(0, "Rollbar Not Initialized");
    }
    /**

...
class TelemetryEvent implements SerializerInterface
{
    ...
    public \Rollbar\Payload\TelemetryBody $body;
    /**
     ...
     */
    public function __construct(public EventType $type, public EventLevel $level, array|\Rollbar\Payload\TelemetryBody $body, public ?float $timestamp = null)
    {
        if (\is_null($this->timestamp)) {
            $this->timestamp = \floor(\microtime(\true) * 1000);
        }
        $this->body = \is_array($body) ? \Rollbar\Payload\TelemetryBody::fromArray($body) : $body;
    }
    #[\RollbarWP\Override]
    public function serialize() : array
    {
        $result = \array_filter(['uuid' => $this->uuid, 'source' => $this->source, 'level' => $this->level->value, 'type' => $this->type->value, 'body' => $this->body->serialize(), 'timestamp_ms' => $this->timestamp]);

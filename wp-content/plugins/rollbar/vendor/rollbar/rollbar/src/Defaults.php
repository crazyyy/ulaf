...
class Defaults
{
    public static function get()
    {
        if (\is_null(self::$singleton)) {
            self::$singleton = new \Rollbar\Defaults();
        }
        return self::$singleton;
    }
    public function __construct()
    {
        $this->psrLevels = array(LogLevel::EMERGENCY => "critical", LogLevel::ALERT => "critical", LogLevel::CRITICAL => "critical", LogLevel::ERROR => "error", LogLevel::WARNING => "warning", LogLevel::NOTICE => "info", LogLevel::INFO => "info", LogLevel::DEBUG => "debug");
        $this->errorLevels = array(\E_ERROR => "error", \E_WARNING => "warning", \E_PARSE => "critical", \E_NOTICE => "debug", \E_CORE_ERROR => "critical", \E_CORE_WARNING => "warning", \E_COMPILE_ERROR => "critical", \E_COMPILE_WARNING => "warning", \E_USER_ERROR => "error", \E_USER_WARNING => "warning", \E_USER_NOTICE => "debug", \E_RECOVERABLE_ERROR => "error", \E_DEPRECATED => "info", \E_USER_DEPRECATED => "info");
        $this->scrubFields = array('passwd', 'password', 'secret', 'confirm_password', 'password_confirmation', 'auth_token', 'csrf_token', 'access_token');
     ...
     * @since 4.1.0
     */
    private array $telemetry = ['maxTelemetryEvents' => Telemeter::MAX_EVENTS, 'filter' => null, 'includeItemsInTelemetry' => \true, 'includeIgnoredItemsInTelemetry' => \false];
}

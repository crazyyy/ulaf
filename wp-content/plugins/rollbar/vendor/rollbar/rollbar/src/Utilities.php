...
final class Utilities
{
     ...
     */
    public static function validateString(mixed $input, string $name = "?", int|array|null $len = null, bool $allowNull = \true) : void
    {
        if (\is_null($input)) {
            if (!$allowNull) {
    ...
    public static function validateBoolean($input, $name = "?", $allowNull = \true)
    {
        if (\is_null($input)) {
            if (!$allowNull) {
                throw new \InvalidArgumentException("\${$name} must not be null");
    ...
    public static function validateInteger($input, $name = "?", $minValue = null, $maxValue = null, $allowNull = \true)
    {
        if (\is_null($input)) {
            if (!$allowNull) {
                throw new \InvalidArgumentException("\${$name} must not be null");
    ...
    public static function serializeForRollbar($obj, ?array $customKeys = null, &$objectHashes = array(), $maxDepth = -1, $depth = 0)
    {
        $returnVal = array();
        if (\is_object($obj)) {
        ...
        foreach ($obj as $key => $val) {
            try {
                if (\is_object($val)) {
                    $val = self::serializeObject($val, $customKeys, $objectHashes, $maxDepth, $depth);
                } elseif (\is_array($val)) {
    ...
    public static function serializeToArray(mixed $obj) : array
    {
        ...
        if (\is_object($obj)) {
            return array('type' => 'object', 'class' => \get_class($obj));
        }
        return array('type' => \gettype($obj));
    }
    private static function serializeObject($obj, ?array $customKeys = null, &$objectHashes = array(), $maxDepth = -1, $depth = 0)
    {
        if (self::serializedAlready($obj, $objectHashes)) {
            return self::circularReferenceLabel($obj);
        }
        // Internal Rollbar classes.
        if ($obj instanceof \Rollbar\SerializerInterface) {
            self::markSerialized($obj, $objectHashes);
        ...
        $serialized = array('class' => \get_class($obj));
        ...
        $serialized['value'] = self::serializeForRollbar($obj, $customKeys, $objectHashes, $maxDepth, $depth + 1);
        return $serialized;
    }
    ...
    {
        $objectHashes[\spl_object_hash($obj)] = \true;

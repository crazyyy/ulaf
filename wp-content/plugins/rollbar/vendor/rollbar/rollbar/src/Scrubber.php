 ...
 * @internal
 */
class Scrubber implements \Rollbar\ScrubberInterface
{
    /**
    ...
    protected function setScrubFields(array $config) : void
    {
        ...
        $this->scrubFields = \Rollbar\Defaults::get()->scrubFields($fromConfig);
    }
    /**
    ...
    public function internalScrub(mixed &$data, array $fields, string $replacement, string $path) : mixed
    {
        ...
        if (\is_string($data)) {
            ...
            if ($query) {
                return \str_replace($query, $this->scrubQueryString($query, $fields), $data);
            }
     ...
     */
    protected function scrubArray(array &$arr, array $fields, string $replacement = '********', string $path = '') : array
    {
        if (!$fields || !$arr) {
            return $arr;
        }
        $scrubber = $this;
        $scrubberFn = function (&$val, $key) use($fields, $replacement, $scrubber, &$path) {

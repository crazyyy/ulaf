 ...
 * @internal
 */
class FatalHandler extends \Rollbar\Handlers\AbstractHandler
{
    ...
    public function handle(...$args)
    {
        ...
        if ($this->isFatal($lastError)) {
            ...
            $exception = $this->logger()->getDataBuilder()->generateErrorWrapper($errno, $errstr, $errfile, $errline);
            $this->logger()->report(Level::CRITICAL, $exception, isUncaught: \true);
        }
    }
    /**
    ...
    {
        return null !== $lastError && \in_array($lastError['type'], self::$fatalErrors, \true) && !(isset($lastError['message']) && \str_starts_with($lastError['message'], 'Uncaught'));
    }
}

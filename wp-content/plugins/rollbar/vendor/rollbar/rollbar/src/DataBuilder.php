...
use Throwable;
/** @internal */
class DataBuilder implements \Rollbar\DataBuilderInterface
{
    ...
    {
        self::$defaults = \Rollbar\Defaults::get();
    ...
    {
        $fromConfig = $config['include_exception_code_context'] ?? null;
    ...
    protected function setUtilities($config)
    {
        ...
        if (!$this->utilities) {
            throw new \InvalidArgumentException('Missing dependency: Utilities not provided to the DataBuilder.');
        }
    }
    ...
    protected function getBody(Throwable|string|Stringable $toLog, array $context) : Body
    {
        ...
        } elseif ($toLog instanceof \Rollbar\ErrorWrapper) {
    ...
    public function getErrorTrace(\Rollbar\ErrorWrapper $error)
    {
     ...
     * @return Trace
     */
    public function makeTrace(Throwable $exception, bool $includeContext, ?string $classOverride = null, string|Stringable|null $message = null) : Trace
    {
        $frames = array();
        ...
        return new Trace($frames, new ExceptionInfo($classOverride ?: \get_class($exception), $exceptionMessage, $description));
    }
    public function makeFrames($exception, $includeContext)
    {
        ...
        foreach ($this->getTrace($exception) as $frameInfo) {
            ...
            $frame->setLineno($lineno)->setMethod($method);
    ...
    {
        if ($exc instanceof \Rollbar\ErrorWrapper) {
    ...
    {
        return new Message((string) $toLog, $this->sendMessageTrace ? \debug_backtrace($this->localVarsDump ? 0 : \DEBUG_BACKTRACE_IGNORE_ARGS) : null);
    }
    protected function getLevel($level, $toLog)
    {
        ...
        if ($level !== null) {
            ...
            if ($level !== null) {
                ...
                return \Rollbar\LevelFactory::fromName($level);
            }
        }
        return null;
    }
    ...
    {
        $request = new Request();
        $request->setUrl($this->getUrl())->setHeaders($this->getHeaders())->setParams($this->getRequestParams())->setBody($this->getRequestBody())->setUserIp($this->getUserIp());
        if (isset($_SERVER)) {
            $request->setMethod($_SERVER['REQUEST_METHOD'] ?? null)->setQueryString($_SERVER['QUERY_STRING'] ?? null);
        }
        if (isset($_GET)) {
            $request->setGet($_GET);
        }
    ...
    public function parseForwardedString($forwarded)
    {
        ...
        foreach ($parts as $forwardedPair) {
            $forwardedPair = \trim($forwardedPair);
    ...
    protected function getUserIp() : ?string
    {
        ...
        if ($this->captureIP === \Rollbar\DataBuilder::ANONYMIZE_IP) {
            if (\filter_var($ipAddress, \FILTER_VALIDATE_IP, \FILTER_FLAG_IPV4)) {
                $parts = \explode('.', $ipAddress);
                $ipAddress = $parts[0] . '.' . $parts[1] . '.' . $parts[2] . '.0';
            } elseif (\filter_var($ipAddress, \FILTER_VALIDATE_IP, \FILTER_FLAG_IPV6)) {
                $parts = \explode(':', $ipAddress);
                $ipAddress = $parts[0] . ':' . $parts[1] . ':' . $parts[2] . ':' . '0000:0000:0000:0000:0000';
            }
        }
        return $ipAddress;
    }
    ...
    protected function getPerson()
    {
        ...
        if (!isset($personData) && \is_callable($this->personFunc)) {
            try {
                ...
                \Rollbar\Rollbar::scope(array('person_fn' => null))->log(Level::ERROR, $exception);
            }
        }
        ...
        if (!\is_array($personData)) {
            $personData = \Rollbar\Utilities::serializeToArray($personData);
        }
    ...
    {
        $server = new Server();
        $server->setHost($this->getHost())->setRoot($this->getServerRoot())->setBranch($this->getServerBranch())->setCodeVersion($this->getServerCodeVersion());
    ...
    protected function resolveCustomContent(mixed $custom) : array
    {
        ...
        if ($custom instanceof \Serializable) {
            ...
            if ($custom instanceof \Rollbar\SerializerInterface) {
                $custom = $custom->serialize();
            }
        }
        ...
        if (\is_object($custom)) {
            \trigger_error("Using an object that does not implement the " . "Rollbar\\SerializerInterface interface has been deprecated.", \E_USER_DEPRECATED);
    ...
    {
        return new \Rollbar\ErrorWrapper($errno, $errstr, $errfile, $errline, $this->buildErrorTrace($errfile, $errline), $this->utilities);
    }
    /**
    ...
    protected function buildErrorTrace(?string $errfile, ?int $errline) : array
    {
        if ($this->captureErrorStacktraces) {
            ...
            // Add the final frame
            \array_unshift($backTrace, array('file' => $errfile, 'line' => $errline));
        } else {
            $backTrace = array();
        }
        return $backTrace;
    ...
    private function hasXdebugGetFunctionStack()
    {
        ...
        return !\str_contains(\ini_get('xdebug.mode'), 'develop') ? \false : \true;
    }
    ...
    private function stripShutdownFrames($backTrace)
    {
        foreach ($backTrace as $index => $frame) {
            \extract($frame);
            $fatalHandlerMethod = isset($method) && $method === 'Rollbar\\Handlers\\FatalHandler::handle';
            $fatalHandlerClassAndFunction = isset($class) && $class === 'Rollbar\\Handlers\\FatalHandler' && isset($function) && $function === 'handle';
            $errorHandlerMethod = isset($method) && $method === 'Rollbar\\Handlers\\ErrorHandler::handle';
            $errorHandlerClassAndFunction = isset($class) && $class === 'Rollbar\\Handlers\\ErrorHandler' && isset($function) && $function === 'handle';
            if ($fatalHandlerMethod || $fatalHandlerClassAndFunction || $errorHandlerMethod || $errorHandlerClassAndFunction) {
                return \array_slice($backTrace, $index + 1);
            }
        }
        return $backTrace;
    }
    public function detectGitBranch($allowExec = \true) : ?string
    {
        if ($allowExec) {
            static $cachedValue;
    ...
    private static function getGitBranch() : ?string
    {
        if (\function_exists('shell_exec')) {
            $stdRedirCmd = \Rollbar\Utilities::isWindows() ? ' > NUL' : ' 2> /dev/null';

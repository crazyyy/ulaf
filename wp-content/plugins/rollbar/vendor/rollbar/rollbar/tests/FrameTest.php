...
/** @internal */
class FrameTest extends \Rollbar\BaseRollbarTest
{
    ...
    {
        $context = m::mock("Rollbar\\Payload\\Context, Rollbar\\SerializerInterface")->shouldReceive("serialize")->andReturn("{CONTEXT}")->mock();
        $this->exception->shouldReceive("serialize")->andReturn("{EXC}")->mock();
        $this->frame->setFilename("rollbar.php")->setLineno(1024)->setColno(42)->setMethod("testEncode()")->setCode('$frame->setFilename("rollbar.php")')->setContext($context)->setArgs(array("hello", "world"));
        $actual = \json_encode($this->frame->serialize());
        $expected = '{' . '"filename":"rollbar.php",' . '"lineno":1024,"colno":42,' . '"method":"testEncode()",' . '"code":"$frame->setFilename(\\"rollbar.php\\")",' . '"context":"{CONTEXT}",' . '"args":["hello","world"]' . '}';

...
/** @internal */
class PayloadTest extends \Rollbar\BaseRollbarTest
{
    public function testPayloadData() : void
    {
        $data = m::mock(Data::class);
        $config = m::mock(Config::class)->shouldReceive('getAccessToken')->andReturn('012345678901234567890123456789ab')->mock();
    ...
    public function testPayloadAccessToken() : void
    {
        ...
        $data = m::mock(Data::class);
        $config = m::mock(Config::class)->shouldReceive('getAccessToken')->andReturn($accessToken)->mock();
    ...
    public function testEncode() : void
    {
        ...
        $data = m::mock('Rollbar\\Payload\\Data, Rollbar\\SerializerInterface')->shouldReceive('serialize')->andReturn(new \ArrayObject())->mock();
        m::mock(DataBuilder::class)->shouldReceive('getScrubFields')->andReturn(array())->shouldReceive('scrub')->andReturn(new \ArrayObject())->mock();
        $payload = new Payload($data, $accessToken);

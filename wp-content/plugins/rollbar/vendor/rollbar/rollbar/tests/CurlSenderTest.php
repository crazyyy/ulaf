...
/** @internal */
class CurlSenderTest extends \Rollbar\BaseRollbarTest
{
    public function testCurlError() : void
    {
        $logger = new \Rollbar\RollbarLogger(array("access_token" => $this->getTestAccessToken(), "environment" => "testing-php", "endpoint" => "fake-endpoint"));
        $response = $logger->report(Level::WARNING, "Testing PHP Notifier", array());
        $this->assertContains($response->getInfo(), array(
            "Couldn't resolve host 'fake-endpointitem'",
            // hack for PHP 5.3

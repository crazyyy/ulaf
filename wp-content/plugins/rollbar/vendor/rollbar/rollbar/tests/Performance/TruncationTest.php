...
class TruncationTest extends BaseRollbarTest
{
    ...
    public function testTruncate($dataName, $data, array $assertions = array()) : void
    {
        ...
        foreach ($assertions as $assertion) {
            $this->assertMatchesRegularExpression($assertion, $performance, "Performance of {$dataName} did not meet expectations");
        }
    }
    public function truncateProvider() : array
    {
        ...
        $massivePayloadTest = new \Rollbar\Performance\MassivePayload();
        ...
        $data = array(array("StringsStrategyTest - truncate strings to 1024", $stringsTestData, array(
            '/^Memory usage: (\\d|[12]\\d) bytes/m',
            // < 30 bytes
            '/^Execution time: \\d.\\d+ ms/m',
        )), array("FramesStrategyTest - nothing to truncate using trace key", $framesTestData, array(
            '/^Memory usage: (\\d|[12]\\d) bytes/m',
            // < 30 bytes
            '/^Execution time: \\d.\\d+ ms/m',
        )), array("MassivePayloadTest", $massivePayloadTest->executeProvider(), array(
            '/^Memory usage: (\\d|[12]\\d{1,7}) bytes/m',
            // < 20,000,000 bytes
            '/^Execution time: (\\d{1,2}.\\d+) ms/m',
        )), array("OneLongString", $stringsTest->payloadStructureProvider(\str_repeat("A", \Rollbar\Truncation\Truncation::MAX_PAYLOAD_SIZE + 1)), array(
            '/^Memory usage: (\\d|[12]\\d) bytes/m',
            // < 30 bytes
            '/^Execution time: \\d.\\d+ ms/m',
        )));

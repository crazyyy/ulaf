...
use Throwable;
/** @internal */
class DefaultsTest extends \Rollbar\BaseRollbarTest
{
    /**
     * @var Defaults
     */
    ...
    {
        $expected = array('passwd', 'password', 'secret', 'confirm_password', 'password_confirmation', 'auth_token', 'csrf_token', 'access_token');
    ...
    {
        $this->assertEquals(\true, $this->defaults->allowExec());
    }
    ...
    public function testIncludedErrnoDefault() : void
    {
        ...
        $this->assertEquals($expected, $this->defaults->includedErrno());
    }
    /**
    ...
    public function testIncludedErrnoDefineOverride() : void
    {
        ...
        $this->assertEquals(\E_USER_WARNING, (new Defaults())->includedErrno());
    }
    ...
    {
        $this->assertSame(['maxTelemetryEvents' => Telemeter::MAX_EVENTS, 'filter' => null, 'includeItemsInTelemetry' => \true, 'includeIgnoredItemsInTelemetry' => \false], $this->defaults->telemetry(\true));
        $this->assertSame(['maxTelemetryEvents' => Telemeter::MAX_EVENTS, 'filter' => null, 'includeItemsInTelemetry' => \true, 'includeIgnoredItemsInTelemetry' => \false], $this->defaults->telemetry([]));
        $this->assertNull($this->defaults->telemetry(null));
        $this->assertNull($this->defaults->telemetry(\false));
        $this->assertSame(['maxTelemetryEvents' => Telemeter::MAX_EVENTS, 'filter' => null, 'includeItemsInTelemetry' => \true, 'includeIgnoredItemsInTelemetry' => \false], $this->defaults->telemetry(['foo' => 'bar']));
        $this->assertSame(['maxTelemetryEvents' => Telemeter::MAX_EVENTS, 'filter' => null, 'includeItemsInTelemetry' => \true, 'includeIgnoredItemsInTelemetry' => \true], $this->defaults->telemetry(['includeItemsInTelemetry' => \true, 'includeIgnoredItemsInTelemetry' => \true]));
        $this->assertSame(['maxTelemetryEvents' => Telemeter::MAX_EVENTS, 'filter' => 'foo', 'includeItemsInTelemetry' => \true, 'includeIgnoredItemsInTelemetry' => \false], $this->defaults->telemetry(['filter' => 'foo']));
    }
    /**
    ...
    {
        $this->assertEquals($value, \Rollbar\Defaults::get()->fromSnakeCase($option));
    }

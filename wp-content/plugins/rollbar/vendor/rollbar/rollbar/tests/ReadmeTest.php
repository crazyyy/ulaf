...
/** @internal */
class ReadmeTest extends \Rollbar\BaseRollbarTest
{
    public function testQuickStart() : void
    {
        // installs global error and exception handlers
        Rollbar::init(array('access_token' => $this->getTestAccessToken(), 'environment' => 'production'));
        try {
        ...
        Rollbar::log(Level::INFO, 'testing extra data', array("some_key" => "some value"));
        // If you want to check if logging with Rollbar was successful
        $response = Rollbar::report(\Rollbar\LevelFactory::fromName(Level::INFO), 'testing wasSuccessful()');
    ...
    {
        Rollbar::init(array('access_token' => $this->getTestAccessToken(), 'environment' => 'test'));
        try {
            do_something();
        } catch (\Exception $e) {
    ...
    public function testBasicUsage2() : void
    {
        ...
        $result2 = Rollbar::report(Level::INFO, 'Here is a message with some additional data', array('x' => 10, 'code' => 'blue'));
        $this->assertEquals(200, $result1->getStatus());
        $this->assertEquals(200, $result2->getStatus());
    }
}

...
/** @internal */
class RequestTest extends \Rollbar\BaseRollbarTest
{
    ...
    {
        $params = array("controller" => "project", "action" => "index");
        $request = new Request();
    ...
    public function testPost() : void
    {
        ...
        $post2 = array("data" => array("Data" => "Parameters"), "access_token" => $this->getTestAccessToken());
    ...
    {
        $request = new Request();
        $request->setUrl("www.rollbar.com/account/project")->setMethod("GET")->setHeaders(array("CSRF-TOKEN" => "42", "X-SPEED" => "THEFLASH"))->setParams(array("controller" => "ProjectController", "method" => "index"))->setGet(array("fetch_account" => "true", "error_level" => "11"))->setQueryString("?fetch_account=true&error_level=11")->setUserIp("170.16.58.0");
        $request->setExtras(array("test" => "testing"));
        $expected = '{' . '"url":"www.rollbar.com\\/account\\/project",' . '"method":"GET",' . '"headers":{' . '"CSRF-TOKEN":"42","X-SPEED":"THEFLASH"' . '},' . '"params":{' . '"controller":"ProjectController",' . '"method":"index"' . '},' . '"GET":{' . '"fetch_account":"true",' . '"error_level":"11"' . '},' . '"query_string":"?fetch_account=true&error_level=11",' . '"user_ip":"170.16.58.0",' . '"test":"testing"' . '}';

...
/** @internal */
class ServerTest extends \Rollbar\BaseRollbarTest
{
    ...
    {
        $server = new Server();
        $server->setHost("server2-ec-us")->setRoot("/home/app/testingRollbar")->setBranch("master")->setCodeVersion("#dca015");
        $extras = array("test" => array(1, 2, "3", array()));
        $server->setExtras($extras);
        $expected = '{' . '"host":"server2-ec-us",' . '"root":"\\/home\\/app\\/testingRollbar",' . '"branch":"master",' . '"code_version":"#dca015",' . '"test":' . '[' . '1,' . '2,' . '"3",' . '[]' . ']' . '}';

...
class StringsStrategyTest extends BaseRollbarTest
{
    protected function execute($data) : array
    {
        ...
        $strategy = new \Rollbar\Truncation\StringsStrategy($truncation);
    ...
    {
        $data = array();
        $data["truncate nothing"] = array(self::payloadStructureProvider(\str_repeat("A", 10)), self::payloadStructureProvider(\str_repeat("A", 10)));
        return $data;
    }
    /**
    ...
    public function testExecuteArray($data, $expectedThreshold) : void
    {
        ...
        foreach ($result['data']['body']['message']['body']['value'] as $key => $toTrim) {
            $this->assertTrue(\strlen($toTrim) <= $expectedThreshold, "The string '{$toTrim}' was expected to be trimmed to " . $expectedThreshold . " characters. " . "Actual length: " . \strlen($toTrim));
        }
    }
    /**
     * Also used by {@see TruncationTest::truncateProvider()}.
    ...
    {
        $data = array();
        $thresholds = \Rollbar\Truncation\StringsStrategy::getThresholds();
    ...
    public static function thresholdTestProvider($threshold) : array
    {
        ...
        while (\strlen(\json_encode($payload)) <= \Rollbar\Truncation\Truncation::MAX_PAYLOAD_SIZE) {
            $payload['data']['body']['message']['body']['value'][] = \str_repeat('A', $stringLengthToTrim);
            $payload['data']['body']['message']['body']['value2'][] = \str_repeat('A', $stringLengthToTrim);
        }
    ...
    {
        return array("data" => array("body" => array("message" => array("body" => array("value" => $message)))));
    }
}

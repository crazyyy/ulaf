...
{
    private \Rollbar\Truncation\Truncation $truncate;
    ...
    {
        $config = new Config(array('access_token' => $this->getTestAccessToken(), 'custom_truncation' => CustomTruncation::class));
        $this->truncate = new \Rollbar\Truncation\Truncation($config);
        $data = new EncodedPayload(array("data" => array("body" => array("message" => array("body" => array("value" => \str_repeat('A', 1000 * 1000)))))));
        $data->encode();
    ...
    public function testTruncateNoPerformance($data) : void
    {
        ...
        $this->assertTrue($size <= \Rollbar\Truncation\Truncation::MAX_PAYLOAD_SIZE, "Truncation failed. Payload size exceeds MAX_PAYLOAD_SIZE.");
    }
    public static function truncateProvider() : array
    {
        $stringsTest = new \Rollbar\Truncation\StringsStrategyTest("StringsStrategyTest");
        $framesTest = new \Rollbar\Truncation\FramesStrategyTest("FramesStrategyTest");
        ...
        $data = \array_merge($stringsTest::executeTruncateNothingProvider(), $stringsTest::executeArrayProvider(), $framesTestData);
        return $data;
    }
}

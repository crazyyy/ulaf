...
class ExceptionHandlerTest extends BaseRollbarTest
{
    ...
    public function testPreviousExceptionHandler() : void
    {
        ...
        \set_exception_handler(function () use($testCase) {
            $testCase->assertTrue(\true, "Previous exception handler invoked.");
        });
    ...
    {
        $handler = $this->getMockBuilder(ExceptionHandler::class)->setConstructorArgs(array(new RollbarLogger(self::$simpleConfig)))->onlyMethods(array('handle'))->getMock();
    ...
    public function testHandle() : void
    {
        ...
        $logger = $this->getMockBuilder(RollbarLogger::class)->setConstructorArgs(array(self::$simpleConfig))->onlyMethods(array('report'))->getMock();
    ...
    public function testDeprecatedDynamicProperties() : void
    {
        ...
        \set_error_handler(function ($errno, $errstr, $errfile, $errline) use(&$errors) {
            $errors[] = array('errno' => $errno, 'errstr' => $errstr, 'errfile' => $errfile, 'errline' => $errline);
        });
        $handler = new ExceptionHandler(new RollbarLogger(self::$simpleConfig));
        $handler->register();
        $handler->handle(new \Exception());

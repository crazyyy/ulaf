...
/** @internal */
class DataTest extends \Rollbar\BaseRollbarTest
{
    ...
    {
        $level = Level::ERROR;
        $this->assertEquals(\Rollbar\LevelFactory::fromName($level), $this->data->setLevel($level)->getLevel());
    }
    ...
    public function testEncode() : void
    {
        ...
        $data = $this->data->setEnvironment("testing")->setBody($body)->setLevel($level)->setTimestamp($time)->setCodeVersion("v0.17.3")->setPlatform("LAMP")->setLanguage("PHP 5.4")->setFramework("CakePHP")->setContext("AppController->updatePerson")->setRequest($request)->setPerson($person)->setServer($server)->setCustom(array("x" => "hello", "extra" => array('key' => 'val')))->setFingerprint("big-fingerprint")->setTitle("The Title")->setUuid("123e4567-e89b-12d3-a456-426655440000")->setNotifier($notifier);
    ...
    private function mockSerialize($mock, $returnVal)
    {
        ...
        return $mock->shouldReceive("serialize")->andReturn($returnVal)->mock();
    }
}

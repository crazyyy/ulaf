...
class Renamer_Controller {
    ...
    public function late_load_components() {
        // Se auto-rename è abilitato, dobbiamo caricare il file processor anche durante upload AJAX
        $auto_rename_enabled = get_option('imgseo_auto_rename_on_upload', 0);

        if ($auto_rename_enabled) {
            // Carica solo il file processor se non già caricato
            if (!$this->file_processor && !class_exists('Renamer_File_Processor')) {
        ...
        // Carica solo quando necessario per le altre operazioni
    ...
    private function is_renamer_ajax() {
        ...
        return in_array($action, $renamer_actions, true);
    }
    ...
    public function ajax_rename_image() {
        ...
        if (!current_user_can('upload_files')) {
            wp_send_json_error(array('message' => esc_html__('You do not have permission to rename images.', 'imgseo-ai-alt-text-generator')));
        }
        ...
        try {
            ...
            $new_filename = isset($_POST['new_filename']) ? sanitize_file_name(wp_unslash($_POST['new_filename'])) : '';
            
            if (!$attachment_id || empty($new_filename)) {
                wp_send_json_error(array('message' => esc_html__('Invalid attachment ID or filename.', 'imgseo-ai-alt-text-generator')));
            }
            ...
            if (class_exists('ImgSEO_Process_Lock') && ImgSEO_Process_Lock::is_globally_locked()) {
                wp_send_json_error(array('message' => esc_html__('ImgSEO operations are temporarily blocked. Please try again in a moment.', 'imgseo-ai-alt-text-generator')));
            }
            ...
            // Abilita sempre l'aggiornamento dei riferimenti per il renamer singolo (come nel bulk)
    ...
    public function ajax_restore_image() {
        ...
        if (!current_user_can('upload_files')) {
            wp_send_json_error(array('message' => esc_html__('You do not have permission to restore images.', 'imgseo-ai-alt-text-generator')));
        }
        ...
        try {
            ...
            $original_filename = isset($_POST['original_filename']) ? sanitize_text_field(wp_unslash($_POST['original_filename'])) : '';
            $current_filename = isset($_POST['current_filename']) ? sanitize_text_field(wp_unslash($_POST['current_filename'])) : '';
            
            if (!$attachment_id || empty($original_filename) || empty($current_filename)) {
                wp_send_json_error(array('message' => esc_html__('Invalid attachment ID or filenames.', 'imgseo-ai-alt-text-generator')));
            }
            ...
            wp_send_json_success(array(
                ...
                'new_filename' => $result['new_filename'],
                'new_url' => $result['new_url'],
                'message' => esc_html__('Image successfully restored to its original filename.', 'imgseo-ai-alt-text-generator')
            ));
        } finally {
            // Ripristina il comportamento normale delle API
            if ($did_http_api_filter) {
    ...
    public function ajax_preview_batch_rename() {
        ...
        if (empty($attachment_ids)) {
            wp_send_json_error(array('message' => esc_html__('No images selected.', 'imgseo-ai-alt-text-generator')));
        }
        ...
        if (!$this->pattern_manager) {
            wp_send_json_error(array('message' => esc_html__('Pattern manager not available.', 'imgseo-ai-alt-text-generator')));
        }
    ...
    public function ajax_batch_rename() {
        ...
        if (!$this->batch_processor) {
            wp_send_json_error(array('message' => esc_html__('Batch processor not available.', 'imgseo-ai-alt-text-generator')));
        }
        ...
        wp_send_json_success(array(
            'results' => $all_results,
            'message' => sprintf(
                // translators: %1$d is the total number of images processed, %2$d is the number of successful renames, %3$d is the number of errors
                esc_html__('Processed %1$d images: %2$d renamed successfully, %3$d errors.', 'imgseo-ai-alt-text-generator'),

...
class ImgSEO_Divi_Integration implements Renamer_Integration_Interface {
    
    /**
     * Updates image references in Divi
     * 
    ...
    public function update_references($old_urls, $new_urls, $attachment_id) {
        ...
        if (!function_exists('et_pb_is_pagebuilder_used') && !function_exists('et_core_is_builder_used_on_current_request')) {
            return array(
                'status' => false,
                'updated' => 0,
                'message' => __('Divi is not active', 'imgseo-ai-alt-text-generator')
            );
        }
        ...
        // 1. Update post contents with Divi Builder
        ...
        return array(
            ...
            'message' => sprintf(
                // translators: %1$d is the number of references updated in posts/pages, %2$d is the number of Divi layouts updated
                __('Updated %1$d references in posts/pages and %2$d Divi layouts', 'imgseo-ai-alt-text-generator'),
                $updated_count, 
                $updated_layouts
            )
        );

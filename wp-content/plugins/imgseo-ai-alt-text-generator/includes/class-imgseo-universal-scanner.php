...
class ImgSEO_Universal_Scanner {
    ...
    /**
     * Start time of the scan for timeout management
     *
     * @var float
     */
    ...
    public function scan_single_post($post_id) {
        ...
        // Initialize scan stats if not already initialized
        if (!isset($this->scan_stats)) {
            $this->init_scan_stats();
        }
    ...
    private function scan_widgets() {
        ...
        // Get sidebars widgets once before loop
    ...
    private function set_scan_limits() {
        // Aumenta il limite di memoria usando wp_raise_memory_limit()
        wp_raise_memory_limit();

        // Invece di set_time_limit(), utilizziamo un approccio più sicuro con controllo del tempo
        // Il timeout verrà gestito manualmente durante l'esecuzione dello scanner
    ...
    public function ajax_scan_content() {
        ...
        if (!isset($_POST['nonce']) || !wp_verify_nonce(wp_unslash($_POST['nonce']), 'imgseo_scan_content')) {
        ...
        // Security: Rate limiting - max 30 scan attempts per 10 minutes per user
        $user_id = get_current_user_id();
        $rate_key = 'imgseo_scan_attempts_' . $user_id;
        $attempts = get_transient($rate_key) ?: 0;
        
        if ($attempts >= 30) {
            wp_send_json(['success' => false, 'error' => __('Too many scan attempts. Please wait 10 minutes.', 'imgseo-ai-alt-text-generator')]);
        }
        ...
        if ($content_type === 'all') {
            ...
            $result = array('success' => false, 'error' => __('Invalid parameters', 'imgseo-ai-alt-text-generator'));
        }

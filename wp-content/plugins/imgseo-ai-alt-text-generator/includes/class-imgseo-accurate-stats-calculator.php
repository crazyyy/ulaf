...
class ImgSEO_Accurate_Stats_Calculator {
    ...
    private function get_scan_stats() {
        global $wpdb;
        $table_scan_status = esc_sql($this->db_manager->get_table_name('scan_status'));
        ...
        return array(
            ...
            'last_full_scan_formatted' => $last_full_scan ? gmdate('Y-m-d H:i:s', $last_full_scan) : 'Mai',
    ...
    private function cache_stats($cache_key, $stats) {
        ...
        $existing = $wpdb->get_var($wpdb->prepare(
            "SELECT id FROM %s WHERE stat_key = %s",
            $table_stats_cache,
            $cache_key
        ));
    ...
    public function clear_stats_cache($cache_key = null) {
        ...
        if ($cache_key) {
            ...
            $wpdb->query($wpdb->prepare("TRUNCATE TABLE %s", $table_stats_cache));
        }

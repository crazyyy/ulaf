...
class ImgSEO_Structured_Data {
    ...
    private function build_schema_from_cached_data($image_data, $attachment_id) {
        ...
        if (!empty($image_data['upload_date'])) {
            $schema['uploadDate'] = gmdate('c', strtotime($image_data['upload_date']));
        }
    ...
    private function get_rendered_page_content() {
        ...
        if (is_singular()) {
            if ($post) {
                ...
                // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Using WordPress core hook intentionally
    ...
    public function ajax_generate_structured_data() {
        ...
        if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'imgseo_structured_data')) {
            wp_send_json_error(['message' => __('Security check failed', 'imgseo-ai-alt-text-generator')]);
        }
        ...
        $images_data = isset($_POST['images']) ? json_decode(stripslashes(sanitize_textarea_field(wp_unslash($_POST['images']))), true) : array();

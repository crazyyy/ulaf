...
class ImgSEO_Image_Sitemap_Generator {
	...
	private function handle_sitemap_request() {
		try {
			...
			// Note: This is a public endpoint for sitemap generation, nonce verification not required
			if (isset($_GET['test']) && sanitize_text_field(wp_unslash($_GET['test'])) === '1') {
			...
			if (file_exists($sitemap_filepath)) {
				...
				if ( $wp_filesystem->exists( $sitemap_filepath ) ) {
					// Sitemap XML content is safe to output directly as it's generated by our plugin
					// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					echo $wp_filesystem->get_contents( $sitemap_filepath );
				}
				return;
			}
	...
	public function generate_sitemap_file() {
		try {
			...
			if (empty($image_data)) {
				return array(
					'status' => 'warning',
					'message' => __('No images found to include in the sitemap.', 'imgseo-ai-alt-text-generator')
				);
			}
			...
			// Verifica che la directory sia scrivibile usando WP_Filesystem
			global $wp_filesystem;
			if ( ! $wp_filesystem ) {
				require_once ABSPATH . 'wp-admin/includes/file.php';
				WP_Filesystem();
			}
			if ( ! $wp_filesystem->is_writable( $sitemap_dir ) ) {
				return array(
					'status' => 'error',
					'message' => __('Could not write sitemap file. The WordPress root directory is not writable. Please check your server permissions.', 'imgseo-ai-alt-text-generator')
				);
			}
			...
			if ($safe_path !== $expected_path) {
				return array(
					'status' => 'error',
					'message' => __('Security error: Invalid sitemap path.', 'imgseo-ai-alt-text-generator')
				);
			}
			...
			if ($result !== false) {
				...
				return array(
					'status' => 'success',
					'message' => sprintf(
						// translators: %1$d is the number of images in the sitemap, %2$s is the sitemap URL
						__('Image sitemap successfully generated with %1$d images. Available at: %2$s', 'imgseo-ai-alt-text-generator'),
						$total_images,
				...
				return array(
					'status' => 'error',
					'message' => __('Could not write sitemap file. Please check folder permissions.', 'imgseo-ai-alt-text-generator')
				);
			}
	...
	public function render_admin_page() {
		...
		if ( file_exists( $template_path ) ) {
			...
			echo '<div class="wrap"><h2>' . esc_html__( 'Image Sitemap Settings', 'imgseo-ai-alt-text-generator' ) . '</h2><p>' . esc_html__( 'Error: Admin page template not found.', 'imgseo-ai-alt-text-generator' ) . '</p></div>';
		}
	}
	 ...
	 *
	 * @param int|WP_Post $attachment_id Attachment ID passato dagli hooks (opzionale, non utilizzato)
	 */
	public function invalidate_sitemap_cache($attachment_id = 0) {
		// FIX: Accetta il parametro passato dagli hooks add_attachment, edit_attachment, delete_attachment
		// anche se non viene utilizzato dalla funzione. Questo evita warning/errori quando WordPress
		// passa l'ID dell'attachment agli action hooks.

...
(function($) {
    ...
    var config = {
        ...
        texts: typeof ImgSEO !== 'undefined' && ImgSEO.texts ? ImgSEO.texts : {
            generate_button: 'Generate Alt Text',
    ...
    $(document).ready(function() {
        ...
        log('ImgSEO Alt Text Generator initialized');
    });
    ...
    function handleGenerateButtonClick(e) {
        ...
        if (!attachmentId) {
            log('Missing attachment ID');
            alert(config.texts.no_id_found || 'Image ID not found');
            return;
        }
        
        // Verifica se l'immagine è già in elaborazione
        if (isProcessing(attachmentId)) {
            log('ID ' + attachmentId + ' already processing, request ignored');
            return;
        }
    ...
    function generateAltText(attachmentId, $button) {
        ...
        $.ajax({
            ...
            complete: function() {
                ...
                setTimeout(function() {
                    if (config.processingIds[attachmentId]) {
                        log('End protection period for ID: ' + attachmentId);
    ...
    function updateTextField(fieldName, value) {
        try {
            log('Field update ' + fieldName + ':', value);
            ...
            log('Error updating field ' + fieldName + ':', e);
        }
    }
    ...
    function updateViaBackbone(fieldName, value) {
        ...
        try {
            ...
            if (attachment && attachment.set) {
                ...
                log('Field ' + fieldName + ' updated via Backbone');
                return true;
            }
        } catch (e) {
            log('Backbone error:', e);
        }
    ...
    function updateViaDom(fieldName, value) {
        ...
        if (!selectors.length) {
            log('No selector found for field:', fieldName);
            return false;
        }
        ...
        for (var i = 0; i < selectors.length; i++) {
            ...
            if ($fields.length) {
                ...
                log('Field ' + fieldName + ' updated via DOM with selector:', selectors[i]);
                updated = true;

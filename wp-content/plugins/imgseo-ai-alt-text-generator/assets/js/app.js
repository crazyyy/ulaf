...
function bulkGenerateAJAX(batchSize = 5, callback) { 
    ...
    function processBatch() {
        ...
        var updateTitle = jQuery('input[name="update_title"]').is(':checked') ? 1 : 0;
        var updateCaption = jQuery('input[name="update_caption"]').is(':checked') ? 1 : 0;
        var updateDescription = jQuery('input[name="update_description"]').is(':checked') ? 1 : 0;
...
;
            ...
            error: function (error) {
                if (retryCount < maxRetries) {
                    retryCount++;
                    showNotification(`An error occurred. Retrying (${retryCount}/${maxRetries})...`, 'error');
                    setTimeout(processBatch, 2000); // Retry after 2 seconds
                } else {
                    showNotification('Process stopped due to too many errors. Please check your connection or try again later.', 'error');
                    jQuery('.imgseo-loader').remove();
                    jQuery('[data-start-bulk-generate]').removeClass('disabled').text('Start Now');
                }
            }
        });
    }

...
jQuery(document).ready(function($) {
    // Debug configuration
    var debugMode = typeof ImgSEOSettings !== 'undefined' && ImgSEOSettings.debug ? true : false;
        ...
        if (confirm('Are you sure you want to disconnect your ImgSEO account? You will need to enter your ImgSEO Token again.')) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        // Use the correct settings URL with anti-cache parameters
                        var reloadUrl = ImgSEOSettings.settings_url + '&disconnected=1&_=' + new Date().getTime();
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    var errorMessage = (response.data && response.data.message) 
                        ...
                        : 'Invalid ImgSEO Token. Make sure you copied it correctly or <a href="https://dashboard.imgseo.net/login" target="_blank">register to get a new one</a>.';
                    
                    showStatusMessage($statusDiv, 'error', errorMessage);
                }
            },
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    $creditsDisplay.text(parseFloat(credits).toFixed(1));

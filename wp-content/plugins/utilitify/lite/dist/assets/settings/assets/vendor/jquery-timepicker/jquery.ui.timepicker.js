	...
	$.extend(
		Timepicker.prototype, {
			...
			_connectTimepicker: function (target, inst) {
				...
				keyup( this._doKeyUp ).
				bind(
					"setData.timepicker", function (event, key, value) {
						inst.settings[key] = value;
					}
				).
				bind(
					"getData.timepicker", function (event, key) {
			...
			_showTimepicker: function (input) {
				...
				if (input.nodeName.toLowerCase() != 'input') {
					input = $( 'input', input.parentNode )[0]; } // find from button/image trigger
			...
			_rebindDialogEvents: function (inst) {
				...
				.trigger( 'mouseover' )
				.end()
				.find( '.ui-timepicker-now' ).bind(
					"click", function(e) {
						$.timepicker.selectNow( e );
					}
				).end()
				.find( '.ui-timepicker-deselect' ).bind(
					"click",function(e) {
						$.timepicker.deselectTime( e );
					}
				).end()
				.find( '.ui-timepicker-close' ).bind(
					"click",function(e) {
						$.timepicker._hideTimepicker();
					}
				).end();
			...
			*/
			...
			_enableTimepicker: function(target) {
				...
				this._disabledInputs = $.map(
					...
					function(value) { return (value == target_id ? null : value); }
				); // delete entry
			},
			...
			_checkOffset: function (inst, offset, isFixed) {
				...
				offset.left -= Math.min(
					offset.left, (offset.left + tpWidth > viewWidth && viewWidth > tpWidth) ?
					Math.abs( offset.left + tpWidth - viewWidth ) : 0
				);
				offset.top  -= Math.min(
					offset.top, (offset.top + tpHeight > viewHeight && viewHeight > tpHeight) ?

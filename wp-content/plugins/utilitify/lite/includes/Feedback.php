...
class Feedback {
		...
		if ( ! defined( 'DOING_AJAX' ) && is_admin() ) {
			...
			if ( $enable_review_notice ) {
				...
				if ( $can_ask_user_for_review ) {
					...
					$nonce_verified = ! empty( $_GET['kc_uf_feedback_nonce'] ) ? wp_verify_nonce( $_GET['kc_uf_feedback_nonce'], 'review' ) : '';

					$action = '';
					if ( $nonce_verified ) {
						$action = ! empty( $_GET['kc_uf_feedback_action'] ) ? sanitize_text_field( wp_unslash( $_GET['kc_uf_feedback_action'] ) ) : '';
					...
					if ( ! $got_feedback ) {
						...
						$review_url      = add_query_arg( 'review_url', $review_url, add_query_arg( 'kc_uf_feedback_nonce', $nonce, add_query_arg( 'kc_uf_feedback_action', $review_done_option, $current_page_url ) ) );
						$maybe_later_url = add_query_arg( 'kc_uf_feedback_nonce', $nonce, add_query_arg( 'kc_uf_feedback_action', $maybe_later_option, $current_page_url ) );
						$already_did_url = add_query_arg( 'kc_uf_feedback_nonce', $nonce, add_query_arg( 'kc_uf_feedback_action', $already_did_option, $current_page_url ) );
						$no_bug_url      = add_query_arg( 'kc_uf_feedback_nonce', $nonce, add_query_arg( 'kc_uf_feedback_action', $no_bug_option, $current_page_url ) );

						?>

                        <style type="text/css">

                            .kc-uf-feedback-notice-links li {
                                display: inline-block;
                                margin-right: 15px;
                            }

                            .kc-uf-feedback-notice-links li a {
                                display: inline-block;
                                color: #10738b;
                            ...
                            .kc-uf-feedback-notice {
                                display: flex;
                                align-items: center;
                            }

                            .kc-uf-feedback-plugin-icon {
                                float: left;
                                margin-right: 0.5em;
                            }
						...
						echo '<div class="notice notice-success kc-uf-feedback-notice">';
						echo '<span class="kc-uf-feedback-plugin-icon"> <img src="' . $icon_url . '" alt="Logo"/></span>';
						echo $message;
						echo "<ul class='kc-uf-feedback-notice-links'>";
	...
	function review_message_data( $review_data ) {
		...
		$icon_url = KC_UF_PLUGIN_ASSETS_DIR_URL . '/images/icon-64.png';

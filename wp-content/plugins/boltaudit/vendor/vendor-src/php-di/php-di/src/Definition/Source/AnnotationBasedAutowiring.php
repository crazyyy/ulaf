...
use BoltAudit\DI\Annotation\Inject;
use BoltAudit\DI\Annotation\Injectable;
use BoltAudit\DI\Definition\Exception\InvalidAnnotation;
use BoltAudit\DI\Definition\ObjectDefinition;
use BoltAudit\DI\Definition\ObjectDefinition\MethodInjection;
use BoltAudit\DI\Definition\ObjectDefinition\PropertyInjection;
use BoltAudit\DI\Definition\Reference;
use BoltAudit\Doctrine\Common\Annotations\AnnotationRegistry;
use BoltAudit\Doctrine\Common\Annotations\Reader;
use BoltAudit\Doctrine\Common\Annotations\SimpleAnnotationReader;
use InvalidArgumentException;
use BoltAudit\PhpDocReader\PhpDocReader;
use ReflectionClass;
...
class AnnotationBasedAutowiring implements DefinitionSource, Autowiring
{
    ...
    private function readProperty(ReflectionProperty $property, ObjectDefinition $definition, $classname = null)
    {
        ...
        $annotation = $this->getAnnotationReader()->getPropertyAnnotation($property, 'BoltAudit\\DI\\Annotation\\Inject');
    ...
    private function getMethodInjection(ReflectionMethod $method)
    {
        // Look for @Inject annotation
        try {
            $annotation = $this->getAnnotationReader()->getMethodAnnotation($method, 'BoltAudit\\DI\\Annotation\\Inject');
    ...
    public function getAnnotationReader()
    {
        if ($this->annotationReader === null) {
            ...
            $this->annotationReader->addNamespace('BoltAudit\\DI\\Annotation');
        }
    ...
    private function readInjectableAnnotation(ReflectionClass $class, ObjectDefinition $definition)
    {
        try {
            ...
            $annotation = $this->getAnnotationReader()->getClassAnnotation($class, 'BoltAudit\\DI\\Annotation\\Injectable');

...
class Urap_Url_Rewrite_Analyzer {
    ...
    public function urap_admin_scripts($hook) {
    ...
    public function urap_do_analyze_page() {
        ...
        $idx = 0;
        if ($rewrite_rules) {
            $rewrite_rules = array_slice($rewrite_rules, $offset, $per_page, true);
        ...
        // Add pagination data to the page
        $pagination = array(
        ...
        wp_localize_script($this->gettext_domain, 'Rewrite_Analyzer_Pagination', $pagination);
    ...
    public function urap_change_ui() {
        ...
        if (empty($style)) {
            wp_send_json_error('Invalid style parameter');
            return;
        }

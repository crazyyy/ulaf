        ...
        $.post({
            ...
            success: function (response) {
                if (response.success) {
                    ...
                    $this.find('span').html('Error: ' + (response.data || 'Unknown error'));
                    $this.removeClass('rotate');
                }
                setTimeout(() => {
            ...
            error: function(xhr, status, error) {
                $this.find('span').html('Error: ' + error);
                $this.removeClass('rotate');
                setTimeout(() => {
                    $this.find('span').html('Refresh permalinks');
                }, 3000);
        ...
        $.post({
            ...
            success: function (response) {
                if (response.success) {
                    ...
                    console.error('Error changing UI:', response.data);
                    alert('Error changing UI: ' + response.data);
                }
            },
            error: function(xhr, status, error) {
                console.error('Error changing UI:', error);
                alert('Error changing UI: ' + error);
            }
        });
    })

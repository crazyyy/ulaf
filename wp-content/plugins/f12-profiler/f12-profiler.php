...
namespace f12_profiler {
	...
	*/
	...
	define( 'F12_PROFILER_VERSION', '1.4.0' );
	...
	if ( version_compare( $wp_version, F12_PROFILER_MINIMUM_WP_VERSION, '<' ) ) {
        wp_die(
            esc_html__( 'F12 Profiler requires at least WordPress Version ', 'f12_profiler' )
            . esc_html( F12_PROFILER_MINIMUM_WP_VERSION )
        );
	}
	...
	class Profiler {
		...
		public function addAdminScripts( $hook ) {
            if ( Profiler::getOptions()['page_metrics'] ) {
                wp_enqueue_script(
                    'f12p_measure',
                    plugin_dir_url( __FILE__ ) . 'assets/measure.js',
                    array( 'jquery' ),
                ...
                wp_enqueue_script(
                    'f12p_measure_async',
                    plugin_dir_url( __FILE__ ) . 'assets/measure_async.js',
                    array( 'jquery' ),
                    '1.0.0', // Versionsnummer
                    true // Lädt das Skript im Footer
                );
            ...
            if ( Profiler::getOptions()['hardware_metrics'] && $hook == 'tools_page_f12p' ) {
                wp_enqueue_script(
                    ...
                    array( 'jquery' ),
                    '3.9.1', // Versionsnummer (Beispiel: basierend auf der Chart.js-Version)
                    true // Lädt das Skript im Footer
                );

                wp_enqueue_script(
                    'Hardware',
                    plugin_dir_url( __FILE__ ) . 'assets/hardware.js',
                    array( 'jquery' ),
                    '1.0.0', // Versionsnummer
                    true // Lädt das Skript im Footer
                );
		...
		public function addAdminStyles() {
            wp_enqueue_style(
                ...
                '1.0.0'  // Statische Versionsnummer
            );

            if ( Profiler::getOptions()['hardware_metrics'] ) {
                wp_enqueue_style(
                    'charts',
                    plugin_dir_url( __FILE__ ) . 'assets/Chart.css',
                    array(), // Abhängigkeiten, falls vorhanden
                    '3.9.1'  // Beispiel: Chart.js CSS-Version
                );
            }
		}
		...
		public function addScripts() {
            if ( Profiler::getOptions()['page_metrics'] ) {
                wp_enqueue_script(
                    ...
                    plugin_dir_url( __FILE__ ) . 'assets/measure.js',
                    array( 'jquery' ),
                    filemtime( plugin_dir_path( __FILE__ ) . 'assets/measure.js' ), // Datei-Änderungszeit als Versionsnummer
                    true // Lädt das Skript im Footer
                );
            }
		}

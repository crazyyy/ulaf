 ...
 *  License: GPLv2
 *  Copyright (C) 2014-2020 Lilaea Media
 */
...
//                    }
        ...
        setup_raw_css: function(){
            var self = this, 
                raw_css_atts = $.extend( {}, self.raw_css_atts );
            raw_css_atts.onChangeLintingErrors = function( errors ) {
                $.chldthmcfg.raw_css_errs = errors;
                if ( 0 === errors.length ) {
                    $( '#ctc_save_new_selectors' ).removeClass( 'disabled' ).prop( 'disabled', false );
                } else {
                    $( '#ctc_save_new_selectors' ).addClass( 'disabled' ).prop( 'disabled', true );
                }
            };
            self.raw_css = window.wp.codeEditor.initialize( $( "#ctc_new_selectors" ), raw_css_atts );
        ...
        update_form: function() {
            var self        = this,
                themedir;
            $( '#input_row_stylesheet_handling_container,#input_row_parent_handling_container,#input_row_addl_css_container,#input_row_new_theme_slug,#input_row_duplicate_theme_slug,#ctc_copy_theme_mods,#ctc_child_header_parameters,#ctc_configure_submit,#input_row_theme_slug' ).slideUp( 'fast' );
        ...
        query_css: function( obj, key, params ) {
            ...
            postdata.ctc_fingerprint = $( '#ctc_fingerprint' ).val();
        ...
        save: function( obj ) {
            ...
            if ( id.match( /ctc_configtype/ ) ) {
                ...
                if ( self.raw_css_errs.length ){
                    // console.log( 'errors', self.raw_css_errs.length );
                    return;
                } else {
                    postdata.ctc_new_selectors = self.raw_css.codemirror.getValue(); // $selector.val();
        ...
        handle_success: function( obj, response ) {
            ...
            if ( self.is_empty( response ) ) {
                ...
                if ( $( 'ctc_new_selectors' ).length ){
                    $( '#ctc_new_selectors' ).val( '' );
                    self.raw_css.codemirror.setValue( '' );                    
                }
                // update data objects   
                // show check mark
...
//                }
        ...
        toggle_sepstylesheet: function(){
            if ( 'separate' === $( 'input[name=ctc_handling]:checked' ).val() ){
                $( '.sepstylesheet-container' ).stop().slideDown();
            } else {
                $( '.sepstylesheet-container' ).stop().slideUp();
            }
        },
        reload_preview: function(){
            var homeurl = window.ctcAjax.homeurl.replace( /^https?/, window.ctcAjax.ssl ? 'https' : 'http' ),
                src     = homeurl + '&preview_iframe=1&template=' + window.ctcAjax.parnt + '&stylesheet=' + window.ctcAjax.child;
        ...
        init: function() {
            ...
            // inject theme info panels into widget list instances at render event
            try {
                $.widget( 'ctc.themeMenu', $.ui.selectmenu, {
                    _renderItem: function( ul, item ) {
                        var id = this.ids.element,
                            li = $( "<li>" ),
                            sel = item.value.replace( /[^\w\-]/g, '' );
                        // console.log( '#' + id + '_option_' + sel  );
                        $( '#' + id + '_option_' + sel )
                            .detach().appendTo( li );
            ...
            // initialize theme data select menu
            if ( $( '#ctc_theme_data' ).length ){

                try {
                    $( '#ctc_theme_data' ).themeMenu();
                } catch( exn ) {
                    if ( 'function' === typeof themeMenu ) {
                        $( '#ctc_theme_data' ).themeMenu( 'destroy' );
                    } else {
                        $( '#ctc_theme_data-button' ).remove();
                    }
                    self.jquery_exception( exn, 'Data Theme Menu' );
                }
            
            }
            if ( !$( '#ctc_theme_parnt' ).is( 'input' ) ) {
                
                // initialize parent theme select menu
                ...
                // if no child themes, prepopulate with default values
            ...
            // separate stylesheet input toggle
            $( '#ctc_main' ).on( 'click', 'input[name="ctc_handling"]', self.toggle_sepstylesheet );
            
            self.toggle_sepstylesheet();
            ...
            $( '#ctc_main' ).on( 'click', '.ctc-upgrade-notice .notice-dismiss', function() { // e ) {
                ...
                var postdata = {
                    ...
                    'ctc_fingerprint': $( '#ctc_fingerprint' ).val()
                };
            ...
            if ( self.is_empty( self.jqueryerr ) ){
                ...
                $( '#ctc_main' ).on( 'click', '#ctc_copy_selector', function(  ) {
                    var txt = $( '#ctc_sel_ovrd_selector_selected' ).text().trim(),
                        cur = self.raw_css.codemirror.getValue();
                    if ( !self.is_empty( txt ) ){
                        cur +=  "\n" + txt + " {\n\n}";
                        self.raw_css.codemirror.setValue( cur );
                ...
                $( '#ctc_main' ).on( 'click', '.ctc-key-notice .notice-dismiss', function() { // e ) {
                    //console.log( 'dismiss key notice clicked!' );
                    //e.preventDefault();
                    var postdata = {
                        'action': 'ctc_keynotice',
                        '_wpnonce': $( '#_wpnonce' ).val()
                    };
                    self.ajax_post( 'dismiss', postdata );
                } );                
    ...
    $.chldthmanalyze = {
        ...
        analyze_theme: function( themetype ) { 
            // console.log( 'analyze_theme: ' + themetype );
            var self        = this,
            ...
            $.get( url, function( data ) {
                // console.log( data );
                // console.log( 'fetchdone ' + themetype + ' ...' );
                $( document ).trigger( 'fetchdone', data );
                self.parse_page( themetype, data );
                // console.log( 'analysisdone ' + themetype + ' ...' );
                ...
                $.ajax( { 
                    ...
                    if ( data.signals.httperr ) {
                        ...
                        // console.log( 'fetchdone trigger (ctc_analyze) ...' );
                        $( document ).trigger( 'fetchdone', data.body );
                        self.parse_page( themetype, data.body );
                    }
                    // console.log( 'analysisdone trigger (ctc_analyze) ...' );
                    $( document ).trigger( 'analysisdone' );
                } ).fail( function( xhr, status, err ){
                    /**
                    ...
                    // console.log( 'analysisdone trigger (fail) ...' );
                    $( document ).trigger( 'analysisdone', 'fail' );
                } );
            } );
        },
        parse_page: function( themetype, body ){
            var self        = this,
                themepath   = ( self.escrgx( window.ctcAjax.theme_uri.replace( /https?:\/\//g, '' ) ) + ( window.ctcAjax.plugin_uri ? '|' + self.escrgx( window.ctcAjax.plugin_uri.replace( /https?:\/\//g, '' ) ) : '' ) ),
                stylesheet  = ( 'child' === themetype ? $.chldthmcfg.currchild : $.chldthmcfg.currparnt ),
                escaped     = self.escrgx( $.chldthmcfg.currparnt ) +
                    ( 'child' === themetype ? '|' + self.escrgx( stylesheet ) : '' ) +
                    ( window.ctcAjax.plugin_dir ? '|' + window.ctcAjax.plugin_dir : '' ),
                regex_link  = new RegExp( "<link( rel=[\"']stylesheet[\"'] id=['\"]([^'\"]+?)['\"])?[^>]+?(" +
                    themepath + ')/(' + escaped + ')/([^"\']+\\.css)(\\?[^"\']+)?["\'][^>]+>', 'gi' ),
            ...
            //console.log( 'parsing page: ' + themepath + ' '  + regex_link );
            /*
            if ( 'child' === themetype ) {
            ...
            */
            ...
            if ( body.match( /HAS_PLUGIN_STYLESHEET/ ) ) {
                self.analysis[ themetype ].signals.thm_has_plugins = 1;
                // console.log( 'signal: thm_has_plugins' );
            }
            ...
            while ( ( csslink = regex_link.exec( body ) ) ) {
                var stylesheetid    = self.trmcss( csslink[ 2 ] ),
                    stylesheettheme = csslink[ 4 ], 
                    stylesheetpath  = csslink[ 5 ],
                    linktheme       = ( window.ctcAjax.plugin_dir && window.ctcAjax.plugin_dir === stylesheettheme ) ? 'plugin' : ( $.chldthmcfg.currparnt === stylesheettheme ? 'parnt' : 'child' ),
                    noid            = 0;
                     // console.log( 'stylesheetid: ' + stylesheetid + ' stylesheettheme: ' + stylesheettheme + ' stylesheetpath: ' + stylesheetpath );
                ...
                if ( 'plugin' === linktheme ){
                    //console.log( 'plugin stylesheet detected!' );
                    if ( window.ctcAjax.plugin_css[ stylesheetpath ] ){
                        var pathparts = stylesheetpath.split( '/' ),
                            plugindir = pathparts.shift(),
                            csspath = pathparts.join( '/' );
                        //console.log( plugindir + stylesheetpath );
                        plugindir = null;
                        self.parseable[ stylesheetpath ] = '<strong>Plugin: ' + window.ctcAjax.plugin_css[ stylesheetpath ] + ' [' + stylesheetid + ']</strong>' + ( themeloaded ? ' ' + $.chldthmcfg.getxt( 'anlzsep' ) : '' )  + ' <small><code>' + csspath + '</code></small>';
                    }
                } else if ( stylesheetpath.match( /^style.*?\.css$/ ) ) {
                    ...
                    self.analysis[ themetype ].signals.thm_rtl = 1;
                    // add stylesheet to parsables
                    self.parseable[ stylesheetpath ] = '<strong>' + stylesheetid + '</strong>' + ( themeloaded ? ' ' + $.chldthmcfg.getxt( 'anlzsep' ) : '' ) + ' <small><code>' + stylesheetpath + '</code></small>';
                    ...
                    self.analysis[ themetype ].deps[ themeloaded ].push( [ stylesheetid, stylesheetpath ] );
                    // if loaded prior to child stylesheet add stylesheet to parsables
                    self.parseable[ stylesheetpath ] = '<strong>' + stylesheetid + '</strong>' + ( themeloaded ? ' ' + $.chldthmcfg.getxt( 'anlzsep' ) : '' )  + ' <small><code>' + stylesheetpath + '</code></small>';
        ...
        css_notice: function() {
                ...
                if ( self.analysis[ themetype ].signals.thm_has_wprocket && self.analysis[ themetype ].signals.thm_has_autoptimize ){
                    ...
                    if ( 'child' === themetype ) {
                        ...
                        if ( window.ctcAjax.copy_mods && window.ctcAjax.copy_mods.length > 1 ){
                            // console.log( 'copy theme mods set, resubmitting.' );
                            // resubmit = 1;
                        ...
                        if ( !self.analysis.child.signals.ctc_child_loaded &&
                            ...
                            // console.log( 'no child stylesheet detected, resubmitting.' );
                            // resubmit = 2;
                        }
                        ...
                        if ( !self.analysis.parnt.signals.thm_no_styles &&
                            ...
                            // console.log( 'no parent theme nor ignoreflag, resubmitting.' );
                            // resubmit = 2;
                        }
                    ...
                    if ( 'child' === themetype && self.analysis.child.signals.thm_has_plugins ) {
                        notice.notices.push( {
                            headline: $.chldthmcfg.getxt( 'plugin_css_headline' ),
                            msg: $.chldthmcfg.getxt( 'plugin_css_descr' ),
                            style: 'updated'
                        } );
                    ...
                    if ( self.analysis[ themetype ].signals.thm_ignoreparnt || self.analysis[ themetype ].signals.ctc_gen_loaded ){
                        ...
                        if ( !$( '#ctc_enqueue_none' ).is( ':checked' ) ) {
                            $( '#ctc_enqueue_none' ).prop( 'checked', true );
                            // console.log( 'ignore parent combined with handling method, resubmitting.' );
                            // resubmit = 2;
                    ...
                    if ( 'child' === themetype && self.analysis[ themetype ].signals.thm_parnt_loaded ) {
                        ...
                        //}
                        $( '#ctc_enqueue_none' ).prop( 'checked', true );
                        // console.log( 'no parent handling required, resubmitting.' );
                        // resubmit = 2;
                        resubmitdata.ctc_enqueue = 'none';
                    }
                    ...
                    if ( self.analysis.parnt.signals.thm_no_styles ) {
                        ...
                        //}
                        $( '#ctc_enqueue_none' ).prop( 'checked', true );
                        // console.log( 'no parent styles detected, resubmitting.' );
                        // resubmit = 2;
                        resubmitdata.ctc_enqueue = 'none';
                    }
                }
            ...
            if ( self.is_success() // && resubmit -- always resubmit to update configvars
                && !self.resubmitting ){
                //if ( resubmit === 2 ){
                ...
                /**
                 * v.2.3.0 display additional stylesheet options
                 */
                
                // console.log( self.parseable );
                // console.log( window.ctcAjax.addl_css );
                
                addl_inputs = '';
                $.each( self.parseable, function( ndx, el ){
                    addl_inputs += '<label><input class="ctc_checkbox ctc-themeonly" ' +
                    '" name="ctc_additional_css[]" type="checkbox" value="' + ndx + '" autocomplete="off" ' +
                    ( -1 !== $.inArray( ndx, window.ctcAjax.addl_css ) ? 'checked' : '' ) +
                    ' />' + el + "</label><br/>\n";
                    
                });
                if ( !addl_inputs.length ){
                    addl_inputs = '<p>' + $.chldthmcfg.getxt( 'noplugins' ) + '</p>';
                } 
                $( '#ctc_stylesheet_files' ).html( addl_inputs );
                ...
                if ( !$( '#ctc_child_type_reset' ).is( ':checked' ) ) {
                    $( '#input_row_stylesheet_handling_container,#input_row_parent_handling_container,#input_row_addl_css_container,#ctc_child_header_parameters,#ctc_configure_submit' ).slideDown( 'fast' );
                    if ( $( '#ctc_child_type_duplicate' ).is( ':checked' ) ) {
                        $( '#ctc_configure_submit .ctc-step' ).text( '9' );
                        $( '#ctc_copy_theme_mods' ).find( 'input' ).prop( 'checked', false );
                    } else {
                        $( '#ctc_configure_submit .ctc-step' ).text( '10' );
                ...
                $( '#ctc_stylesheet_files, #ctc_configure_submit_plugins' ).fadeIn();
        ...
        resubmit: function( data ) {
            ...
            data.ctc_fingerprint = $( '#ctc_fingerprint' ).val();

...
class ChildThemeConfiguratorCSS {
    ...
    var $dict_qs;           // query/selector mapping
    var $dict_rule;         // css rules
    var $dict_val;          // css values
    var $dict_seq;          // child load order (priority)
    //var $dict_token;
    // hierarchies
    var $val_ndx;           // query/selector => rule => value hierarchy mapping
    // key counters
    ...
    var $fp_stylesheet;     // used to check if styles changed since last update
    var $fp_editor;         // used to check if editor styles changed since last update
    ...
    */
    ...
    function ctc() {
        return ChildThemeConfiguratorCore::ctc();
    }
    ...
    function get_stylesheet_path() {
        $stylesheet = $this->get_child_target( $this->ctc()->get_child_stylesheet() );
        $this->fp_stylesheet = filesize( $stylesheet );
        $this->ctc()->debug( 'updated file size: ' . $this->fp_stylesheet, __FUNCTION__, __CLASS__ );
        return $stylesheet;
            ...
            switch ( $dict ):
                ...
                default:
                    ...
                    foreach ( $this->{ $dict } as $key => $val ):
                        if ( 'dict_sel' == $dict )
                            $this->dict_sel[ $key ] = //$this->tokenize( 
                            (string) $val;
                            //);
                        else
    ...
    function denorm_query_sel( $qsid ) {
        ...
        $qselarr = array(
            ...
            'selector'  => //$this->detokenize( 
                            $this->get_dict_value( 'sel', $s ),
                            //),
            'seq'       => $seq,
        );
        return $qselarr;
    ...
    function denorm_dict_qs( $query = NULL, $norm = TRUE ) {
        ...
        if ( $query ):
            ...
            foreach ( $selarr as $qsid => $qs ):
                // only return selectors that have property/value arrays
                if ( FALSE === ( $this->unpack_val_ndx( $qsid ) ) )
                    continue;
                // separate into query / selector
                list( $q, $s ) = explode( ':', $qs );
                if ( $norm ) // return qsid -> selector string map
                    $retarray[ $qsid ] = //$this->detokenize( 
                    $this->get_dict_value( 'sel', $s );
                    //);
                else // return selectorid -> qsid map
                    $retarray[ $s ] = $qsid;
            endforeach;
        else:
    ...
    function get_dict_id( $dict, $value ) {
        if ( FALSE === ( $id = $this->lookup_dict_value( $dict, $value ) ) ):
            $property = 'dict_' . $dict;
            // increment highest key
            $id = count( $this->{ $property } ) ? max( array_keys( $this->{ $property } ) ) + 1 : 1;
    ...
    function get_prop( $property, $params = NULL ) {
        switch ( $property ):
            case 'fp_stylesheet':
                return empty( $this->fp_stylesheet ) ? FALSE : $this->fp_stylesheet;
            case 'fp_editor':
                return empty( $this->fp_editor ) ? FALSE : $this->fp_editor;
            case 'converted':
            ...
            case 'properties':
                // these are loaded from codemirror list
                include( CHLD_THM_CFG_DIR . '/includes/views/properties.php' );
                return $property_keywords;
            case 'rules':
                // these are mapped to actual values
                $this->load_config( 'dict_rule' );
            ...
            case 'noswap': // v2.3.1
                return empty( $this->noswap ) ? array() : array_keys( $this->noswap );
            case 'parnt_deps':
            ...
            case 'sepstylesheet':
                return empty( $this->sepstylesheet ) ? 'ctc-style.css' : $this->sepstylesheet;
            case 'templates':
            ...
            case 'mpriority':
                return empty( $this->mpriority ) ? 20 : $this->mpriority;
            case 'parntloaded':
            ...
            case 'preview':
                ...
                if ( empty( $params[ 'key' ] ) || 'child' == $params[ 'key' ] ):
                    if ( 'inline' == $this->ctc()->get_mode() )
                        return $this->ctc()->state()->get_inline_css();
    ...
    function load_config( $key = NULL ) {
        ...
        if ( $key && isset( $this->dicts[ $key ] ) && !$this->dicts[ $key ] ):
            if ( !( $this->{ $key } = $this->ctc()->load_state( $key ) ) )
                $this->{ $key } = array();
            $this->dicts[ $key ] = 1; // flag as loaded
        endif;
        // initial setup
        if ( empty( $key ) && !isset( $this->enqueue ) ):
            if ( $configarray = $this->ctc()->load_state( 'configvars' ) ):
                // populate CSS class properties from configvars
    ...
    function pack_val_ndx( $qsid, $valarr ){
        if ( $valarr )
            $this->val_ndx[ $qsid ] = $this->ctc()->pack_data( $valarr, $qsid, isset( $this->val_ndx[ $qsid ] ) ? $this->val_ndx[ $qsid ] : NULL );
        /** moved to State
        try {
    ...
    function parse_css( $template, $basequery = NULL, $parse_imports = TRUE, $relpath = '', $reset = FALSE ) {
        ...
        // prepopulate rules
        if ( empty( $this->dict_rule ) ):
            foreach ( $this->get_prop( 'properties' ) as $property ):
                $ruleid = $this->get_dict_id( 'rule', $property );
            endforeach;
        endif;
        $ruleset = array();
    ...
    function parse_post_data() {
        ...
        elseif ( isset( $_POST[ 'ctc_child_imports' ] ) ):
            $this->ctc()->debug( 'parsing imports', __FUNCTION__, __CLASS__ );
            ...
            foreach ( preg_grep( '#^ctc_ovrd_child_seq_#', array_keys( $_POST ) ) as $post_key ):
                if ( preg_match( '#^ctc_ovrd_child_seq_(\d+)$#', $post_key, $matches ) ):
                    ...
                    if ( $seq != $qsid ):
                        ...
                        $this->unset_dict_value( 'dict_seq', $seq );
                    endif;
                endif;
            endforeach;
    ...
    function prune_if_empty( $qsid ) {
        ...
        if ( FALSE === ( $valndxarr = $this->unpack_val_ndx( $qsid ) ) ) 
            return FALSE;
        foreach ( $valndxarr as $ruleid => $arr ):
            ...
            endforeach;
        endforeach;
        // no values, prune from sel index and val index dict data ( keep other dictionary records )
        $this->unset_dict_value( 'val_ndx', $qsid );
        $this->unset_dict_value( 'dict_seq', $qsid );
        return TRUE;
    }
    ...
    function read_stylesheet( $template = 'child', $file = 'style.css' ) {
        ...
        // v.2.3.0 - using existing functions
        // chld_thm_cfg_parnt filter swaps plugin path if plugin stylesheet file exists
        $stylesheet = 'child' == $template ? $this->get_child_target( $file ) 
            : apply_filters( 'chld_thm_cfg_parnt', $this->get_parent_source( $file ), $file );
        /*
        ...
        $stylesheet = apply_filters( 'chld_thm_cfg_' . $template, trailingslashit( $themedir ) 
            . $file, $file ); // support for plugins extension < 2.0
        */
        
        
        // load styles after verifying file is okay to read
        if ( $stylesheet_verified = $this->is_file_ok( $stylesheet, 'read' ) ):
            $this->ctc()->debug( 'Stylesheet verified: ' . $stylesheet_verified, __FUNCTION__, __CLASS__ );
            /**
            ...
            */
            ...
            $this->ctc()->debug( 'Stylesheet INVALID: ' . $stylesheet, __FUNCTION__, __CLASS__ );
        endif;
    }
    ...
    function save_config() {
        ...
        $this->ctc()->save_state( 'configvars', $configarray );
        foreach ( $this->dicts as $configkey => $loaded ):
            if ( $loaded ): // only save if dict is loaded
                $this->ctc()->debug(  'saving dict: ' . $configkey, __FUNCTION__, __CLASS__ );
                $this->ctc()->save_state( $configkey, $this->{$configkey} );
            endif;
        endforeach;
    ...
    function unpack_val_ndx( $qsid ){
        return $this->ctc()->unpack_data( isset( $this->val_ndx[ $qsid ] ) ? $this->val_ndx[ $qsid ] : NULL, $qsid );
            /** moved to State
            try {
    ...
    function unset_dict_value( $dict, $id ){
        unset( $this->{ $dict }[ $id ] );
        if ( isset( $this->ctc()->state()->{ $dict } ) )
            unset( $this->ctc()->state()->{ $dict }[ $id ] );
    }
    ...
    function write_css() {
        ...
        // retrieve current editing mode from user state
        $mode = $this->ctc()->get_mode();
        /**
         * if using custom css target mode, switch to inline css function
         */
        if ( 'inline' == $mode ):
            $this->ctc()->state()->update_inline_css( $output );
            return TRUE;
        /**
         * otherwise, continue...
         */
            ...
            // $stylesheet = $this->get_stylesheet_path(); // moved to state
            $stylesheet = $this->get_child_target( $this->ctc()->get_child_stylesheet() );
        endif;
        ...
        if ( $stylesheet_verified = $this->is_file_ok( $stylesheet, 'write' ) ):
            ...
            $chmod = 'direct' == $this->ctc()->fs_method ? FALSE : 0666;
            ...
            elseif ( FALSE === $wp_filesystem->put_contents( $this->ctc()->fspath( $stylesheet_verified ), $output, $chmod ) ):
            ...
            $hash = md5_file( $stylesheet_verified );
            $property = 'stylesheet' == $mode ? 'fp_stylesheet' : 'fp_editor';
            $this->ctc()->debug( 'Setting fingerprint: ' . $property . ' ' . $hash, __FUNCTION__, __CLASS__ );
            $this->set_prop( $property, $hash );
            return TRUE;
        endif;   
        return FALSE;
    }

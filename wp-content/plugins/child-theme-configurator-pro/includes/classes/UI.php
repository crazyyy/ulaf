...
class ChildThemeConfiguratorUI {
    ...
    function __construct() {
        ...
        //add_filter( 'chld_thm_cfg_localize_array',      array( $this, 'filter_localize_array' ) ); // v.2.3.0 - no longer using pluginmode
    ...
    function css() {
        return ChildThemeConfiguratorCore::css();
    }
    ...
    function render() {
        ...
        add_thickbox();
        include ( CHLD_THM_CFG_DIR . '/includes/views/main.php' ); 
    } 
    ...
    function configured(){
        return $this->ctc()->get_theme_count( 'child', TRUE ) ? 1 : 0;
    }
    ...
    function render_file_form_buttons( $template ){
        include ( CHLD_THM_CFG_DIR . '/includes/views/file-form-buttons.php' ); 
    }
    
    function render_theme_menu( $template = 'child', $selected = NULL, $id = 'ctc_theme_child', $config = FALSE ) {
        $themes = $this->ctc()->get_theme_group( $template, $config );
        include ( CHLD_THM_CFG_DIR . '/includes/views/theme-menu.php' ); 
    }
    ...
    function render_file_form( $template = 'parnt' ) {
        ...
        if ( $theme = $this->ctc()->css()->get_prop( $template ) ):
            ...
            foreach ( $files as $file ):
                ...
                include ( CHLD_THM_CFG_DIR . '/includes/views/file.php' );            
            endforeach;
            $inputs = ob_get_contents();
            ob_end_clean();
            if ( $counter ):
                include ( CHLD_THM_CFG_DIR . '/includes/views/fileform.php' );            
            endif;
        else:
    ...
    function render_image_form() {
        ...
        if ( $theme = $this->ctc()->css()->get_prop( 'child' ) ):
            ...
            foreach ( $files as $file ):
                ...
                include( CHLD_THM_CFG_DIR . '/includes/views/image.php' );             
            endforeach;
            $inputs = ob_get_clean();
            if ( $counter ) include( CHLD_THM_CFG_DIR . '/includes/views/images.php' );
        endif;
    }
    
    function render_settings_errors() {
        include ( CHLD_THM_CFG_DIR . '/includes/views/settings-errors.php' ); 
    }
    ...
    function render_addl_tabs( $active_tab = NULL, $hidechild = '', $enqueueset = TRUE ) {
        include ( CHLD_THM_CFG_DIR . '/includes/views/addl_tabs.php' );            
    }

    function render_addl_panels( $active_tab = NULL, $hidechild = '', $enqueueset = TRUE ) {
        include ( CHLD_THM_CFG_DIR . '/includes/views/addl_panels.php' );            
    }

    function render_lilaea_plug() {
        include ( CHLD_THM_CFG_DIR . '/includes/views/related.php' );
    }
    ...
    function render_notices( $msg ) { 
        include ( CHLD_THM_CFG_DIR . '/includes/views/notices.php' );
    }
    ...
    function enqueue_scripts() {
        $raw_css_settings = wp_enqueue_code_editor(
            array(
                'type'       => 'text/css',
                'codemirror' => array(
                    'indentUnit' => 2,
        ...
        $localize_array = apply_filters( 'chld_thm_cfg_localize_array', array(
            'enqueue_is_set'            => $this->enqueue_is_set(),
            'raw_css_settings'          => $raw_css_settings,
            ...
            'themes'                    => $this->ctc()->get_themes(),
            'source'                    => get_theme_root_uri() . '/' 
                . $this->css()->get_prop( 'parnt' ) . '/style.css',
            'target'                    => get_theme_root_uri() . '/' 
                . $this->css()->get_prop( 'child' ) . '/style.css',
            ...
            'noswap'                    => $this->css()->get_prop( 'noswap' ),
            ...
            'anlzsep_txt'               => __( '(requires "Separate" stylesheet option)', 'child-theme-configurator' ),

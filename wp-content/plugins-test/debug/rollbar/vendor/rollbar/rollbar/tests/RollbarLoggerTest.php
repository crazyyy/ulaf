...
/** @internal */
class RollbarLoggerTest extends \Rollbar\BaseRollbarTest
{
    ...
    {
        $logger = new \Rollbar\RollbarLogger(array("access_token" => $this->getTestAccessToken(), "environment" => "testing-php"));
        $logger->addCustom("foo", "bar");
        $dataBuilder = $logger->getDataBuilder();
        $result = $dataBuilder->makeData(Level::INFO, "This test message should have custom data attached.", array());
        $customData = $result->getCustom();
        $this->assertEquals("bar", $customData["foo"]);
        $logger = new \Rollbar\RollbarLogger(array("access_token" => $this->getTestAccessToken(), "environment" => "testing-php", "custom" => array("baz" => "xyz")));
        $logger->addCustom("foo", "bar");
        $dataBuilder = $logger->getDataBuilder();
        $result = $dataBuilder->makeData(Level::INFO, "This test message should have custom data attached.", array());
        $customData = $result->getCustom();
    ...
    {
        $logger = new \Rollbar\RollbarLogger(array("access_token" => $this->getTestAccessToken(), "environment" => "testing-php", "custom" => array("foo" => "bar", "bar" => "xyz")));
    ...
    {
        $l = new \Rollbar\RollbarLogger(array("access_token" => $this->getTestAccessToken(), "environment" => "testing-php"));
    ...
    public function testLog() : void
    {
        ...
        $logger = new \Rollbar\RollbarLogger(["access_token" => $this->getTestAccessToken(), "environment" => "testing-php", "verbose_logger" => $verbose]);
    ...
    public function testNotLoggingPayload() : void
    {
        ...
        $logger = new \Rollbar\RollbarLogger(["access_token" => $this->getTestAccessToken(), "environment" => "testing-php", "log_payload" => \false, "log_payload_logger" => $logPayloadLoggerMock, "verbose_logger" => $verbose]);
        $this->assertSame(0, $verbose->count(Level::INFO, 'Occurrence successfully logged'));
        $logger->log(Level::WARNING, "Testing PHP Notifier", []);
        $this->assertSame(1, $verbose->count(Level::INFO, 'Attempting to log: [' . Level::WARNING . '] Testing PHP Notifier'));
        $this->assertSame(1, $verbose->count(Level::INFO, 'Occurrence successfully logged'));
    }
    public function testReport() : void
    {
        $logger = new \Rollbar\RollbarLogger(["access_token" => $this->getTestAccessToken(), "environment" => "testing-php"]);
    ...
    {
        $test = $this;
        $logger = new \Rollbar\RollbarLogger(["access_token" => $this->getTestAccessToken(), "environment" => "testing-php", 'check_ignore' => function ($isUncaught) use($test) {
            $test::assertTrue($isUncaught);
        }]);
        $response = $logger->report(Level::WARNING, "Testing PHP Notifier", isUncaught: \true);
        $this->assertEquals(200, $response->getStatus());
    }
    public function testReportTelemetry() : void
    {
        // Init used so that the telemeter is initialized.
        \Rollbar\Rollbar::init(["access_token" => $this->getTestAccessToken(), "environment" => "testing-php"]);
        \Rollbar\Rollbar::logger()->report(Level::WARNING, "Testing PHP Notifier", isUncaught: \true);
        $events = \Rollbar\Rollbar::getTelemeter()->copyEvents();
    ...
    {
        $logger = new \Rollbar\RollbarLogger(array("access_token" => $this->getTestAccessToken(), "environment" => "testing-php", "verbose" => \Rollbar\Config::VERBOSE_NONE));
        $this->assertInstanceOf(NoopHandler::class, $logger->verboseLogger()->getHandlers()[0]);
    }
    public function testVerbose() : void
    {
        $logger = new \Rollbar\RollbarLogger(array("access_token" => $this->getTestAccessToken(), "environment" => "testing-php", "verbose" => \RollbarWP\Psr\Log\LogLevel::DEBUG));
    ...
    public function testEnabled() : void
    {
        ...
        $this->assertEquals(200, $response->getStatus());
        $logger = new \Rollbar\RollbarLogger(array("access_token" => $this->getTestAccessToken(), "environment" => "testing-php", "enabled" => \false));
        $response = $logger->report(Level::WARNING, "Testing PHP Notifier", array());
    ...
    public function testTransmit() : void
    {
        ...
        $response = $logger->report(Level::WARNING, "Testing PHP Notifier");
        $this->assertEquals(200, $response->getStatus());
        $logger = new \Rollbar\RollbarLogger(array("access_token" => $this->getTestAccessToken(), "environment" => "testing-php", "transmit" => \false));
        $response = $logger->report(Level::WARNING, "Testing PHP Notifier");
        $this->assertEquals(0, $response->getStatus());
    ...
    public function testTransmitBatched() : void
    {
        ...
        // transmit on (default)
        $logger = new \Rollbar\RollbarLogger(array("access_token" => $this->getTestAccessToken(), "environment" => "testing-php", "batched" => \true, "sender" => $senderMock));
        $response = $logger->report(Level::WARNING, "Testing PHP Notifier");
        $this->assertEquals(0, $response->getStatus());
        ...
        $logger->configure(array('transmit' => \false, 'sender' => $senderMock));
        $response = $logger->report(Level::WARNING, "Testing PHP Notifier");
        $this->assertEquals(0, $response->getStatus());
        $this->assertEquals("Pending", $response->getInfo());
    ...
    {
        $logger = new \Rollbar\RollbarLogger(array("access_token" => $this->getTestAccessToken(), "environment" => "testing-php", "transformer" => MalformedPayloadDataTransformer::class));
        $response = $logger->report(Level::ERROR, "Forced payload's data to false value.", array());
        $this->assertEquals(422, $response->getStatus());
    }
    public function testFlush() : void
    {
        $senderMock = $this->getMockBuilder(SenderInterface::class)->getMock();
        $senderMock->expects($this->once())->method('sendBatch')->with($this->containsOnlyInstancesOf(EncodedPayload::class), $this->anything());
        $logger = new \Rollbar\RollbarLogger(array("access_token" => $this->getTestAccessToken(), "environment" => "testing-php", "batched" => \true, "sender" => $senderMock));
        $response = $logger->flush();
        $this->assertEquals(0, $response->getStatus());
    ...
    {
        $l = new \Rollbar\RollbarLogger(array("access_token" => $this->getTestAccessToken(), "environment" => "testing-php"));
        $response = $l->report(Level::ERROR, new \Exception("Testing PHP Notifier"), array("foo" => "bar"));
        $this->assertEquals(200, $response->getStatus());
    }
    ...
    {
        $l = new \Rollbar\RollbarLogger(array("access_token" => $this->getTestAccessToken(), "environment" => "testing-php", "error_sample_rates" => array(\E_ERROR => 0)));
        $response = $l->report(Level::ERROR, new \Rollbar\ErrorWrapper(\E_ERROR, '', null, null, array(), new \Rollbar\Utilities()), array());
        $this->assertEquals(0, $response->getStatus());
    }
    public function testExceptionSampleRates() : void
    {
        $l = new \Rollbar\RollbarLogger(array("access_token" => "ad865e76e7fb496fab096ac07b1dbabb", "environment" => "testing-php", "exception_sample_rates" => array(SilentExceptionSampleRate::class => 0.0)));
    ...
    {
        $l = new \Rollbar\RollbarLogger(array("access_token" => $this->getTestAccessToken(), "environment" => "testing-php", "included_errno" => \E_ERROR | \E_WARNING));
        $response = $l->report(Level::ERROR, new \Rollbar\ErrorWrapper(\E_USER_ERROR, '', null, null, array(), new \Rollbar\Utilities()), array());
        $this->assertEquals(0, $response->getStatus());
    }
    private function scrubTestHelper($config = array(), $context = array())
    {
        $scrubFields = array('sensitive');
        $defaultConfig = array('access_token' => $this->getTestAccessToken(), 'environment' => 'tests', 'scrub_fields' => $scrubFields);
        $config = new \Rollbar\Config(\array_replace_recursive($defaultConfig, $config));
     ...
     */
    private function scrubTestAssert(string $dataName, array $result, string $scrubField = 'sensitive', bool $recursive = \true, string $replacement = '*') : void
    {
        $this->assertEquals(\str_repeat($replacement, 8), $result[$scrubField], "{$dataName} did not get scrubbed.");
        if ($recursive) {
            $this->assertEquals(\str_repeat($replacement, 8), $result['recursive'][$scrubField], "{$dataName} did not get scrubbed recursively.");
        }
    }
    public static function scrubDataProvider() : array
    {
        return array(array(
            ...
            array(
                ...
                'sensitive' => 'value 2',
                'recursive' => array('sensitive' => 'value 1', 'nonsensitive' => 'value 2'),
            ),
        ));
    }
    /**
    ...
    public function testGetScrubbedHeaders() : void
    {
        ...
        $this->scrubTestAssert(
            ...
            $scrubField,
            // field names in headers are slight different convenstion
            \false
        );
    }
    /**
    ...
    {
        $extras = array('extraField1' => $testData);
        $result = $this->scrubTestHelper(array('requestExtras' => $extras));
        $this->scrubTestAssert("Request extras", $result['data']['request']['extraField1']);
    }
    /**
     * @dataProvider scrubDataProvider
     */
    public function testMakeDataScrubServerExtras($testData) : void
    {
        ...
        $this->scrubTestAssert("Server extras", $result['data']['server']['extraField1']);
    }
    /**
     * @dataProvider scrubDataProvider
     */
    public function testMakeDataScrubCustom($testData) : void
    {
        ...
        $this->scrubTestAssert("Custom", $result['data']['custom']);
    }
    /**
     * @dataProvider scrubDataProvider
     */
    ...
    {
        $testData['id'] = '123';
        $result = $this->scrubTestHelper(array('person' => $testData, 'scrub_safelist' => array('data.person.recursive.sensitive')));
        $this->assertEquals(\str_repeat('*', 8), $result['data']['person']['sensitive'], "Person did not get scrubbed.");
        $this->assertNotEquals(\str_repeat('*', 8), $result['data']['person']['recursive']['sensitive'], "Person recursive.sensitive DID get scrubbed even though it's safelisted.");
    }
    /**
     * @dataProvider scrubDataProvider
     */
    ...
    {
        $bodyContext = array('context1' => $testData);
        $result = $this->scrubTestHelper(array('custom' => $bodyContext), $bodyContext);
        $this->scrubTestAssert("Request body context", $result['data']['body']['extra']['context1']);
    }
    ...
    public function testGetUrlScrub($testData) : void
    {
        ...
        $this->scrubTestAssert("Url", $parsed, 'sensitive', \true, 'x');
    }
    /**
     * @dataProvider scrubQueryStringDataProvider
     */
    public function testGetRequestScrubQueryString($testData) : void
    {
        ...
        $this->scrubTestAssert("Query string", $parsed, 'sensitive', \true, 'x');
    }
    /**
     * @testWith ["emergency"]
     *           ["alert"]
    ...
    public function testMaxItems($maxItemsConfig) : void
    {
        ...
        $maxItems = $maxItemsConfig ?? \Rollbar\Defaults::get()->maxItems();
        ...
        $response = $logger->report(Level::INFO, 'testing info level');
        $this->assertEquals(0, $response->getStatus());
        $this->assertEquals("Maximum number of items per request has been reached. If you " . "want to report more items, please use `max_items` " . "configuration option.", $response->getInfo());
    }
    public static function maxItemsProvider() : array
    {
        return array('use default max_items' => array(null), 'use provided max_items' => array(3));
    }
    public function testRaiseOnError() : void
    {
        $logger = new \Rollbar\RollbarLogger(array("access_token" => $this->getTestAccessToken(), "environment" => 'test', "raise_on_error" => \true));

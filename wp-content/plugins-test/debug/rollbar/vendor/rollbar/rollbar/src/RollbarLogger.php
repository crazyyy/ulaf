...
use Exception;
use RollbarWP\Psr\Log\InvalidArgumentException;
...
use Stringable;
use Throwable;
use RollbarWP\Psr\Log\AbstractLogger;
...
class RollbarLogger extends AbstractLogger
{
    ...
    {
        $this->config = new \Rollbar\Config($config);
     ...
     * @since 3.0
     */
    public function getConfig() : \Rollbar\Config
    {
        return $this->config;
    }
    /**
     ...
     * @throws Exception
     */
    public function scope(array $config) : \Rollbar\RollbarLogger
    {
        return new \Rollbar\RollbarLogger($this->extend($config));
    }
    /**
     ...
     */
    public function report(string|Level $level, string|Stringable $message, array $context = array(), bool $isUncaught = \false) : \Rollbar\Response
    {
        if ($this->disabled()) {
            $this->verboseLogger()->notice('Rollbar is disabled');
            return new \Rollbar\Response(0, "Disabled");
        }
        ...
        if ($level instanceof Level) {
            $level = (string) $level;
        } elseif (!\Rollbar\LevelFactory::isValidLevel($level)) {
        ...
        $telemeter = \Rollbar\Rollbar::getTelemeter();
        ...
        if ($ignored) {
            ...
            return new \Rollbar\Response(0, "Ignored");
        }
        if ($this->config->checkIgnored($payload, $message, $isUncaught)) {
            $this->verboseLogger()->info('Occurrence ignored');
            $response = new \Rollbar\Response(0, "Ignored");
        } else {
        ...
        if ($response->getStatus() === 0) {
            ...
            $this->verboseLogger()->error('Occurrence rejected by the API: with status ' . $response->getStatus() . ': ' . ($info['message'] ?? 'message not set'));
        } else {
     ...
     * @since 4.1.0
     */
    public function captureTelemetryEvent(EventType $type, EventLevel $level, array|TelemetryBody $metadata, ?string $uuid = null, ?int $timestamp = null) : ?TelemetryEvent
    {
        return \Rollbar\Rollbar::getTelemeter()?->capture($type, $level, $metadata, $uuid, $timestamp);
    }
    /**
     ...
     */
    public function flush() : ?\Rollbar\Response
    {
        ...
        return new \Rollbar\Response(0, "Queue empty");
    }
    /**
     ...
     * @return Response
     */
    protected function send(EncodedPayload $payload, string $accessToken) : \Rollbar\Response
    {
        if ($this->reportCount >= $this->config->getMaxItems()) {
            $response = new \Rollbar\Response(0, "Maximum number of items per request has been reached. If you " . "want to report more items, please use `max_items` " . "configuration option.");
        ...
        if ($this->config->getBatched()) {
            $response = new \Rollbar\Response(0, "Pending");
     ...
     * @return Payload
     */
    protected function getPayload(string $accessToken, string $level, string|Stringable $toLog, array $context) : Payload
    {
     ...
     * @return DataBuilder
     */
    public function getDataBuilder() : \Rollbar\DataBuilder
    {
     ...
     * @return void
     */
    protected function handleResponse(Payload $payload, \Rollbar\Response $response) : void
    {

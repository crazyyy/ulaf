...
/** @internal */
class CurlSender implements \Rollbar\Senders\SenderInterface
{
    use UtilitiesTrait;
    private $endpoint;
    private $timeout;
    ...
    public function __construct($opts)
    {
        ...
        if (\array_key_exists('ip_resolve', $opts) && \in_array($opts['ip_resolve'], [\CURL_IPRESOLVE_V4, \CURL_IPRESOLVE_V6, \CURL_IPRESOLVE_WHATEVER])) {
    ...
    public function send(EncodedPayload $payload, string $accessToken) : Response
    {
        ...
        $result = $result === \false ? \curl_error($handle) : \json_decode($result, \true);
        $data = $payload->data();
    ...
    private function maybeSendMoreBatchRequests(string $accessToken)
    {
        ...
        for (; $idx < $len && $idx < $max; $idx++) {
            ...
            $this->inflightRequests[$handleArrayKey] = \true;
        }

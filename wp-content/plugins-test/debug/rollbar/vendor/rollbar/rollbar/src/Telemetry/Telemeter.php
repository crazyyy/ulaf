...
class Telemeter
{
     ...
     */
    public function __construct(int $maxTelemetryEvents = self::MAX_EVENTS, private ?\Rollbar\Telemetry\TelemetryFilterInterface $filter = null, private bool $includeItemsInTelemetry = \true, private bool $includeIgnoredItemsInTelemetry = \false)
    {
     ...
     * @return EventType
     */
    private static function getTypeFromLevel(string $level) : \Rollbar\Telemetry\EventType
    {
        return match ($level) {
            Level::EMERGENCY, Level::ALERT, Level::CRITICAL, Level::ERROR, Level::WARNING => \Rollbar\Telemetry\EventType::Error,
            Level::NOTICE, Level::INFO => \Rollbar\Telemetry\EventType::Log,
            default => \Rollbar\Telemetry\EventType::Manual,
        };
    }
    /**
     ...
     * @return EventLevel
     */
    public static function getLevelFromPsrLevel(string $level) : \Rollbar\Telemetry\EventLevel
    {
        return match ($level) {
            Level::EMERGENCY, Level::ALERT, Level::CRITICAL => \Rollbar\Telemetry\EventLevel::Critical,
            Level::ERROR => \Rollbar\Telemetry\EventLevel::Error,
            Level::WARNING => \Rollbar\Telemetry\EventLevel::Warning,
            Level::DEBUG => \Rollbar\Telemetry\EventLevel::Debug,
            default => \Rollbar\Telemetry\EventLevel::Info,
        };
    }
    /**
     ...
     * @return void
     */
    public function scope(int $maxTelemetryEvents = self::MAX_EVENTS, ?\Rollbar\Telemetry\TelemetryFilterInterface $filter = null, bool $includeItemsInTelemetry = \true, bool $includeIgnoredItemsInTelemetry = \false) : void
    {
     ...
     */
    public function capture(\Rollbar\Telemetry\EventType $type, \Rollbar\Telemetry\EventLevel $level, array|TelemetryBody $metadata, ?string $uuid = null, ?int $timestamp = null) : ?TelemetryEvent
    {
        if ($this->maxQueueSize === 0) {
            return null;
        }
     ...
     *                             was filtered out.
     */
    public function captureError(array|string|ErrorWrapper|Throwable $error, \Rollbar\Telemetry\EventLevel $level = \Rollbar\Telemetry\EventLevel::Error, ?string $uuid = null, ?int $timestamp = null) : ?TelemetryEvent
    {
        if (\is_string($error)) {
            return $this->capture(\Rollbar\Telemetry\EventType::Error, $level, new TelemetryBody(message: $error), $uuid, $timestamp);
        }
        if ($error instanceof ErrorWrapper) {
            $metadata = new TelemetryBody(message: $error->getMessage(), subtype: 'error', stack: $this->stringifyBacktrace($error->getBacktrace()));
            return $this->capture(\Rollbar\Telemetry\EventType::Error, $level, $metadata, $uuid, $timestamp);
        }
        if ($error instanceof Throwable) {
            $metadata = new TelemetryBody(message: $error->getMessage(), subtype: 'exception', stack: $this->stringifyBacktrace($error->getTrace()));
            return $this->capture(\Rollbar\Telemetry\EventType::Error, $level, $metadata, $uuid, $timestamp);
        }
        return $this->capture(\Rollbar\Telemetry\EventType::Error, $level, $error, $uuid, $timestamp);
    }
    /**
     ...
     * @return TelemetryEvent|null
     */
    public function captureLog(string $message, \Rollbar\Telemetry\EventLevel $level = \Rollbar\Telemetry\EventLevel::Info, ?string $uuid = null, ?int $timestamp = null) : ?TelemetryEvent
    {
        return $this->capture(\Rollbar\Telemetry\EventType::Log, $level, new TelemetryBody(message: $message), $uuid, $timestamp);
    }
    /**
     ...
     * @return TelemetryEvent|null
     */
    public function captureNetwork(string $method, string $url, string $status_code, \Rollbar\Telemetry\EventLevel $level = \Rollbar\Telemetry\EventLevel::Info, ?string $uuid = null, ?int $timestamp = null) : ?TelemetryEvent
    {
        return $this->capture(type: \Rollbar\Telemetry\EventType::Log, level: $level, metadata: new TelemetryBody(method: $method, url: $url, status_code: $status_code), uuid: $uuid, timestamp: $timestamp);
    }
    /**
     ...
     * @return TelemetryEvent|null
     */
    public function captureNavigation(string $from, string $to, \Rollbar\Telemetry\EventLevel $level = \Rollbar\Telemetry\EventLevel::Info, ?string $uuid = null, ?int $timestamp = null) : ?TelemetryEvent
    {
        return $this->capture(\Rollbar\Telemetry\EventType::Log, $level, new TelemetryBody(from: $from, to: $to), $uuid, $timestamp);
    }
    /**
     ...
     */
    public function captureRollbarItem(string $level, string|Stringable|Throwable $message, array $context = [], bool $ignored = \false, ?string $uuid = null) : ?TelemetryEvent
    {
        ...
        return $this->capture(type: self::getTypeFromLevel($level), level: self::getLevelFromPsrLevel($level), metadata: new TelemetryBody(message: $this->getRollbarItemMessage($message)), uuid: $uuid);
    }
    /**
     ...
     */
    public function getFilter() : ?\Rollbar\Telemetry\TelemetryFilterInterface
    {
        return $this->filter;
    }
    /**
     ...
     *                                              the queue.
     */
    public function setFilter(?\Rollbar\Telemetry\TelemetryFilterInterface $filter, bool $apply = \true) : void
    {
        $this->filter = $filter;

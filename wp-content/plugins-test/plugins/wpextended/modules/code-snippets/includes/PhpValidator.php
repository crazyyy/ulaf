...
class PhpValidator
{
    ...
    /**
     * @since 3.1.0
     * @return string|array<string|int>|null
     */
    ...
    public function validate()
    {
        while (!$this->end()) {
            ...
            if ($this->end()) {
                return new \WP_Error(
                    'parse_error',
                    __('Parse error: syntax error, unexpected end of snippet.', WP_EXTENDED_TEXT_DOMAIN),
                    array(
                        'line' => $token[2],
                    )
                );
                ...
                if ($this->check_duplicate_identifier($structure_type, $token[1])) {
                    switch ($structure_type) {
                        case T_FUNCTION:
                            ...
                            $message = __('Cannot redeclare function %s.', WP_EXTENDED_TEXT_DOMAIN);
                            break;
                        case T_CLASS:
                            /* translators: %s: PHP class name */
                            $message = __('Cannot redeclare class %s.', WP_EXTENDED_TEXT_DOMAIN);
                            break;
                        case T_INTERFACE:
                            /* translators: %s: PHP interface name */
                            $message = __('Cannot redeclare interface %s.', WP_EXTENDED_TEXT_DOMAIN);
                            break;
                        default:
                            /* translators: %s: PHP identifier name*/
                            $message = __('Cannot redeclare %s.', WP_EXTENDED_TEXT_DOMAIN);
                    }
    ...
    /**
     * Perform a non-executing runtime check using token_get_all.
     *
     * @since 3.1.0
    ...
    public function checkRunTimeError()
    {
        ...
        // Only check for syntax errors without executing
        try {
            // Use token_get_all to check syntax without execution
            token_get_all($code);
            return true;
        } catch (\ParseError $parse_error) {
            return new \WP_Error('runtime_error', $parse_error->getMessage(), array(

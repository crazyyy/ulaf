...
class Adminify_Pro
{
    ...
    {
        // Move to init hook for WP 6.7+ textdomain compatibility
        // Priority order: 0=textdomain (main), 0=pro dependencies (runs after textdomain since added later), 1=main plugin files, 3=pro classes, 5=framework setup
        // Pro dependencies must load at priority 0 to register filters BEFORE main plugin's priority 1 creates AdminSettings
        add_action('init', array($this, 'jltwp_adminify_dependencies'), 0);
        add_action('init', array($this, 'jltwp_adminify_includes_classes'), 3);
    ...
    public static function adminify_pro_activation_hook(){
        $plugin_option = AdminSettings::get_instance()->get();
        // White Label Options
        if (jltwp_adminify()->is_plan('agency')) {
            $plugin_option['white_label']['adminify']['plugin_option'] = false;

(function($) {
    ...
    let ACFFCM = {
        ...
        addModal: function($layout) {
            ...
            let edit = $('<a class="acf-icon -pencil small light" href="#" data-event="edit-layout" title="Edit layout" />');
        ...
        open: function() {
            ...
            let $layout = $(this).parents('.layout:first');

            let caption = $layout.find('> .acf-fc-layout-handle').html();
            let a = $('<a class="dashicons dashicons-no -cancel" />').on('click', ACFFCM.close);
        ...
        close: function() {
            ...
            let $layout = ACFFCM.modals.pop();

            // Refresh layout title

            let fc = $layout.parents('.acf-field-flexible-content:first');
            let field = acf.getInstance(fc);
        ...
        overlay: function(show) {
            ...
            if(show === true && !$('body').hasClass('acf-modal-open')) {
                ...
                let overlay = $('<div id="acf-flexible-content-modal-overlay" />').on('click', ACFFCM.close);
        ...
        refresh: function() {
            ...
            let index = ACFFCM.modals.length - 1;

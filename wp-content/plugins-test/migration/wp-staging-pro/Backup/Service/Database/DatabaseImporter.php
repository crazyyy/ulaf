    ...
    private function execute()
    {
        ...
        if ($result === false) {
            switch ($this->client->errno()) {
                ...
                case 1709:
                    ...
                    if ($replaceUtf8Mb4 && $result) {
                        $this->logger->warning(esc_html__('Encoding changed to UTF8 from UTF8MB4, as your current MySQL version max key length support is 767 bytes', 'wp-staging'));
                    }
                ...
                case 1059:
                    ...
                    if ($result) {
                        foreach ($shortIdentifiers as $shortIdentifier => $identifier) {
                            $this->logger->warning(sprintf(__('Key identifier `%s` exceeds the characters limits, it is now shortened to `%s` to continue restoring.', 'wp-staging'), $identifier, $shortIdentifier));
                        }
                    }

                    break;
                case 1813:
                    throw new RuntimeException(__('Could not restore the database. MySQL returned the error code 1813, which is related to a tablespace error that WP STAGING can\'t handle. Please contact your hosting company.', 'wp-staging'));
            }
            ...
            if (defined('WPSTG_DEBUG') && WPSTG_DEBUG) {
                $this->logger->warning(__(sprintf('Database Restorer - Failed Query: %s', substr($query, 0, 1000)), 'wp-staging'));
                debug_log(__(sprintf('Database Restorer Failed Query: %s', substr($query, 0, 1000)), 'wp-staging'));
            }
            ...
            throw new RuntimeException(__(sprintf('Could not restore query. MySQL has returned the error code %d, with message "%s".', $errorNo, $errorMsg)) . $additionalInfo);
        }
    }

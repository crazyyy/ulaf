...
class WP_Optimize_Premium {
	...
	public function init() {
		...
		try {
			...
			$updraft_updater_instance = new Updraft_Manager_Updater_1_9('https://teamupdraft.com/plugin-info/', 2, 'wp-optimize-premium/wp-optimize.php', array('require_login' => false));
		} catch (Exception $e) {
			error_log($e->getMessage().' at '.$e->getFile().' line '.$e->getLine()); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Catching exception for debugging purpose
		}
	...
	public function loggers_classes($loggers_classes) {
		...
		if (function_exists('openlog') && function_exists('syslog')) {
			$premium_classes['Updraft_Syslog_Logger'] = WPO_PLUGIN_MAIN_PATH.'includes/class-updraft-syslog-logger.php';
		}

		if (class_exists('SimpleHistory')) {
			$premium_classes['Updraft_Simple_History_Logger'] = WPO_PLUGIN_MAIN_PATH.'includes/class-updraft-simple-history-logger.php';
		}
	...
	/**
	 * Whether to show the innodb_force_optimize_single checkbox
	 *
	...
	public function wpo_additional_options() {
		...
		$is_all_selected = is_array($sites_options) && in_array('all', $sites_options);
		?>
					...
					<ul>
						...
						foreach ($sites as $site) {
							...
							<li>
								...
								<label for="<?php echo esc_attr($dom_id); ?>"><?php echo esc_html($site->domain.$site->path) . (1 === (int) $site->blog_id ? '<em>('.esc_html__('The network-wide tables will be optimized with this site', 'wp-optimize').')</em>' : ''); ?></label>
							</li>
						<?php } ?>
					</ul>
				</div>
	 ...
	 * @return array $admin_page_tab modified $admin_page_tabs which doesn't have may_also key element
	 */
	...
	public function wpo_js_translations($translations) {
		$premium_translations = array(
			...
			'no_unused_image_sizes' => __('You have no unused image sizes.', 'wp-optimize'),
	...
	/**
	 * Automatic scheduled optimization default options
	 *
	 * @param array $new_auto_options
	 * @return array
	 */
	...
	public function wpo_faq_url() {
		return 'https://teamupdraft.com/documentation/wp-optimize/';
	}

	/**
	 * Include lazy load files, if needed and add action to output script.
	 */
	...
	/**
	 * Show additional settings in the general settings page.
	 */
	...
	public function get_purge_cache_permissions() {
		$options = WP_Optimize()->get_options();
		return (array) $options->get_option('purge-cache-permissions', array('administrator'));
	}
	...
	public function handle_unused_images_csv() {
		$unused_images_csv = TeamUpdraft\WP_Optimize\Includes\Fragments\fetch_superglobal('get', 'wpo_unused_images_csv');

		if ($unused_images_csv) {
			$nonce = TeamUpdraft\WP_Optimize\Includes\Fragments\verify_nonce('_nonce', 'wpo_unused_images_csv');
	...
	public function load_compatibilities() {
		...
		if (defined('WCML_VERSION') && function_exists('WC')) {
	...
	public function add_forgot_password_link($html, $slug) {
		if (isset($slug) && 'wp-optimize-premium' === $slug) {

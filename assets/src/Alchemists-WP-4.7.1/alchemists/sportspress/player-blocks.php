...
<?php echo wp_kses_post( $gallery_div ); ?>
	...
	foreach ( $groups as $group ):
		...
		if ( ! empty( $group->name ) ):
			...
			$gallery .= '<div class="card__header card__header--single"><' . $grouptag . ' class="sp-gallery-group-name player-group-name player-gallery-group-name">' . $group->name . '</' . $grouptag . '></div>';
		endif;

		$gallery .= '<div class="team-roster team-roster--grid-sm team-roster--grid-col-' . $columns . '">';
		...
		if ( $show_all_players_link && ( 'position' !== $grouping || $j == count( $groups ) ) ) {
			echo '<a href="' . get_permalink( $id ) . '" class="btn btn-sm btn-default">' . esc_html__( 'View all players', 'alchemists' ) . '</a>';
		}

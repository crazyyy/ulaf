...
<tr>
	...
	<td class="image">
		...
		{if $image}
		<a href="players/{$purchase->product->url}"><img src="{$image->filename|resize:50:50}" alt="{$product->name|escape}"></a>
		{/if}
	</td>
	
	{* Название товара *}
	<td class="name">
		<a href="players/{$purchase->product->url}">{$purchase->product->name|escape}</a>
...
<ul class="tiny_products">
	...
	<li class="product">
		...
		<!-- Название товара -->
		<h3><a data-product="{$product->id}" href="players/{$product->url}">{$product->name|escape}</a></h3>

...
{$canonical="/players/{$product->url}" scope=parent}
...
<div class="product">
	...
	<div id="back_forward">
		{if $prev_product}
			←&nbsp;<a class="prev_page_link" href="players/{$prev_product->url}">{$prev_product->name|escape}</a>
		{/if}
		{if $next_product}
			<a class="next_page_link" href="players/{$next_product->url}">{$next_product->name|escape}</a>&nbsp;→
		{/if}
	</div>
...
<ul class="tiny_products">
	...
	<li class="product">
		...
		<div class="image">
			<a href="players/{$related_product->url}"><img src="{$related_product->image->filename|resize:200:200}" alt="{$related_product->name|escape}"/></a>
		</div>
		{/if}
		<!-- Фото товара (The End) -->

		<!-- Название товара -->
		<h3><a data-product="{$related_product->id}" href="players/{$related_product->url}">{$related_product->name|escape}</a></h3>

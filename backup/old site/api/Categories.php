...
class Categories extends Simpla
{
	...
	private function init_categories()
	{
		...
		$query = $this->db->placehold("SELECT c.id, c.parent_id, c.name, c.description, c.url, c.meta_title, c.meta_keywords, c.meta_description, c.image, c.visible, c.visible_logo, c.position
                                        ,c.city ,c.head, c.coach, c.create_club, c.site
    ...
    public function delete_image_c($id)
    {
        $query = $this->db->placehold("SELECT filename FROM __images_category WHERE id=?", $id);
        $this->db->query($query);
        $filename = $this->db->result('filename');
        $query = $this->db->placehold("DELETE FROM __images_category WHERE id=? LIMIT 1", $id);
        $this->db->query($query);
        $query = $this->db->placehold("SELECT count(*) as count FROM __images_category WHERE filename=? LIMIT 1", $filename);
        $this->db->query($query);
        $count = $this->db->result('count');
        if($count == 0)
        {
    ...
    function get_images_c($filter = array())
    {
        $category_id_filter = '';
        $group_by = '';

        if(!empty($filter['category_id']))
            $category_id_filter = $this->db->placehold('AND i.category_id in(?@)', (array)$filter['category_id']);

        // images
        $query = $this->db->placehold("SELECT i.id, i.category_id, i.name, i.filename, i.position, i.fio, i.pos_in_foto
									FROM __images_category AS i WHERE 1 $category_id_filter $group_by ORDER BY i.category_id, i.position");
        $this->db->query($query);
        return $this->db->results();
    }

    public function update_image_c($id, $image)
    {

        $query = $this->db->placehold("UPDATE __images_category SET ?% WHERE id=?", $image, $id);
        $this->db->query($query);

        return($id);
    }

    public function add_image_c($category_id, $filename, $name = '')
    {
        $query = $this->db->placehold("SELECT id FROM __images_category WHERE category_id=? AND filename=?", $category_id, $filename);
        $this->db->query($query);
        $id = $this->db->result('id');
        if(empty($id))
        {
            $query = $this->db->placehold("INSERT INTO __images_category SET category_id=?, filename=?", $category_id, $filename);
            $this->db->query($query);
            $id = $this->db->insert_id();
            $query = $this->db->placehold("UPDATE __images_category SET position=id WHERE id=?", $id);
            $this->db->query($query);
        }
        return($id);
    }

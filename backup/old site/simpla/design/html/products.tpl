{* Вкладки *}
{capture name=tabs}
	<li class="active"><a href="{url module=ProductsAdmin keyword=null category_id=null brand_id=null filter=null page=null}">Игроки</a></li>
...
{else}
	{$meta_title='Игроки' scope=parent}
{/if}
...
<div id="header">	
	{if $products_count}
		...
		{else}
			<h1>{$products_count} {$products_count|plural:'игрок':'игроков':'игрока'}</h1>
		{/if}		
	{else}
		<h1>Нет игроков</h1>
	{/if}
	<a class="add" href="{url module=ProductAdmin return=$smarty.server.REQUEST_URI}">Добавить игрока</a>
</div>	
	...
	<input type="hidden" name="session_id" value="{$smarty.session.id}">
		...
		<div class="{if !$product->visible}invisible{/if} {if $product->featured}featured{/if} row">
			...
			<div class="icons cell">
				<a class="preview"   title="Предпросмотр в новом окне" href="../players/{$product->url}" target="_blank"></a>			

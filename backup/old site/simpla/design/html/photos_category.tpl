...
<div class="message message_success">
	...
	<a class="link" target="_blank" href="../photos/{$category->url}">Открыть альбом на сайте</a>
	{if $smarty.get.return}
	<a class="button" href="{$smarty.get.return}">Вернуться</a>
	{/if}
	
	<span class="share">		
		<a href="#" onClick='window.open("http://vkontakte.ru/share.php?url={$config->root_url|urlencode}/photos/{$category->url|urlencode}&title={$category->name|urlencode}&description={$category->description|urlencode}&image={$config->root_url|urlencode}/files/categories/{$category->image|urlencode}&noparse=true","displayWindow","width=700,height=400,left=250,top=170,status=no,toolbar=no,menubar=no");return false;'>
  		<img src="{$config->root_url}/simpla/design/images/vk_icon.png" /></a>
		<a href="#" onClick='window.open("http://www.facebook.com/sharer.php?u={$config->root_url|urlencode}/photos/{$category->url|urlencode}","displayWindow","width=700,height=400,left=250,top=170,status=no,toolbar=no,menubar=no");return false;'>
  		<img src="{$config->root_url}/simpla/design/images/facebook_icon.png" /></a>
		<a href="#" onClick='window.open("http://twitter.com/share?text={$category->name|urlencode}&url={$config->root_url|urlencode}/photos/{$category->url|urlencode}&hashtags={$category->meta_keywords|replace:' ':''|urlencode}","displayWindow","width=700,height=400,left=250,top=170,status=no,toolbar=no,menubar=no");return false;'>
...
<input type=hidden name="session_id" value="{$smarty.session.id}">
	...
	<div id="column_left">
		...
		<div class="block layer">
			...
			<ul>
				<li><label class=property>Адрес</label><div class="page_url">/photos/</div><input name="url" class="page_url" type="text" value="{$category->url|escape}" /></li>
		...
		<div class="block layer images">
			...
			<ul>
				<li>
					...
					<img src="../{$config->photos_categories_images_dir}{$category->image}" alt="" />
				</li>
			</ul>
			{/if}
		</div>
    ...
    <div class="block layer images dropped_images">
		<h2>Добавить изображения в <a href="{url module=PhotosAdmin category_id=$category->id return=null id=null}" target="_blank">альбом</a></h2>
		<ul>
            {foreach $photos as $p}
                <li>
					<a href='#' class="delete"><img src='design/images/cross-circle-frame.png'></a>
					<img src="../{$config->photos_images_dir}{$p->filename}" alt="" />
                    <input type="hidden" value="{$p->id}" name="gallery_images[]" />
                    <a href='{url module=PhotoAdmin id=$p->id return=null}' target="_blank" class="edit"><img src='design/images/pencil.png'></a>
				</li>
            {/foreach}
        </ul>
		<div id=dropZone>
        ...
        {*или <span class=add_image_url><i class="dash_link" id="add_image_url">загрузить из интернета</i></span>*}
	</div>

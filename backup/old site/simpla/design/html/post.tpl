{capture name=tabs}
    ...
    {if in_array('articles', $manager->permissions)}<li><a href="{url module=ArticlesAdmin id=null page=null}">Статьи</a></li>
        <li><a href="{url module=ArticleCategoriesAdmin id=null page=null}">Категории статей</a></li>{/if}
    {if in_array('photos', $manager->permissions)}
...
<div class="message message_success">
	...
	<span class="share">		
		<a href="#" onClick='window.open("http://vkontakte.ru/share.php?url={$config->root_url|urlencode}/news/{$post->url|urlencode}&title={$post->name|urlencode}&description={$post->annotation|urlencode}&noparse=false","displayWindow","width=700,height=400,left=250,top=170,status=no,toolbar=no,menubar=no");return false;'>
  		<img src="{$config->root_url}/simpla/design/images/vk_icon.png" /></a>
		<a href="#" onClick='window.open("http://www.facebook.com/sharer.php?u={$config->root_url|urlencode}/news/{$post->url|urlencode}","displayWindow","width=700,height=400,left=250,top=170,status=no,toolbar=no,menubar=no");return false;'>
  		<img src="{$config->root_url}/simpla/design/images/facebook_icon.png" /></a>
		<a href="#" onClick='window.open("http://twitter.com/share?text={$post->name|urlencode}&url={$config->root_url|urlencode}/news/{$post->url|urlencode}&hashtags={$post->meta_keywords|replace:' ':''|urlencode}","displayWindow","width=700,height=400,left=250,top=170,status=no,toolbar=no,menubar=no");return false;'>
            ...
            {if $post->image}
                <ul>
                    <li>
                        ...
                        <img src="../{$config->posts_images_dir}{$post->image}" alt="" />
                    </li>
                </ul>
            {/if}
        </div>
